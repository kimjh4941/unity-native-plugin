...
3. インタラクティブ入力で参照ファイルを確定する（必須）
   ...
   - 入力がない場合は次を候補として提示する
     - `artifact/designs/<feature>/` 配下の `*-sample-scene-design*.md`（改訂版がある場合は最新バージョンのみ）
...
6. コードレビューを実施する（必須）
   ...
   **プラットフォーム独立性**（`agent-rules/coding-rules/common.md`「命名: OS 接頭辞と、共通ファイルを作らない方針」）
   - **P1**: 追加した ExampleController のファイル名と型名に OS 接頭辞が付いているか。**`*SampleSceneWiringTests` のファイル名・クラス名も対象**
   - **配置**: `UI/<Platform>/<Feature>/` に置かれているか。`UI/` は**ディレクトリでプラットフォームを分ける唯一の例外**であり、ファイル名の接頭辞も併せて必要
   - **P2**: **他プラットフォームの ExampleController / UXML / USS を変更していないか。** 共通化のために既存のサンプルへ手を入れていないか
   - **P4**: `UI/Common/`（`NativeToolkitSampleNavigator` 等）への追加が、特定プラットフォームの機能ロジックになっていないか
   - **既知の逸脱 11 件を前例として引用していないか**（`artifact/OS_PREFIX_VIOLATIONS.md`）
   - P1 / P2 / P4 と配置の違反は **A 区分**として扱う（止める基準の表を参照）
...
10. 修正対応を確認する（必須）
   ...
   **止める基準（必須）**
   ...
   区分が異なる。** 対象は既にコンパイルもテストも通ったコードなので、「実装時に気づける」
   区分は存在しない。

   | 区分 | 内容 | 扱い |
   |---|---|---|
   | **A. 画面の振る舞いを変える** | 操作が効かない、結果が表示されない、権限・プラットフォームガードが誤っている、ライフサイクルでイベント購読が漏れる・二重購読する | **止めない。直して再レビュー** |
   | **B. 検証手段を変える** | wiring test の穴、手動確認観点の抜け | **中心導線なら直す。網羅系は記録して先へ** |
   | **C. 記述だけを変える** | サンプルシーン計画書・実装結果ファイルとの不一致、UXML / USS の命名ゆれ | 安ければ直す。記録して先へでも可 |

   **A が 0 になったら止める。** ただし次の 3 条件をすべて満たすこと。

   1. **レビュアーを替えて 1 回通していること**
   2. **`*SampleSceneWiringTests` が、UXML の name を壊すと落ちることを確認済みであること。**
      UXML と Controller の name 不一致は目視では毎回すり抜けるので、機械照合に寄せる
   3. **直さない残件が、実装結果ファイルに理由つきで明記されていること**
