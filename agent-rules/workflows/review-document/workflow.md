...
4. レビュー観点を設定する（必須）
   ...
   - **サンプルシーン計画書（`*-sample-scene-design*.md`）の場合:**
     - **入力欄（`TextField`）を置く計画になっていないか。** 既定は「設けない」
       （`agent-rules/coding-rules/common.md`「サンプルシーン」）。置く場合は
       **なぜこの機能では固定 fixture で足りないのかが明示されているか**を確認する。
       理由の無い入力欄は **A1 区分**として扱う（既存サンプル画面との差分としてしか
       現れず、実装フェーズでは気づけないため）
...
8. ユーザーに改善実施を確認する（必須）
   ...
   **A1 が 0 になったら止める。** ただし次を満たすこと。
   ...
   2. **機械照合が通っていること**
      ...
      **`python3` と打たないこと。** Windows では Microsoft Store の実行エイリアスに解決され、
      **1 件も検査せずに exit 49 で終わる**。通ったのか走らなかったのか区別が付かなくなる。
