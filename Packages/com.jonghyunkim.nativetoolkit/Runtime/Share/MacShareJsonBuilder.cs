...
{
    /// <summary>
    /// Builds JSON strings for the macOS <c>shareContent</c> / <c>shareViaService</c> native
    /// operations with full control over optional fields. Does not use external JSON libraries;
    /// matches the hand-written serializer pattern used by <c>IosShareJsonBuilder</c>.
    /// </summary>
    public static class MacShareJsonBuilder
    {
        /// <summary>
        /// Builds JSON for the <c>shareContent</c> / <c>shareViaService</c> native operations.
        /// </summary>
        /// <param name="payload">Share content payload to serialize.</param>
        /// <returns>JSON string matching the <c>UnityMacShareJsonParser</c> schema.</returns>
        public static string BuildShareContentJson(MacShareContentPayload payload)
        {
            var obj = new Dictionary<string, object?> { ["items"] = BuildItemsList(payload.items) };
            AddStringArray(obj, "recipients", payload.recipients);
            AddIfNotNullOrWhiteSpace(obj, "subject", payload.subject);
            AddStringArray(obj, "excludedServiceTitles", payload.excludedServiceTitles);
            return SerializeObject(obj);
        }

        private static List<object?> BuildItemsList(MacShareItem[] items)
        {
        ...
        private static void AddStringArray(Dictionary<string, object?> target, string key, string[]? values)
        {
