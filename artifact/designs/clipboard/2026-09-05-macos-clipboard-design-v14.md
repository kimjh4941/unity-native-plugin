...
- レビュー: v11 で `artifact/reviews/clipboard/2026-09-03-macos-clipboard-design-review-v8.md`（Codex。監視領域に絞ったラウンド。A1 / A2 ゼロ、B 2 件・C 1 件）。**v12 の変更はログ方針のみでレビュー未実施**
...
結果は `artifact/results/clipboard/2026-09-05-macos-clipboard-implementation-feature-result-v5.md`（31 / 32 項目実施、macOS 26.3）。実装の欠陥は 1 件のみで、それは Runtime ではなく `Editor/Build/PostBuildProcessor.cs` にあった（修正済み・コミット `c6e9ec4`）。**本計画書が対象とする Runtime の設計に誤りは見つかっていない。**
...
**段 1 は一度実装したうえで巻き戻した。** 実装・テスト（EditMode 388 / PlayMode 44 とも pass）まで完了していたが、方針変更により `git restore` で完全に戻した。経緯は `artifact/results/clipboard/2026-09-03-macos-clipboard-implementation-feature-result-v1.md`。
...
| `Clipboard/ClipboardOperationResult.cs` | **Android 専用**（`AndroidClipboardManager` とそのテストのみが使う）。接頭辞が無いのは規則ではなく逸脱（`artifact/OS_PREFIX_VIOLATIONS.md`） | **流用しない**。macOS は数値 errorCode を返すため別型 |
