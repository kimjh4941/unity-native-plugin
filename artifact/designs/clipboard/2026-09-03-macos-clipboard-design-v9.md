...
- **改名はルール上の必須事項。** `agent-rules/coding-rules/common.md`「命名: OS 接頭辞」は、2 つ以上のプラットフォームが使う型に接頭辞を付けないと定めている。`ClipboardJsonReader` は iOS と macOS の 2 つが使うため、`Ios` 接頭辞のままにはできない
  ...
  - **v1〜v8 が根拠としていた「本パッケージは共有型を接頭辞なしで置く方針（`ClipboardOperationResult` / `ClipboardReadResult`）」は誤りだった。** 挙げた 2 型は Android 専用であり、接頭辞が無いのは規則ではなく逸脱である（2.4 / `artifact/OS_PREFIX_VIOLATIONS.md`）
