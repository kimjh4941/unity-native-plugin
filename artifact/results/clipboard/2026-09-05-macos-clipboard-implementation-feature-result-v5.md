...
サンプル計画 v3 のボタン採用基準 3（「駆動できる = 期待結果を観測できる」）を満たしていない。**2 つの fixture を別文字列に分ける修正が必要。**
...
1. **`PostBuildProcessor.cs` の修正をコミット**（§4.2）
2. **`TooLarge` と `Malformed` の分離**（§4.1）。**設計変更であり優先度は低い。** 実施する場合は計画書の改訂から入る
3. **設計書 v13 の訂正**（§5.1〜5.5）
4. **サンプルの `#26` fixture 分離**（§5.6）
5. **上限値を 32 MiB のまま維持するか iOS の 64 MiB に揃えるかを判断**（§4.1）
