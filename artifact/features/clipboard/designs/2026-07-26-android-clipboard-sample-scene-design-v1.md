# Android Clipboard サンプルシーン実装計画 v1
...
- 実装結果: `artifact/features/clipboard/results/2026-07-26-android-clipboard-implementation-feature-result-v3.md`（review-implementation-feature で LGTM 判定済み）
- 機能実装計画: `artifact/features/clipboard/designs/2026-07-26-android-clipboard-design-v4.md`
...
| Copy | `CopyMultipleTextButton` | Copy Multiple Text | `CopyMultipleText` |
...
- ボタンのクラス割り当て:
  - 主要操作（Copy 系 5 / Read 系 3 / Start Observing）: `android-clipboard-button`（青）
  - 副次・破壊・エラー系（Home / Clear / Stop Observing / Error Cases 4 / empty allowed 1）: `android-secondary-button`（グレー）
...
public class AndroidClipboardManagerExampleController : MonoBehaviour
{
    ...
    // 16 個の Button フィールド（3.1 の name に対応）
}
#endif
```
...
| 複数テキスト | Copy Multiple Text | `Success`（3 件） |
...
- 3.1 の全16ボタンが UXML と Controller で名前一致している（7.1 の EditMode テストが passed）
