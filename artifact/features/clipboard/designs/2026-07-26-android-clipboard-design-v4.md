...
対象レビュー: `artifact/features/clipboard/reviews/2026-07-26-android-clipboard-design-review-v3.md`（高優先度・中優先度の指摘なし。総合評価は承認水準）

| 指摘 | severity | 対応 |
|---|---|---|
| `stopObserving` の 0 引数 JNI 呼び出し経路を確認したことが手動確認表から読み取りづらい | low（任意対応） | 7.3 の「監視停止」行の期待に 0 引数 JNI 解決の確認を明記し、要検証 9.3 との対応を相互参照できるようにした |

### v2 からの変更点（レビュー反映）

対象レビュー: `artifact/features/clipboard/reviews/2026-07-26-android-clipboard-design-review-v2.md`（高優先度の指摘なし）
...
対象レビュー: `artifact/features/clipboard/reviews/2026-07-26-android-clipboard-design-review-v1.md`
...
| 監視停止 | `StopObserving()` → 他アプリでコピー | 発火しない。`stopObserving` の成功が届く。**あわせて 0 引数の JNI 呼び出しが解決できていることを確認する**（`AndroidJavaException` / `NoSuchMethodError` が出ず、`IsSuccess == true` が届く。要検証 9.3 の検証項目） |
...
3. **`stopObserving` の activity 不要呼び出し**: `requiresActivity` を用いた契約は 5.5 の擬似コードで確定済み。ただし `AndroidJavaObject.Call(methodName, Array.Empty<object?>())`（引数 0 個）の呼び出しが既存コードに前例が無いため、実機で JNI 解決が通ることを確認する。通らない場合は `pluginInstance.Call("stopObserving")` のオーバーロード直接呼び出しへ切り替える（`CallOperation` の失敗経路契約は変えない）
   - **検証手段**: 7.3「監視停止」の手動確認項目で確認する。`AndroidJavaException` / `NoSuchMethodError` が発生せず `stopObserving` の成功結果が届けば解決できている。失敗する場合は `CallOperation` 内の `pluginInstance.Call(operationName, fullArgs)` のみを差し替え、`TryPrepareCall` / `FireOperationResult` の契約は維持する
