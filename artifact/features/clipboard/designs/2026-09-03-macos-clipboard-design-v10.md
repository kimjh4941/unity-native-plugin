# macOS Clipboard 実装計画書 (v10)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v9.md`
- レビュー: なし（方針変更「共通ファイルを作らない」による改訂）
...
2. `RunDestroyCleanup(stop: StopObservingForTeardown, managedCleanup: ...)` を呼ぶ
   - **段 3 では `stop` に空実装を渡す。** 監視を開始する手段がまだ無く、止めるものが存在しないため。段 4 で `StopObservingForTeardown` に差し替える（12 章）
...
| `TeardownStopReissueCountForTests` | `int`（プロパティ） | D-16 の再発行が起きた回数。Editor では P/Invoke がコンパイルされないため、`StopObservingForTeardown` の呼び出し自体を数えて観測する |
...
- **copy 完了で copy の callback だけが発火し、copy のスロットと in-flight が解放されること。append も同様に、append の callback だけが発火すること** `(3)`
...
- **teardown と進行中 Start の競合** `(4)`
  - **Start を pending にしたまま `OnDestroy` を実行し、そのあとに Start の成功完了を注入すると、`clipboardStopObserving` が再発行されること**（D-16 の回帰。Editor では P/Invoke がコンパイルされないため、再発行の呼び出し自体を観測するシーム（例: `TeardownStopReissueCountForTests`）を 5.6.10 に足して検証する）
  - Start の**失敗**完了を注入した場合は再発行されないこと
...
- [ ] （段 4 のみ）5.6.8 の teardown を全体として実装し、段 3 で置いた `stop` の空実装を `StopObservingForTeardown` に置き換えている
...
- **段 3 に入るシーム**: `CompleteOperationForTests`（removePasteboard）/ `CompleteObservationControlForTests` / `CompleteSnapshotForTests` / `CompleteCreatePasteboardForTests` / `CompleteDetectPatternsForTests` / `CompleteDetectValuesForTests` / `CompleteDetectMetadataForTests` / `CompleteAccessBehaviorForTests` / `CompleteForegroundChangeForTests` / `DeliverChangeEventForTests` / `HasChangeRegistrationForTests` / `HasPendingChangeRegistrationForTests` / `TeardownStopReissueCountForTests`
