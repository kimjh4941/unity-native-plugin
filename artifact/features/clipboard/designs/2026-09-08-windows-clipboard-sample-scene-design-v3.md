...
すべて「ボタン → 実行 → 結果表示」。**入力欄は置かない**
（`agent-rules/coding-rules/common.md`「サンプルシーン」。既定であり、本機能に例外を要する理由は無い）。

- カスタムフォーマット名は固定値。未登録名の読み出しは**専用ボタン**で見る
- 履歴 item id は実行時にしか存在しないため Controller の内部状態で保持し、
  状態行に**保持の有無**だけ出す。id 自体は履歴サービスが持つ不透明な識別子で、読み手に何も伝えない

**v1 / v2 は入力欄を 2 つ置いていた。** 先行する Android / iOS の計画が明記していた既定を
理由なく破ったもので、計画レビュー 2 巡・実装レビュー 2 巡のいずれも検出できなかった。
...
**種別ごとに 1 つの補助メソッドを用意する。** 本計画の擬似コードでは `[done]` を出すものを
`AppendDone` と書く。呼び出し側が種別を選ぶのではなく種別ごとに入口を分けることで、
`[accept]` を出さずに `[done]` だけ出すような取りこぼしが呼び出し側で起きない。
実装時の名前は実装者が決めてよい。
...
太字は v2 で追加。**入力欄は無い**（4.2）。
`Paste Custom Format (unknown name)` は、入力欄で名前を変えて見ていた「未登録形式の読み出し」を
固定値のボタンに置き換えたもの。**成功・空**で返る（`FormatUnavailable` は `EmptySuccess` に正規化される）。
