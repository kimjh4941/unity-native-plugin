# macOS Clipboard 実装計画書 (v14)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03（v14 は 2026-09-05）
- 前版: `2026-09-03-macos-clipboard-design-v13.md`
- レビュー: v11 で `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v8.md`（Codex。監視領域に絞ったラウンド。A1 / A2 ゼロ、B 2 件・C 1 件）。**v12 の変更はログ方針のみでレビュー未実施**
...
## v13 からの主な変更（2026-09-05）

**実機確認 32 項目の結果を反映した版。実装の変更は伴わない。**

結果は `artifact/features/clipboard/results/2026-09-05-macos-clipboard-implementation-feature-result-v5.md`（31 / 32 項目実施、macOS 26.3）。実装の欠陥は 1 件のみで、それは Runtime ではなく `Editor/Build/PostBuildProcessor.cs` にあった（修正済み・コミット `c6e9ec4`）。**本計画書が対象とする Runtime の設計に誤りは見つかっていない。**

**訂正したのは、計画書がネイティブの挙動を説明していた記述である。**

| 変更 | 内容 |
| --- | --- |
| 1.6 | **型の派生の機序を訂正。** 「pasteboard が型を派生させる」ではなく「**書き込み側が複数の flavor を載せる**」。結論（型の完全一致を前提にするな）は変わらない |
| 1.6 | **lazy data provider に限定条件を追加。** 「終了すると貼り付けられない」は**一度も読み出されなかった場合に限る** |
| 1.6 | **pasteboard 解放後の挙動を追加。** 解放済みの名前を読むと 1507 ではなく**成功 + 空**が返る |
...
## v12 からの主な変更（2026-09-05）

**V-4（コールバック到着スレッド）を実測可能にするための変更。** 段 1〜3 の実装は完了済みだが、V-4 だけは既存の実装では測れないことが、サンプルシーン計画のレビューで判明した。

**問題:** `MacClipboardManager` はネイティブコールバックを受けたあと必ず `UnityMainThreadDispatcher.Enqueue` を通す。したがって購読者が結果を受け取る時点では**定義上メインスレッド**であり、`[MonoPInvokeCallback]` 入口に到着した瞬間のスレッドは**どこからも復元できない**。サンプルシーンからも測れない。

**対応:** 入口でスレッドを記録し、dispatcher 経由でログに出す（5.6.14）。

| 変更 | 内容 |
| --- | --- |
| 5.6.14（新設） | コールバック到着スレッドの計測。16 入口すべてに記録を追加し、**dispatcher に載せた closure の中から**ログに出す |
| 5.6.9 | **入口から Unity API を呼んではならない**規則を明文化。不一致だった場合、`Debug.Log` 自体が off-thread 呼び出しになる |
| 5.6.11 | 診断ログ 1 行を許可リストに追加（flag と count のみ） |
| 7.5 #28 | 期待を「Player.log に `callbackOnMainThread: true` が出る」に具体化 |
| 8 章 V-4 | 計測手段が用意されたことを反映 |

**この変更はサンプルシーン実装より先に入れる。** サンプル側は Manager の内部診断に依存しないため手戻りは無いが、**実機確認を 1 回で済ませるには 32 項目を走らせる前に入っている必要がある。**

## v11 からの主な変更（2026-09-05）

段 1〜3 の実装は v11 の内容で完了済み（EditMode 517 / PlayMode 116）。本版は**実機確認 32 項目に着手する前**のログ方針の変更のみで、実装の設計そのものには触れていない。

**ログ許可リストに `utType` と `intervalSeconds` を加えた**（5.6.11）。

| 変更前 | 変更後 |
| --- | --- |
| `ReadData` は `hasUtType: True` | `utType: public.png`（値） |
| `StartObserving` は間隔を出さない | `intervalSeconds: 0.5`（値） |

理由:

- **どちらもクリップボードの中身ではない。** ログ規則の目的はパスワード・トークン・文書を出さないことで、uniform type identifier と数値のポーリング間隔はどちらにも当たらない
- **この 2 つが 1302 と 1523 の唯一の診断材料である。** 実機ログに「readData failed, errorCode 1302」とだけ出ても、どの引数が空だったのか特定できない。1523 も同様で、範囲外だった間隔の値が分からなければ原因を追えない。7.5 の手動確認 32 項目はまさにこれを追う作業になる
- **`utType` には iOS の前例がある**（`IosClipboardManager` は値を出している）。v11 の記述に厳密に従った結果、macOS だけが例外になっていた

許容したリスク: **カスタム UTI はアプリ内部の型名を露出する**（例: `com.acme.internal-format`）。ただしこれはユーザーデータではなくアプリのメタデータであり、同一マシン上のどのアプリからも pasteboard 経由で観測できる情報なので、実害は低いと判断した。

## v10 からの主な変更

**段 1・段 2 は実装完了済み**（EditMode 517 / PlayMode 80 とも pass）。本版は段 3 の着手前に行った監視領域レビュー（review-v8）の反映である。**設計そのものの欠陥（A1 / A2）は指摘ゼロで、active / pending の 2 スロット方式と D-16 は Swift 正本に照らして成立している**と確認された。

| 指摘 | 分類 | 反映内容 |
| --- | --- | --- |
| B-1 | B | `TeardownStopReissueCountForTests` の計数定義が曖昧で、**D-16 を実装し忘れた実装をテストが通しうる**。`OnDestroy` 自身が `StopObservingForTeardown` を呼ぶため、全呼び出し回数として実装すると遅延 Start が来る前から値が 1 になり、`count == 1` を確認するだけのテストが素通りする。**シーム名を `TeardownStopIssueCountForTests` に改め、期待値を 1 / 2 / 1 と明示した**（5.6.10 / 7.2） |
| B-2 | B | **pending 中に届いた変更イベントの配送先を直接検証するテストが無かった。** 完了後だけを見るテストは、段階 7 で誤って active を差し替える実装でも通りうる。7.2 に「A を active、B を pending にしたままイベントを注入し、A だけが発火する」項目を追加した |
| C-1 | C | 3 段構成なのに**旧 4 段時代の段番号が残存**していた（`段 4` が 3 箇所、7.2 の `(4)` マーカーが 4 件）。字義どおり読むと**段 3 の DoD から監視テスト・D-16・`stop` の差し替えが漏れ、ゲートとして機能しない**。5.6.8 / 7.2 / 10 章の段番号を現行の 3 段に揃えた |

**C-1 の修正は Codex の推奨より 1 段広い。** Codex は「7.2 の `(4)` を `(3)` に変更」としたが、それだけでは全項目が `(3)` になり段 2 と段 3 の区別が消える。`(3)` は段 2 で実装済みの項目群なので、**`(3)` → `(2)`、`(4)` → `(3)` の 2 段シフト**として反映した。

**B-1 の反映も Codex の推奨より踏み込んでいる。** Codex は「全 teardown Stop 発行回数と定義し、期待値を明示する」を推奨したが、その定義では `Reissue`（再発行）という名前が実態と食い違ったままになる。曖昧さの原因は名前と説明の不一致なので、**名前自体を `Issue` に変えて**根本を断った。

---

## v9 からの主な変更

レビュー指摘による改訂ではない。**「機能ディレクトリに共通ファイルを作らない」という方針が決まった**ことによる。

| 分類 | 内容 |
| --- | --- |
| 共有化の取り消し | `IosClipboardJsonReader` を `ClipboardJsonReader` へ改名して iOS / macOS で共有する計画（v1〜v9 の D-1 / 4.2）を**撤回**した。iOS 側は `IosClipboardJsonReader` のまま一切変更せず、macOS 用に **`MacClipboardJsonReader.cs` を新規作成する**（既存実装からの複製 + `TryGetInt64` 追加） |
| 段の再構成 | 段 1（共有基盤）が消滅し、**4 段 → 3 段**になった。旧段 2〜4 が新段 1〜3 に繰り上がる |
| ファイル数 | 新規 17 → **18**（`MacClipboardJsonReader.cs` を追加）。既存変更は**ゼロ**になった |
| 根拠の差し替え | `common.md`「命名: OS 接頭辞と、共通ファイルを作らない方針」を参照する形に統一（D-1） |

**段 1 は一度実装したうえで巻き戻した。** 実装・テスト（EditMode 388 / PlayMode 44 とも pass）まで完了していたが、方針変更により `git restore` で完全に戻した。経緯は `artifact/features/clipboard/results/2026-09-03-macos-clipboard-implementation-feature-result-v1.md`。
...
### 0.2 成果物境界の確認結果（2026-09-03 実測。V-1 クローズ）

**一致を確認した。1 章はバイナリ 1.3.0 に対して有効である。**

| 観点 | 結果 |
| --- | --- |
| バイナリのビルド時刻 | `UnityMacPlugin` 本体 2026-09-03 13:51:37 |
| clipboard ソースの最終コミット | `549b612` 2026-09-03 12:05:22（バイナリはこれより後にビルドされている） |
| 未コミットの Swift 変更 | なし（`git status` の差分は `.pbxproj` と Xcode の UI 状態のみ） |
| エラーメッセージ文字列 5 種 | `Pasteboard ownership was lost` / `Observation interval must be greater than 0 and at most 60 seconds.` / `The type filter must not be empty` / `Pasteboard detection requires macOS` / `Standard pasteboard cannot be released` をバイナリ内に確認 |
| JSON スキーマのキー 7 種 | `probableWebURL` / `shipmentTrackingNumbers` / `matchingItemIndexes` / `contentTypeIdentifier` / `changeCount` / `localOnly` / `alwaysAllow` を確認 |
| 検出パターンが複数形であること | `phoneNumbers` / `emailAddresses` / `moneyAmounts` を確認（5.1.1 の対応表が iOS の単数形と異なる根拠） |
| C 関数 15 個 | `nm -gU` で確認済み（3.1） |

ネイティブを再ビルドした場合は、この確認をやり直すこと。
...
| `OwnershipJson` | `{"scope":ScopeJson,"changeCount":Int}`。**Swift の `Int` は 64bit macOS で 64 ビット**なので、C# 側は `long` で受ける（5.5.1） |
...
- **読み出しは書き込みの鏡ではない**（実測で機序を確定）
  - 型を増やしているのは**書き込み側**である。AppKit を使うアプリはリッチテキストをコピーするとき、`public.rtf` / `public.utf8-plain-text` に加えて `public.utf16-external-plain-text` などの flavor を**自分で宣言して載せる**
  - **読み出し時に pasteboard が型を派生させるのではない。** `Read` は宣言済みの型を列挙するだけである
  - したがって**本ライブラリ自身の書き込みでは型は増えない**（実測: `Copy`（HTML 2 型）→ 2 型、`Copy`（URL 1 型）→ 1 型）。増えるのは**他アプリが書いた内容を読むとき**である
  - 結論は変わらない。**`Read` の結果が `Copy` の入力と一致する前提を置かない。件数や完全一致で判定する利用者コードは他アプリの内容で壊れる**
...
- **10 MiB 超の単一 item は lazy data provider 経路に入る**（重要）
  ...
  - つまり **Copy 成功 ≠ 貼り付け可能**。ただし条件がある（実測）
    - **一度も読み出されないままプロセスが終了すると、データは失われる**
    - **一度でも読み出されればバイトが実体化し、プロセス終了後も pasteboard に残る**
    - 実用上の危険は「コピーしたので安心してアプリを閉じた」場合に限られる
...
- **解放しても名前は消えない**（実測）。`RemovePasteboard` は「pasteboard を消滅させる」のではなく「**内容を破棄する**」。解放済みの名前を再び読むと **1507 ではなく成功 + 空（`items` が 0）**が返る
  - この結果、**1507 に到達する経路が現状の公開 API には存在しない**（11 章の到達可能性表は 1507 を Read / Snapshot に割り当てているが、実機では再現できていない）
...
- **受信側にも 32 MiB の上限がある**（`MacClipboardLimits.MaxResponseBytesPerRepresentation`、5.2.2）。**representation ごと**の上限であり合計ではない
  - 他アプリが 32 MiB を超える representation を置くと、`Read` は **9006**（`ResponseParseFailed`）で失敗する。base64 デコード時の `TooLarge` を 9006 に集約しているため（5.5.1）
  - **実測で境界を確認した。** 判定は厳密超過（`>`）なので **ちょうど 32 MiB は通る**（32 MiB OK / 36 MiB NG）。失敗は 1 秒未満で結果として返り、クラッシュもフリーズもしない
  - **他アプリが置いた内容なのでアプリ側から制御できない。** 利用者はこの失敗を通常経路として扱う必要がある（8 章 V-14）
- **macOS 15.4 の分岐**
...
- **監視はアプリ非アクティブ中に停止し、アクティブ復帰時に追いつく**。他アプリの変更は前面復帰時に報告される
  - **追いつきは畳み込まれない**（実測）。非アクティブ中に N 回変更があれば、復帰時に **N 件のイベントが `changeCount` 昇順で配信される**。クリップボード履歴のような用途でも取りこぼさない
  - **`CheckForegroundChange` と併用してはならない**（実測）。監視のポーリングが基準 `changeCount` を先に更新するため、監視中の `CheckForegroundChange` は**ほぼ常に `false`** を返す。監視していない状態なら初回 `true` / 2 回目 `false` と正しく動く
- **`startObserving` の再呼び出しは新しい設定で再開する**（重複購読にならない）。`stopObserving` は冪等
- **`startObserving` / `stopObserving` は投入順に実行される保証が無い。** Swift ファサードはどちらも非構造化の `Task { @MainActor in ... }` を投入して即座に戻る。MainActor は排他実行を保証するが、**投入順の実行は保証しない**。C# から Start → Stop の順に呼んでも、ネイティブ側で Stop → Start の順に実行されうる
  - 通常経路では single-flight（`ObservationControlKey` の共有）がこの交差を防ぐ
  - **防げないのは teardown だけ。** `OnDestroy` はガードチェーンを迂回して直接 Stop を呼ぶため、Start の応答待ち中に交差しうる（5.6.8）
- **再 Start が失敗した場合、既存の監視はそのまま継続する。** `ClipboardChangeMonitor.start()` は interval の検証と scope の解決を `stop()` の**前**に行う。実装コメントに「Resolve before touching any existing observation. A caller that mistypes a scope must not lose the monitoring they already had (M-5, IT-09)」とあり、ネイティブテスト IT-09 がこの契約を保証している
  - つまり `StartObserving` が 1523（interval 不正）や scope 解決の失敗を返しても、**ネイティブ側は前回の監視を動かし続けている**
  - C# 側もこれに合わせ、失敗した Start で既存の `onChanged` 登録を失ってはならない（5.6.5）
...
- **監視の世代管理**: context を持てない static コールバックへ「どの登録に責任を持つか」を渡す（macOS 版は active / pending の 2 スロットで置き換える。5.6.5 / D-13）
- **`[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` による static リセット**
- **`#if UNITY_EDITOR` のテストシーム群**: `ResetForTests` / `BridgeAvailableOverrideForTests` に加え、`CompleteOperationForTests` / `CompleteObservationControlForTests` / `CompleteReadForTests` / `CompleteSnapshotForTests` / `DeliverChangeEventForTests` / `IsInFlightForTests` / `HasChangeRegistrationForTests` / `PendingObservationGenerationForTests`（iOS 固有。macOS は `HasPendingChangeRegistrationForTests`。5.6.10） / `HasAnyPendingCallbackForTests` / `InFlightCountForTests`
...
| 型 | 現状 | 判断 |
| --- | --- | --- |
| `Clipboard/IosClipboardJsonReader.cs`（`JsonValue` / `JsonValueKind` / `JsonBase64Status` を含む） | `#if UNITY_IOS \|\| UNITY_EDITOR`。iOS のみが利用 | **共有化しない。** `common.md`「共通ファイルを作らない方針」により、macOS 用に `MacClipboardJsonReader.cs` を複製する（4.1 #4）。**iOS 側は一切変更しない** |
| `Clipboard/ClipboardOperationResult.cs` | **Android 専用**（`AndroidClipboardManager` とそのテストのみが使う）。接頭辞が無いのは規則ではなく逸脱（`artifact/topics/os-prefix-violations/README.md`） | **流用しない**。macOS は数値 errorCode を返すため別型 |
| `Clipboard/ClipboardReadResult.cs`（`ClipItem` / `ClipContents`） | **Android 専用**。ClipData モデル。接頭辞が無いのは逸脱 | **流用しない**。macOS は UTI → bytes の representation モデル |
...
| 4 | `MacClipboardJsonReader.cs` | 手書きの再帰下降 JSON リーダー。`MacJsonValue` / `MacJsonValueKind` / `MacJsonBase64Status` / `MacClipboardJsonReader` を持つ。**`IosClipboardJsonReader.cs` からの複製 + `TryGetInt64` 追加**（5.4.1） |
| 5 | `MacClipboardJsonBuilder.cs` | 7 種の入力 JSON を組み立てる `public static` メソッド群 |
| 6 | `MacClipboardJsonParser.cs` | 12 種の出力（出力専用 9 + 共用 2 + イベント 1）を結果型へ変換する `internal static` メソッド群 |
| 7 | `MacClipboardOperationResult.cs` | 値を返さない操作（removePasteboard / startObserving / stopObserving） |
| 8 | `MacClipboardOwnershipResult.cs` | copy / append。`Operation` を持つ |
| 9 | `MacClipboardReadResult.cs` | `MacClipboardItem`（読み出し item） / `MacClipboardReadContents`（`ChangeCount` + `Items`） / `MacClipboardReadResult` の 3 型 |
| 10 | `MacClipboardReadDataResult.cs` | `byte[]? Data`（型不在は成功 + null） |
| 11 | `MacClipboardSnapshotResult.cs` | `MacClipboardSnapshot` と結果型 |
| 12 | `MacClipboardChangeCountResult.cs` | `clear` の結果 |
| 13 | `MacPasteboardScopeResult.cs` | `createPasteboard` の結果 |
| 14 | `MacClipboardDetectionResults.cs` | `MacClipboardDetectedLink` / `MacClipboardLabeledValue` / `MacClipboardPostalAddress` / `MacClipboardCalendarEvent` / `MacClipboardShipmentTracking` / `MacClipboardFlightNumber` / `MacClipboardMoneyAmount` / `MacClipboardDetectedValues` / **`MacClipboardDetectedMetadata`** と、patterns / values / metadata の 3 結果型 |
| 15 | `MacClipboardAccessBehaviorResult.cs` | `accessBehavior` の結果 |
| 16 | `MacClipboardForegroundChangeResult.cs` | `checkForegroundChange` の結果 |
| 17 | `MacClipboardChangeEvent.cs` | 監視イベント（結果型ではない） |
| 18 | `MacClipboardManager.cs` | Manager 本体 |

内訳: payload / 定数 / エラー 3 ファイル、Reader / Builder / Parser 3 ファイル、**結果型 12 種を 10 ファイル**（`MacClipboardDetectionResults.cs` が 3 結果型を持つ）、イベント 1 ファイル、Manager 1 ファイル。

### 4.2 既存変更（Runtime）

**なし。** `common.md`「共通ファイルを作らない方針」により、既存の iOS / Android 実装には一切触れない。

v1〜v9 は `IosClipboardJsonReader.cs` を `ClipboardJsonReader.cs` へ改名して共有する計画だったが、**撤回した**（D-1）。macOS は `MacClipboardJsonReader.cs`（4.1 #4）を自前で持つ。

### 4.3 新規作成（Tests）— 6 ファイル
...
| ファイル | 内容 |
| --- | --- |
| `MacClipboardJsonReaderTests.cs` | JSON リーダーの仕様（パース・アクセサ・エスケープ・サロゲートペア・深さ制限・base64 のデコード長）。**`IosClipboardJsonReaderTests.cs` からの複製 + `TryGetInt64` のケース追加** |
...
| ファイル | 内容 |
| --- | --- |
| `MacClipboardManagerIntegrationTests.cs` | ガードチェーンの 6 段、dispatch 順序、監視の active / pending 遷移、tombstone、完了シーム経由の結果配送 |

### 4.4 既存変更（Tests）

**なし。** 既存の iOS テストには触れない。
...
public sealed class MacPasteboardOwnership
{
    ...
    /// Swift の Int は 64bit macOS で 64 ビット。int に狭めると、int.MaxValue を超えた
    /// 時点で正常なネイティブ応答が 9006 に化ける（5.5.1 の「changeCount の幅」）。
...
`MacClipboardJsonReader.Parse(json)` で `MacJsonValue` を得てから結果型へ変換する。`internal static`。
...
| 6 | `TryParseChangeCount` | `ChangeCountJson` | `long` |
...
- **`changeCount` は `long` で読む。** 複製元の `TryGetInt` は `int` 範囲外を**失敗**として返すため、そのまま使うと `changeCount > int.MaxValue` の正常な応答が 9006 に化ける。`MacJsonValue` に `TryGetInt64(out long)` を追加し、`OwnershipJson` / `ReadResultJson` / `SnapshotJson` / `ChangeCountJson` / `ChangeEventJson` の `changeCount` はこれで読む（5.4.1）
- base64 のデコードは `MacJsonValue.TryGetBase64Bytes(maxDecodedLength, out bytes)` を使い、`JsonBase64Status.TooLarge` は失敗（9006）にする。`maxDecodedLength` は `MacClipboardLimits.MaxResponseBytesPerRepresentation`（5.2.2）を渡す
...
#### 5.4.1 `MacClipboardJsonReader.cs`

`IosClipboardJsonReader.cs`（803 行）を複製し、macOS 用に調整する。**iOS 側は一切変更しない**（`common.md`「共通ファイルを作らない方針」）。

| 元 | 複製後 |
| --- | --- |
| `IosClipboardJsonReader` | `MacClipboardJsonReader` |
| `JsonValue` | `MacJsonValue` |
| `JsonValueKind` | `MacJsonValueKind` |
| `JsonBase64Status` | `MacJsonBase64Status` |

- ガードは `#if UNITY_STANDALONE_OSX || UNITY_EDITOR`
- **`TryGetInt64(out long)` を追加する**（D-14）。既存 `TryGetInt(out int)` も残す。実装は `TryGetInt` と同じ二段構え（`long.TryParse` を試し、失敗したら整数値の `double`（`"3.0"` 形式）を受け入れる）
- クラスの XML `<summary>` は macOS のスキーマに合わせて書き直す。iOS 固有の記述（`allTypeIdentifiers` / `loadItem` is polymorphic on `kind`）は macOS に存在しないため、macOS の該当例（`itemTypes` の配列の配列、`ReadDataJson.data` の明示的 null）に差し替える
- ログを出さない方針は複製元から引き継ぐ

**重複を許容する理由:** 803 行の重複コストより、iOS と macOS が独立して変更できることを取る（`common.md`）。複製後は別の型なので、片方の修正がもう片方に波及しない。
...
| 値型（`long` / `bool` / enum） | **既定値**（`0` / `false` / `Unknown`） | `ChangeCount`（`long`。D-14） / `Changed` / `Behavior` |
...
- `IsSuccess == true` ⇔ `Error == null`。**ただし factory（`Success` / `Failure`）が生成した値に限る**
- **`default(T)` は失敗結果ではなく「未初期化値」である。** `readonly struct` の既定値は `IsSuccess = false` / `Error = null` / `Operation = null` になり、上の不変条件も「`Operation` は非 null」も破る。Manager は `default` を返さず、必ず factory を通す
  - 結果型を `sealed class` にすれば `default` は null になり型で防げるが、既存の `Ios*` / `Mac*` 結果型がすべて `readonly struct` なのでパッケージ全体の一貫性を優先する
  - 呼び出し側が `default` を作れてしまう点は残る。XML コメントに「`default` を結果として扱わない」と明記する
...
| `MacClipboardReadContents` | `long ChangeCount` / `IReadOnlyList<MacClipboardItem> Items` | Items は常に非 null。**iOS の `IosClipboardReadResult` はフラット構成だが、macOS は `changeCount` と `items` が ownership と同じ「その時点の pasteboard 状態」を表す 1 組の値なので入れ子にする**（`MacClipboardSnapshotResult` が `MacClipboardSnapshot` を入れ子にしているのと同じ理由） |
...
| `MacClipboardSnapshot` | `long ChangeCount` / `IReadOnlyList<IReadOnlyList<string>> ItemTypes` / `IReadOnlyList<int> MatchingItemIndexes` | いずれも常に非 null。`MatchingItemIndexes` は managed collection の添字なので `int` のまま |
| `MacClipboardSnapshotResult` | `bool IsSuccess` / `MacClipboardErrorInfo? Error` / `MacClipboardSnapshot? Snapshot` | |
| `MacClipboardChangeCountResult` | `bool IsSuccess` / `MacClipboardErrorInfo? Error` / `long ChangeCount` | 失敗時は `ChangeCount == 0` |
...
| `MacClipboardChangeEvent` | `MacPasteboardScope Scope` / `long ChangeCount` | イベントなので成否を持たない |
...
/// onNativeFailure は pending 登録の破棄など、操作固有の巻き戻しに使う（5.6.5）。
...
3. 段階 7 で登録するのは **pending スロットだけ**。`s_onChanged`（active）には触れない（5.6.5）
4. `InvokeNative` の `onNativeFailure` に pending の破棄を渡す
...
// 段階 7。pending にだけ載せる。active（s_onChanged）には触れない。
// ネイティブは再 Start が失敗しても既存の監視を止めないので、成功するまで
// 旧登録を生かしておく必要がある（1.6 / IT-09）。
...
InvokeNative(op, key,
    ...
    onNativeFailure: () => s_pendingOnChanged = null);   // active は維持する
    // startObserving は MacClipboardOperationResult を返すので
    // onNativeFailureResult は不要（既定のフォールバックが正しい型を組み立てる）。
```

`StopObserving` は pending を作らない。段階 7 では `s_onStopObserving = onResult;` だけを行い、active の破棄は**成功完了時**に行う（5.6.5）。
...
| 7 | per-call スロット格納・pending 登録・P/Invoke | 9002 | 通常経路 `FireXxxResult`（**`EndOperation` する**） |
...
#### 5.6.5 監視登録の active / pending 分離

`StartObserving` / `StopObserving` は `MacClipboardOperations.ObservationControlKey` を共有し、同時に 1 件だけ実行できる（5.2.1）。

**ネイティブは再 Start が失敗しても既存の監視を止めない**（1.6）。したがって C# 側も、ネイティブが成功を返すまで既存の登録に触れてはならない。登録を 2 本のスロットに分ける。

| スロット | 意味 |
| --- | --- |
| `s_onChanged`（**active**） | **成功した** `StartObserving` の登録。変更イベントの配送先はこれだけ |
| `s_pendingOnChanged`（**pending**） | 進行中の `StartObserving` の候補。まだ 1 度も配送先にならない |

状態遷移:

| 契機 | active（`s_onChanged`） | pending（`s_pendingOnChanged`） |
| --- | --- | --- |
| `StartObserving` 段階 7 | **変更しない** | `= onChanged` |
| `StartObserving` 完了・成功 | `= s_pendingOnChanged`（**昇格**） | `= null` |
| `StartObserving` 完了・失敗 | **変更しない**（旧登録を維持） | `= null` |
| `StartObserving` 段階 7 の P/Invoke 例外 | **変更しない** | `= null` |
| `StopObserving` 完了・成功 | `= null` | 変更しない（空のはず） |
| `StopObserving` 完了・失敗 / 段階 7 の例外 | **変更しない**（ネイティブは停止していない） | 変更しない |
| `ClearAllPendingCallbacks` | `= null` | `= null` |
| 破棄後に届いた `StartObserving` の**成功**完了 | すでに null | すでに null。**加えて `clipboardStopObserving` を再発行する**（5.6.8 / D-16） |

- **配送は active だけを見る。** `FireClipboardChanged` は `Dispatch(e, _instance?.ClipboardChanged, s_onChanged)` を呼ぶ。再 Start の処理中に旧監視のイベントが届いても、まだ昇格していない新 callback には流れない
- 初回 Start の失敗では active はもともと null なので、「旧登録を維持する」は「null のまま」になる。ネイティブも未監視なので一致する

**v6 までの世代カウンタ（`s_observingGeneration` / `s_onChangedGeneration` / `s_pendingObservationGeneration`）と `ReleaseChangeRegistrationIfOwned()` は撤去する**（D-13）。

- 世代は「context を受け取れない static コールバックへ、どの登録に責任を持つかを渡す」ために置いていた。pending スロットがその識別そのものになるため役割が重複する
- single-flight により制御呼び出しは同時に 1 件だけなので、届いた完了コールバックは pending スロットの持ち主以外にありえない。世代の比較は不要
- 破棄後に届く遅延コールバックは tombstone（`DiscardIfTerminated`）が捨てる。世代とは別の機構である
- v3 から v6 まで、この世代機構の周りで A クラスの指摘が 3 回出ている。表現できない状態を比較演算で近似していたことが原因なので、状態そのものを 2 スロットで持つ形に置き換える
...
2. `RunDestroyCleanup(stop: StopObservingForTeardown, managedCleanup: ...)` を呼ぶ
   - **段 2 では `stop` に空実装を渡す。** 監視を開始する手段がまだ無く、止めるものが存在しないため。段 3 で `StopObservingForTeardown` に差し替える（12 章）
3. `managedCleanup` は `ClearAllPendingCallbacks()`（per-call スロット 17 本の全消し・`s_inFlight.Clear()`。5.6.12）を行い、`finally` で `_instance = null` にする
...
- **`StopObservingForTeardown` はガードチェーンを迂回してネイティブを直接叩く。** `ObservationControlKey` が in-flight（`StartObserving` の応答待ち）である可能性がある
  - **managed 状態は安全**。tombstone が先に立っており新規操作は入らず、到着するコールバックはすべて `DiscardIfTerminated` で捨てられる
  - **ネイティブ状態は安全ではない。** ネイティブの Start / Stop は投入順に実行される保証が無い（1.6）。teardown の Stop が先に走り、そのあとに遅延した Start が実行されると、**破棄後もネイティブの poller が 0.5 秒ごとに pasteboard を読み続ける**。tombstone は managed 配送を捨てるだけで、ネイティブの監視を止めない
  - **対策（D-16）**: `DiscardIfTerminated` が **`startObserving` の成功完了**を捨てるとき、捨てる前に `clipboardStopObserving(null)` を再発行する

```csharp
// 破棄後に届いた startObserving の完了。ネイティブは Stop より後に実行された可能性があり、
// その場合 poller が動き出している。managed 側は tombstone で捨ててよいが、
// ネイティブは明示的に止めないと残る。stopObserving は冪等なので、
// 交差していなかった場合に呼んでも害はない。
private static void HandleStartObservingCallback(bool isSuccess, long errorCode, string? errorMessage)
{
    if (s_isTerminated)
    {
        if (isSuccess) { StopObservingForTeardown(); }
        return;
    }
    // 通常経路（5.6.5 の昇格 / 破棄）
}
```
  ...
  - 失敗した Start は監視を開始していないので再発行は不要
  - `stopObserving` は冪等（6.4）なので、交差していなかった場合に呼んでも副作用が無い
  - この規則は `startObserving` だけに要る。他の 14 操作はネイティブ側に残る状態を作らない
- 順序は「`stop` → `managedCleanup`」。active / pending 登録のクリアは `stop` の**後**に走る
...
- **入口では Unity API を呼んではならない**（`Debug.Log` を含む）。ネイティブが**どのスレッドから呼ぶかは実測できていない**（8 章 V-4）ため、入口はメインスレッドである保証が無い。とくに 5.6.14 の計測はスレッド不一致を検出するためのものであり、**不一致だったときに `Debug.Log` を呼ぶのは、まさにその検出対象の違反そのものになる。** 記録は interlocked な整数演算だけで行い、出力は dispatcher に載せた closure の中で行う
...
Editor では P/Invoke がコンパイルされないため、`BridgeAvailableOverrideForTests` だけでは操作が永久に pending のままになる。結果配送・in-flight 解放・active / pending 遷移・tombstone を検証するには**完了シームが必須**。`IosClipboardManager.cs:1656-1704` と同形で用意する。
...
| `TeardownStopIssueCountForTests` | `int`（プロパティ） | **teardown が `clipboardStopObserving` を発行した総回数。**「再発行だけの回数」ではない。Editor では P/Invoke がコンパイルされないため、`StopObservingForTeardown` の**呼び出し自体**を数えて観測する。`ResetForTests()` で 0 に戻す |
...
| `CompleteObservationControlForTests` | `(string operation, bool isSuccess, long errorCode, string? errorMessage)` | **startObserving / stopObserving 専用。** この経路でのみ 5.6.5 の active / pending 遷移（start 成功で昇格、start 失敗で pending 破棄、stop 成功で active 破棄）が走る |
...
| `HasChangeRegistrationForTests` / `HasPendingChangeRegistrationForTests` | なし | active / pending 登録の観測（5.6.5） |
| `HasAnyPendingCallbackForTests` | なし | per-call スロット 17 本（5.6.12）のリーク検出 |

- **`CompleteObservationControlForTests` を `CompleteOperationForTests` で代用してはならない。** 通常の operation 完了経路には pending の昇格・破棄が無く、7.2 の「再 Start 失敗で旧登録が残る」「再 Start 成功で置換される」を駆動できない
- **`TeardownStopIssueCountForTests` は「総発行回数」であり、初期値が 0 でないことに注意する。** `OnDestroy` 自身が `StopObservingForTeardown` を 1 回呼ぶため、破棄直後の値は **1** である。したがって「1 であること」を確認するテストは、**D-16 の再発行を実装し忘れていても pass してしまう**。期待値は 7.2 のとおり `OnDestroy` 直後 1 / 遅延 Start 成功後 **2** / 遅延 Start 失敗後 **1** の 3 点で固定する
  - v10 の名称は `TeardownStopReissueCountForTests`（再発行回数）だったが、数える対象は総発行回数なので名前が実態と食い違っていた。この食い違い自体が上の取り違えを招くため改名した（review-v8 B-1）
...
- `s_inFlight` / per-call スロット / active・pending 登録はメインスレッドからしか触らないためロック不要
...
- 出してよいのは shape / count / flag と、**呼び出し側が指定した非機微な設定値**に限る
  - shape / count / flag: `itemCount`、`representationCount`、`totalBytes`、`hasScope`、`scopeKind`、`hasCallback`、`operation`、`errorCode`
  - **設定値: `utType`、`intervalSeconds`**（v12 で追加）
  - **診断: `callbackOnMainThread`（flag）と `samples` / `mismatches`（count）**（v13 で追加。5.6.14）
- **設定値を許可する条件は「クリップボードの中身でないこと」と「特定のエラーコードの唯一の診断材料であること」の両方**である。`utType` は 1302、`intervalSeconds` は 1523 に対応する。この 2 条件を満たさない値を後から足さないこと
  - 具体的に**出してはならない**もの: representation のバイト列、`Read` / `Snapshot` が返した型名（他アプリのカスタム UTI を含みうる）、pasteboard 名、検出された値（`DetectValues` の結果）
...
| `s_onChanged`（active） / `s_pendingOnChanged`（pending） | `Action<MacClipboardChangeEvent>?` | 監視イベント（操作ではなく登録単位）。配送先は active のみ。5.6.5 |

計 17 本（操作 15 + 変更イベントの active / pending 2）。
...
- `ClearAllPendingCallbacks()` は 17 本すべてを null にする。`HasAnyPendingCallbackForTests` も 17 本すべてを見る
...
#### 5.6.14 コールバック到着スレッドの計測（V-4）

**目的は 1 つだけ。** ネイティブコールバックが Unity のメインスレッドに到着しているかを macOS Standalone Player で実測し、V-4 を閉じる。

**なぜ他の手段で測れないか:**

- 購読者側では測れない。`Dispatch` が `UnityMainThreadDispatcher.Enqueue` を通すため、購読者が動く時点では必ずメインスレッドである
- Editor でも測れない。`[MonoPInvokeCallback]` 本体は `#if UNITY_STANDALONE_OSX && !UNITY_EDITOR` の中にあり、コンパイルされない
- Swift 側の `Thread.isMainThread` でも足りない。それは **AppKit のメインスレッド**であることしか示さず、Unity が `Awake` で記録した managed のメインスレッド ID と一致することの証明にならない

**構成:**

| 要素 | 内容 |
| --- | --- |
| `s_callbackThreadSamples` | 到着したコールバックの総数。`Interlocked.Increment` |
| `s_callbackThreadMismatches` | メインスレッド以外から到着した数。`Interlocked.Increment` |
| `RecordCallbackThread()` | **入口で呼ぶ。** `Thread.CurrentThread.ManagedThreadId` と `s_mainThreadId` を比較して上の 2 つを更新するだけ。Unity API に一切触れない |
| `FlushCallbackThreadDiagnostics()` | **dispatcher に載せた closure の先頭で呼ぶ。** 初回の到着で 1 行、初回の不一致でもう 1 行だけログに出す |

- **16 入口すべてに `RecordCallbackThread()` を置く**（操作 15 + 変更通知 1）。ABI 形が 3 種類（値あり / 値なし / 変更）あり、どれか 1 つだけ測っても他が同じ経路とは限らない
- 記録は `DiscardIfTerminated` より**前**に行う。破棄されるコールバックも到着はしているため、計測対象である
- 出力は最大 2 行。連続で出すとログが本来の確認項目を埋める
- 破棄されたコールバックは dispatch に進まないため、その分の flush は**次の dispatch まで遅れる**。計測値は失われない

**ログの形（5.6.11 の許可リスト内）:**

```
[MacClipboardManager] callbackOnMainThread: true (samples=1)
[MacClipboardManager] callbackOnMainThread: false (samples=7, mismatches=1)
```

flag と count のみで、clipboard 本文にも pasteboard 名にも触れない。

**Editor では検証できない。** 入口がコンパイルされないため `RecordCallbackThread()` は呼ばれず、EditMode / PlayMode のどちらでも計測値は 0 のままである。**これは欠陥ではなく V-4 が実機項目である理由そのものである。** 自動テストで担保するのは次の 1 点だけとする。

- `FormatCallbackThreadDiagnostic(bool onMainThread, int samples, int mismatches)` を `internal static` の純粋関数として切り出し、**文言と、機微情報を含まないこと**を EditMode で検証する

`ResetForTests()` と `[RuntimeInitializeOnLoadMethod]` の両方で 2 つのカウンタと報告済みフラグを 0 に戻す。

### 5.7 実装順序

12 章の 3 段階に対応する（対応は下表の「段」列）。各段の完了条件は 10 章。
...
1. `MacClipboardPayloads.cs` / `MacClipboardErrorInfo.cs` / `MacClipboardConstants.cs`
2. `MacClipboardJsonReader.cs`（複製 + `TryGetInt64`）+ `MacClipboardJsonReaderTests.cs`
...
9. 残り 10 操作 + 監視の active / pending 遷移 + 対応するテスト
...
- base64 上限超過の失敗
- **`changeCount` が `int.MaxValue + 1` の `OwnershipJson` / `ReadResultJson` / `SnapshotJson` / `ChangeCountJson` / `ChangeEventJson` が成功として解析されること**（`long` 幅の回帰。`int` で読むと 9006 になる）
...
- **`default(T)` を失敗結果として扱わないこと。** `default` は `Error == null` かつ `Operation == null` になるため、不変条件の検証対象から除外する。代わりに「factory を通した値だけが不変条件を満たす」ことと、`Failure` が必ず `Error != null` / `Operation != null` を返すことを検証する
...
各**テスト項目**の末尾の `(2)` / `(3)` は、12 章のどの段で追加するかを示す。**段 2 の DoD は `(2)` の項目、段 3 の DoD は `(3)` の項目で判定する。** マークが付かない行はテスト項目ではなく実行上の注意である（ガードチェーンの行は子項目側にマークを持つ）。
...
- ガードチェーン 6 段の各拒否経路（段階 1: 9003 / 段階 2: 9004 / 段階 3: 9005・9007 / 段階 4: 9002 / 段階 5: 9002 / 段階 6: 9001）を駆動する
  ...
  - 6 段すべて `(2)`
- dispatcher 経由で共通イベント → 個別 callback の順序が保たれること `(2)`
- **`EndOperation` が dispatch より前に走ること**（callback の中から同じ操作を再実行しても 9001 にならない）`(2)`
- **copy 完了で copy の callback だけが発火し、copy のスロットと in-flight が解放されること。append も同様に、append の callback だけが発火すること** `(2)`
  - `MacClipboardOwnershipResult.Operation` の値を確認するだけでは足りない。`FireOwnershipResult` が `s_onCopy` を決め打ちしていても値の検証は通ってしまうため、**両操作の callback をそれぞれ登録して、呼ばれた方と呼ばれなかった方を確認する**
- 監視の active / pending 遷移 `(3)`
  - Start 成功後に `HasChangeRegistrationForTests == true` になり、変更イベントを注入すると `onChanged` が発火する
  - **再 Start が pending の「最中」に届いたイベントが、旧 callback にだけ流れること**（review-v8 B-2）
    - 手順: callback A で Start を成功させる → callback B で再 Start を開始し、**完了を注入せず pending のまま保持する** → `DeliverChangeEventForTests` を実行 → **A だけが発火し B は発火しない**。そのあと B を成功完了させ、次のイベントでは **B だけが発火する**
    - この窓を検査しないと、**段階 7 で誤って active を B に差し替える実装**でも、完了時の巻き戻し次第で「再 Start 失敗後」「再 Start 成功後」のテストを通せてしまう
    - ネイティブ側でも実在する経路である。Start の `Task` が実行を開始するまで旧監視が生きているため、この窓で届くイベントは旧監視のものになる
    - 既存の `DeliverChangeEventForTests` で駆動できる。新しいシームは要らない
  - **成功済み Start → 再 Start 失敗のあと、旧 callback が残っており新 callback は発火しない**（1.6 のネイティブ契約と一致させる）
  - 再 Start 成功時に初めて旧 callback から新 callback へ置換される
  - Stop 成功で active 登録が失われる
  - **ネイティブ呼び出しの開始前に失敗した場合（段階 5 / 6 / 段階 7 の P/Invoke 例外）は旧登録を維持する**
  - どの段階でも `HasPendingChangeRegistrationForTests` が完了後に false へ戻ること
  - **Stop の完了が失敗を返した場合、active 登録が維持されること**（ネイティブは停止していない）
- **teardown と進行中 Start の競合** `(3)`
  - **Start を pending にしたまま `OnDestroy` を実行し、そのあとに Start の成功完了を注入すると、`clipboardStopObserving` が再発行されること**（D-16 の回帰。Editor では P/Invoke がコンパイルされないため、`TeardownStopIssueCountForTests`（5.6.10）で発行回数を観測する）
    - **期待値は 3 点で固定する: `OnDestroy` 直後 = 1、遅延 Start の成功完了後 = 2、遅延 Start の失敗完了後 = 1。** このカウンタは総発行回数であり `OnDestroy` 自身の 1 回を含むため、**「1 であること」だけを確認するテストは D-16 未実装でも pass する**（review-v8 B-1）
  - Start の**失敗**完了を注入した場合は再発行されないこと（カウンタが 1 のまま）
  - `OnDestroy` 後に active / pending の両方が null になっていること
  - 破棄後に届いた変更イベントが捨てられること（既出）
- observation 共有キーの Busy メッセージが専用文言であること `(3)`
- Editor（非 macOS Player）での失敗経路 `(2)`
- **破棄前に取得した参照を保持したまま**呼んだ場合、9004 で拒否され共通イベントが発火すること（決定 2 が保証する範囲）`(2)`
- 破棄後に `Instance` から取り直して呼んだ場合、9004 で拒否されるが**旧購読者には届かない**こと（決定 2 が保証しない範囲。5.6.2）`(2)`
...
**2026-09-05〜09-06 に macOS 26.3 で 32 項目すべてを実施した。** 結果は `artifact/features/clipboard/results/2026-09-05-macos-clipboard-implementation-feature-result-v5.md`。
...
| 4 | `Read`（他アプリがコピーしたテキスト） | 読める。**リッチテキストなら書き込み側が載せた flavor（`utf16-external` など）も含まれる**ことを確認。plain text では増えない（1.6） |
...
| 9 | `CreatePasteboard(Unique)` → `Copy` → `Read` → `RemovePasteboard` | 一連が成功。unique は general と独立した `changeCount` を持つ。**解放後の `Read` は 1507 ではなく成功 + 空**（1.6） |
...
| 21 | 単一 item で 10 MiB 超の `Copy` → **一度も貼り付けずに** Player を終了 → 他アプリで貼り付け | **貼り付けできない**。**終了前に一度でも貼り付けていれば成功する**ので、その手順を挟まないこと（1.6） |
| 22 | 単一 item で 10 MiB 超の `Copy` → Player 起動中に貼り付け | 貼り付けできる。所要時間とフレーム落ちを記録 |
| 23 | 32 MiB 超の `Copy` | **9007**（C# 側で拒否） |
| 24 | 他アプリが 50 MiB 超をコピーした状態で `Read` | **9006**（受信側 32 MiB 上限、1.6）。境界（32 MiB は通り 36 MiB は落ちる）と所要時間を記録 |
...
| # | 項目 | 内容 |
| --- | --- | --- |
| V-1 | xcframework とソースの版対応 | **クローズ済み（2026-09-03）。** ビルド時刻・コミット時刻・バイナリ内の文字列 15 種を突き合わせて一致を確認した。結果は 0.2。ネイティブを再ビルドしたら再確認する |
...
| V-9 | `detectValues` の許可 UI | **通知は出ないことを確認（2026-09-05）。** `AccessBehavior` は `AlwaysAllow`。**1514 は到達不能**で、2 経路（システム設定 / `tccutil reset Pasteboard`）とも拒否の導線が現れなかった。ネイティブ側も「拒否の導線が存在するかも未確認」と記録している（native-toolkit の 2026-09-03 手動確認結果）。**判別条件が正しいかは依然として未検証** |
| V-10 | lazy data provider の実挙動 | **クローズ済み（2026-09-05、実機 macOS 26.3）。** 一度も読み出さずに終了すると貼り付け不可、**一度でも貼り付けていれば終了後も可**（1.6）。12 MiB の `Copy` は 532 ms、貼り付け側に待ちが出る |
| V-11 | Domain Reload / Scene Reload 双方無効の構成 | `Awake` が再実行されず `s_dispatcher` が捕捉されないまま dispatch 経路が死ぬ可能性 |
| V-12 | single-flight の粒度 | **クローズ済み（2026-09-05）。** 49 回の操作を通じて **9001 は一度も発生しなかった**。読み取り系の連打でも過剰ではない。緩和は不要 |
| V-13 | 世代カウンタ撤去の妥当性 | **クローズ済み（2026-09-05、実機 macOS 26.3）。** 監視を別 `onChanged` で再開始したとき、**旧登録は凍結し新登録だけが発火した**（7.5 #16）。二重購読は起きていない。停止後のイベントも 0 件（#18）。撤去は妥当 |
| V-14 | **受信上限の超過を 9006 に集約していること（iOS との乖離）** | **本版では対応しない。別計画で扱う。** iOS は `CLIPBOARD_CONTENT_TOO_LARGE` と 64 MiB を持ち、「大きすぎる」を解析失敗から分離している（`IosClipboardJsonParser.cs:30 / 33 / 831-833`）。macOS は `TooLarge` も `Malformed` も 9006 になるため、**利用者は原因を判別できない**。他アプリが置いた内容の大きさが原因なのに「ブリッジのバグ」を疑わせる。対応するなら専用コードの追加と、上限値を 32 MiB のまま（送受対称）とするか 64 MiB に揃えるかの判断が要る |
...
| # | 事項 | 決定 |
| --- | --- | --- |
| D-1 | JSON リーダーの扱い | **共有化しない。`MacClipboardJsonReader.cs` として複製する**（5.4.1）。`common.md`「共通ファイルを作らない方針」による。iOS 側の `IosClipboardJsonReader.cs` は一切変更しない。v1〜v9 の「改名して共有する」計画は撤回した |
...
| D-13 | 監視登録の管理方式 | **世代カウンタを撤去し、active / pending の 2 スロットにする**（5.6.5）。ネイティブが再 Start 失敗時に既存監視を継続する契約（1.6 / IT-09）は、比較演算では表現できない。v3〜v6 でこの機構の周りに A クラスの指摘が 3 回出たことも根拠 |
| D-14 | `changeCount` の型 | **`long`。** Swift の `Int` は 64bit であり、`int` に狭めると `int.MaxValue` 超で正常応答が 9006 になる。`ClipboardJsonReader.TryGetInt64` を追加する（A1-2） |
| D-15 | 結果型の `default` | **「未初期化値」と定義し、不変条件は factory 生成値に限定する。** `sealed class` 化はせず、既存 `Ios*` / `Mac*` 結果型との一貫性を優先する（A1-3） |
| D-17 | **コールバック到着スレッドを入口で記録し、dispatcher 経由でログに出す** | V-4 は購読者側からもサンプルからも観測できない。入口で測るしかなく、かつ入口では Unity API を呼べない（5.6.9 / 5.6.14）。公開 API を増やさずに済むよう、観測面はログにする |
| D-16 | teardown と進行中 Start の競合 | **破棄後に届いた `startObserving` の成功完了で `clipboardStopObserving` を再発行する**（5.6.8）。ネイティブの Start / Stop は投入順に実行される保証が無く、tombstone は managed 配送しか止めないため、これが無いと破棄後も poller が残る |
...
- [ ] （段 3 のみ）5.6.8 の teardown を全体として実装し、段 2 で置いた `stop` の空実装を `StopObservingForTeardown` に置き換えている
- [ ] （段 3 のみ）**D-16 の再発行規則を実装し、7.2 の `TeardownStopIssueCountForTests` による回帰テストが pass する**
- [ ] （段 3 のみ）**5.6.5 の active / pending 遷移が 7.2 の `(3)` 項目すべてで検証されている**（pending 中の配送先を含む）
...
- [ ] **16 のコールバック入口すべてに `RecordCallbackThread()` があり、`DiscardIfTerminated` より前に置かれている**（5.6.14）
- [ ] **入口から Unity API を呼んでいない**（5.6.9）
- [ ] `FormatCallbackThreadDiagnostic` の EditMode テストがある

段 3 完了時の追加条件:

- [ ] 7.5 の手動確認 32 項目を macOS Standalone Player で実施し、結果を実装結果ファイルに記録している
- [ ] V-1 〜 V-13 のすべてに結論または継続課題としての記載がある
...
**1 回の implement-feature で通さず 3 段に分ける。** リスクが Manager コアに集中しているため、その段を小さく保つことに最大の価値がある。

v9 までは 4 段だったが、段 1（共有基盤の準備）が「共通ファイルを作らない」方針により消滅し、旧段 2〜4 が段 1〜3 に繰り上がった。

| 段 | 範囲 | 単独マージ可否の根拠 |
| --- | --- | --- |
| 1 | `MacClipboardPayloads` / `MacClipboardErrorInfo` / `MacClipboardConstants` / **`MacClipboardJsonReader`** / 結果型 12 種（10 ファイル）/ `MacClipboardChangeEvent` / Builder / Parser + EditMode 4 テスト | ネイティブにも Manager にも依存しない。**既存ファイルを一切変更しない**ので、iOS / Android への影響がゼロであることが構造的に保証される |
| 2 | Manager 骨格（Singleton / lifecycle / tombstone / dispatcher / ガードチェーン / dispatch / per-call スロット / テストシーム）+ Copy / Append / Read / ReadData / Clear + Dispatch テスト + PlayMode テスト。**監視には一切触れない** | 設計上のリスク（5.6.1 / 5.6.3 / 5.6.4 / 5.6.6 / 5.6.8）がすべてこの段に集中する |
| 3 | Snapshot / CreatePasteboard / RemovePasteboard / Detect×3 / AccessBehavior / CheckForegroundChange / Start・StopObserving + 監視一式 | 2 で確立したパターンの反復。監視は新機構なので必要なら 3a（単発操作）/ 3b（監視）に分割してよい |

- **監視に関わるものはすべて段 3 に置く。** 段 2 には公開 `StartObserving` が存在せず、**段 2 の時点で監視が動いていることはありえない**。したがって段 2 の teardown に監視の後始末は要らない
  - v3 のレビュー S-3 は「teardown が監視機構に依存するので前倒しする」としたが、依存の向きの解決として逆だった。**前倒しではなく、teardown の監視部分ごと後の段に送る**のが正しい
  - この切り方により、**段 1〜2 が A1 の出続けている監視領域から完全に切り離される**
- **段 2 が宣言する公開イベントは 4 本**（`OwnershipChanged` / `ReadCompleted` / `ReadDataCompleted` / `ClearCompleted`）。5.6.2 の 13 本のうち残り 9 本（`ClipboardChanged` を含む）は段 3 で追加する。段 1 で結果型 12 種はすべて揃っているので、どちらの順でもコンパイルは通る
- **段 2 の `OnDestroy` は監視に触れない。** `RunDestroyCleanup(stop:, managedCleanup:)` の `stop` には**空実装**を渡し、`managedCleanup` だけを実行する。段 3 で `StopObservingForTeardown` と D-16 の再発行規則を実装するときに `stop` を差し替える
  - 段 2 完了時点で teardown が監視を止めないコードになるが、**段 2 には監視を開始する手段が無いため実害は無い**
  - 段 3 の DoD に「5.6.8 の teardown を全体として実装し、`stop` の空実装を置き換えたこと」を含める
- **段 3 で追加する監視一式**: `clipboardStartObserving` / `clipboardStopObserving` の `extern` 宣言、`StopObservingForTeardown`、static 状態 2 件（`s_onChanged` / `s_pendingOnChanged`）と `ClearAllPendingCallbacks` でのリセット、`ClipboardChanged` イベント、`MacClipboardChangeEvent` の parse、`FireClipboardChanged`、変更コールバックへの `DiscardIfTerminated`、D-16 の再発行規則
- **段 2 に入るシーム**: `ResetForTests` / `BridgeAvailableOverrideForTests` / `MaxRequestBytesOverrideForTests` / `CompleteOwnershipForTests` / `CompleteReadForTests` / `CompleteReadDataForTests` / `CompleteClearForTests` / 観測系 3 本（`IsInFlightForTests` / `InFlightCountForTests` / `HasAnyPendingCallbackForTests`）
- **段 3 に入るシーム**: `CompleteOperationForTests`（removePasteboard）/ `CompleteObservationControlForTests` / `CompleteSnapshotForTests` / `CompleteCreatePasteboardForTests` / `CompleteDetectPatternsForTests` / `CompleteDetectValuesForTests` / `CompleteDetectMetadataForTests` / `CompleteAccessBehaviorForTests` / `CompleteForegroundChangeForTests` / `DeliverChangeEventForTests` / `HasChangeRegistrationForTests` / `HasPendingChangeRegistrationForTests` / `TeardownStopIssueCountForTests`
- **段 1 と段 2 の間に一度レビューを挟む**（`review-implementation-feature`）
- 段 3 の着手前に、5.6.2 の決定 2 の保証範囲・5.6.4 の `TryPassGuards` 分割・5.6.10 のシーム粒度を実装者が理解していることを確認する
