...
- 実装結果: `artifact/features/clipboard/results/2026-08-15-ios-clipboard-implementation-feature-result-v3.md`（レビュー LGTM）
- 実装計画: `artifact/features/clipboard/designs/2026-08-15-ios-clipboard-design-v5.md`
- 前版: `artifact/features/clipboard/designs/2026-08-16-ios-clipboard-sample-scene-design-v4.md`
- 対象レビュー: `artifact/features/clipboard/reviews/2026-08-16-ios-clipboard-sample-scene-design-review-v4.md`（総合評価「要修正（高優先度なし）」）
...
| 指摘 | severity | 対応 |
|---|---|---|
| `ErrObserveMissingNamedButton` が missing scope を `IssueStartObserving` へ渡せない | medium | **指摘は正しい**。helper を `IssueStartObserving(string marker, int owner, IosPasteboardScope targetScope)` に変更し、**scope の capture を呼び出し側へ移した**（6.5.4）。通常 Start / Restart / Busy は `_activeScope` を capture して渡し、error ボタンは**クリックごとに生成した missing named scope** を明示的に渡す。「error ボタンが active scope ではなく supplied scope を使う」ことを発行 helper のテストで固定（6.8.2） |
| S-25b は deferred Stop の完了前に外部変更すると teardown 判定が race する | medium | **指摘は正しい**。新 Controller の `Observing: off` は既定値であり、旧 Controller の Stop 完了を証明しない。S-25b に **`observe.stop.teardown` の成功ログを待つ同期点**を追加し、待たずに copy した場合は判定不能である旨と timeout 時の扱いを明記（7.2）。画面外 callback の shape-only ログ契約（marker / sequence / isSuccess / errorCode。`owner` は observation control の 2 ハンドラのみ）も 6.6 に明文化 |
| main-thread rejection のコード名が実装と一致しない | low | `CLIPBOARD_NOT_MAIN_THREAD` → **`CLIPBOARD_MAIN_THREAD_REQUIRED`**（`IosClipboardManager.MainThreadRequiredErrorCode`）に修正（6.5.3） |
| `CLIPBOARD_MANAGER_DESTROYED` 時の旧 observation の説明が `OnDestroy` cleanup を反映していない | low | 「`OnDestroy` が tombstone 設定後に native Stop を試行する。例外時のみ native subscription が残りうるが callback は tombstone で破棄され、以後の操作は拒否される」に修正（6.5.3） |

ボタン数・要検証（V-1〜V-6）は v4 から変更なし。

### v3 からの変更点（v4 で反映済み。参考）
...
| `MissingScopeNamePrefix` | `"com.jonghyunkim.nativetoolkit.example.missing."` + **クリックごとの GUID**（`Named(...)` を組み立てる。名前はログにも画面にも出さない） | `ErrObserveMissingNamedButton` | S-25d（`CLIPBOARD_UNAVAILABLE`） |
...
| `CLIPBOARD_MANAGER_DESTROYED` | しない | `OnDestroy` が tombstone 設定後に native Stop を試行するため、**通常は cleanup 済み**。cleanup が例外になった場合のみ native subscription が残りうるが、以後の native callback は tombstone で破棄され、Manager 操作もすべて拒否される | 未観測へ（Stop も不可能なので状態を維持しても意味がない）。サンプルは Manager を破棄しないため到達しない |
| `CLIPBOARD_MAIN_THREAD_REQUIRED`（`IosClipboardManager.MainThreadRequiredErrorCode`） | しない | 残る | UI callback からのみ呼ぶため到達しない |

#### 6.5.4 Start / Restart の発行と `targetScope`（v5 で引数化）
...
v4 は helper 内部で `_activeScope` を capture していたが、それでは `ErrObserveMissingNamedButton` が missing named scope を渡せず、active scope が general の状態では **Start が成功してしまう**（エラー導線として成立しない）。**`targetScope` は引数にし、capture を呼び出し側へ移す。**

```csharp
/// Issues one StartObserving. The owner token and the scope this call asked for travel together:
/// the state machine and the status line must describe the same call, and _activeScope can change
/// while the start is pending (a CreatePasteboard issued earlier may complete first).
///
/// The scope is a parameter rather than a capture of _activeScope so the error button can aim at a
/// pasteboard that does not exist.
private void IssueStartObserving(string marker, int owner, IosPasteboardScope targetScope)
{
...
| 呼び出し元 | owner | `targetScope` |
|---|---|---|
| `StartObservingButton` | `_observation.BeginStart()` | `_activeScope` |
| `RestartObservingButton` | `_observation.BeginStart()` | `_activeScope`（capture し直す） |
| `BusyStartObservingTwiceButton` の 1 本目 | `_observation.BeginStart()` | `_activeScope` |
| 同 2 本目 | `NonOwningToken` | 1 本目と同じ scope 変数 |
| `ErrObserveMissingNamedButton` | `_observation.BeginStart()` | **クリックごとに生成した missing named scope** |
...
private void OnBusyStartObservingTwiceClicked()
{
    ...
    IssueStartObserving("observe.busy#1", _observation.BeginStart(), targetScope);
    IssueStartObserving("observe.busy#2", IosClipboardSampleObservationState.NonOwningToken, targetScope);
}

private void OnErrObserveMissingNamedClicked()
{
    // A pasteboard that was never created: resolving it fails natively with CLIPBOARD_UNAVAILABLE.
    // Generated per click so a previous run cannot leave it resolvable. The name is never logged.
    IosPasteboardScope missing = IosPasteboardScope.Named(MissingScopeNamePrefix + Guid.NewGuid().ToString("N"));
    IssueStartObserving("observe.err.missingNamed", _observation.BeginStart(), missing);
}
```

`BeginStart()` は Pending 中に呼ばれると `NonOwningToken` を返すため、仮に Busy デモの 2 本目も `BeginStart()` 経由で発行してしまっても状態は壊れない。**二重の防御**とする。

`ErrObserveMissingNamedButton` も **owner を取る**。失敗を前提としたボタンだが、万一成功した場合に追跡されない observation が残ることを避ける（この場合 `_observedScope` は missing scope になり、status が `active general (observing named(len=N))` と表示されるため、異常であることが画面から分かる）。
...
**画面外 callback の shape-only ログ契約（v5 で明文化）**: 画面外の完了は UI に出せないため、**ログが唯一の観測手段**になる。全ての完了ハンドラは手順 1 で次を出力する。

```
[IosClipboardManagerExampleController][<handler>] seq: <n>, marker: <marker>, isSuccess: <bool>, errorCode: <code or null>
```

`owner` は **observation control（`CompleteStartObserving` / `CompleteStopObserving`）だけ**に存在する。owner token を持たない他の操作には付けず、この 2 ハンドラのみ `owner: <token>` を上記へ加える。

teardown 経路のログは `marker: observe.stop.teardown` / `observe.stop.deferred` で識別できる。S-25b の実機確認は**この成功ログを同期点として使う**（7.2）。本文・base64・検出値・pasteboard 名は従来どおり出さない。
...
| **error ボタンの scope**（v5 で追加） | `ErrObserveMissingNamedButton` の発行が **active scope ではなく supplied scope** を使うこと。発行 helper を「(marker, owner, targetScope) を記録するだけの seam」に切り出し、`_activeScope` が general でも `Named(...)` が渡ることを固定する |
...
| S-25b | **S-25a の直後（observing on）から続ける**。Stop を押さずに `Back To Home` → **Xcode のログで `marker: observe.stop.teardown` の `isSuccess: True` を待つ**（同期点。7.2.1）→ 再入場 → 他アプリでコピー → `Events` が増えないこと | teardown（画面外に observation を残さない） |
...
#### 7.2.1 S-25b の同期点（v5 で追加）

再入場後の `Observing: off` は**新しい Controller の既定値**であり、旧 Controller の Stop が完了した証拠にはならない。Stop 完了前に他アプリで copy すると、旧 native observation が正当にイベントを出し、新 Controller の共通 `ClipboardChanged` に届く。これは teardown 漏れと区別がつかない。

| 手順 | 内容 |
|---|---|
| 同期点 | `Back To Home` 後、Xcode のデバイスログで `[IosClipboardManagerExampleController][CompleteStopObserving] … marker: observe.stop.teardown … isSuccess: True` を確認する |
| 待たずに copy した場合 | 判定不能。S-25a からやり直す |
| 10 秒待っても completion が来ない場合 | **teardown failure** として記録する（`Events` が増えなかったことをもって合格としない） |
| `isSuccess: False` の場合 | エラーコードを記録する。native Stop は失敗しない（O-6）ため、managed rejection の調査対象になる |

Start pending 中に離脱した場合は `marker: observe.stop.deferred` が同期点になる。
...
- 実装計画 v5 との用語一致を確認した（`CLIPBOARD_MAIN_THREAD_REQUIRED` など、エラーコードは実装の公開定数に合わせた）
...
- ユーザー回答:
  - **承認する**（2026-08-16）。レビュー v5 は LGTM。高・中優先度の指摘および不足項目なし
  - 非ブロッキングの low 1 件（shape-only ログの `owner` は observation control callback のみに存在する）は、v6 を作らず本ファイルの 6.6 に 1 行反映した
  - 基準ファイル: 本ファイル（v5）。次工程は `implement-sample-scene`
