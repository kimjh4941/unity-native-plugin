# Windows Clipboard 実装計画書 v4

- 作成日: 2026-09-05
- 対象機能: clipboard
- 対象プラットフォーム: Windows（Windows 11 以降）
- 対象リポジトリ: unity-native-plugin（C# 層のみ）
- 前版: `2026-09-05-windows-clipboard-design-v1.md` → `-v2.md` → `-v3.md`
...
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v2.md`（high 3 / medium 4 / low 1 / 不足 3 を v3 で反映）
- **実機検証**: `artifact/features/clipboard/results/2026-09-05-windows-clipboard-spike-verification-result-v1.md`（V-1 / V-2 合格、F-1〜F-3 を本版で反映）
...
4. アパートメント確認（COM 所有権の記録を含む。M2 の反映）:
   ...
   **実機確認済み（2026-09-05）**: Unity 6000.4.2f1 の Windows Player メインスレッドは `CoGetApartmentType` = `APTTYPE_MAINSTA`(3)、
   ...
   - `CoGetApartmentType` が `S_OK` かつ `Sta` / `MainSta` なら**本層は何も初期化していない**（`s_comOwnership = None`）。そのまま次へ（**実機ではここを通る**）
