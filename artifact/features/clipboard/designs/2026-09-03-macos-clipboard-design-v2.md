# macOS Clipboard 実装計画書 (v2)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v1.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v1.md`（サブエージェント 3 本の統合レビュー）
...
| 分類 | 内容 |
| --- | --- |
| 事実誤りの修正 | 6.3 のエラー到達可能性（1507 / 1505 / 1504 / 1524）、1.5 の 1524 の分類、7.4 #2 の append の期待値 |
| ネイティブ仕様の追記 | lazy data provider 経路、`ScopeJson.name` の省略、`detectMetadata` の 15.4 分岐、`CheckForegroundChange` と監視の併用禁止、1508 の対象範囲、`OptionsJson` のキー省略 |
| 確定した設計判断 | `BOOL` は `[MarshalAs(UnmanagedType.I1)]`、事前検証は null とサイズのみ（決定 1）、拒否経路の event 取得元は `this`（決定 2）、`ClipboardJsonReader` は改名、Editor 実行は非対応 |
| 追加した設計 | 型仕様表（5.5）、Manager lifetime 契約（5.6.1）、実行段階表（5.6.4）、監視の世代管理の状態遷移表（5.6.5）、`s_dispatcher` 規約（5.6.6）、IL2CPP 制約（5.6.9）、Editor 完了シーム（5.6.10）、Definition of Done（10 章）、採用しなかった案（11 章） |
| テスト方針の修正 | ガードチェーンの検証を EditMode から PlayMode へ移動 |
| 実装計画 | 4 段階に分割（12 章） |
...
- C# の delegate では `long` で受け、`MacClipboardErrorInfo.Create` の 1 箇所だけで `int` に narrow する（5.2）
...
- Clipboard 機能の直近の前例である `IosClipboardManager.cs:193-200` は既にこの問題を認識し、`I1` で幅を固定したうえで理由コメントを付けている。macOS 版もこれに揃える
- 「既存 `MacShareManager` が素の `bool` で動いている」ことは根拠にならない（上位ビットが偶然ゼロである場合と区別できない）。`MacShareManager.cs:58` の未修正は本計画の範囲外の別課題として残す
...
- **10 MiB 超の単一 item は lazy data provider 経路に入る**（重要）
  ...
  - C# の `MaxRequestBytes`（32 MiB、5.6.3）までの copy は、単一 item なら**必ずこの経路に入る**
- **named / unique pasteboard はプロセス終了後も残る**（pasteboard server 上）。unique は `RemovePasteboard` で明示解放する。機密データを named に置かない
- **標準 pasteboard は解放できない**（1508）。対象は general だけでなく `font` / `ruler` / `find` / `drag` を含む
...
- **delegate 型の宣言は narrow guard の外**（`MacShareManager.cs:57-58` が `#if` より前）
...
**delegate 型の宣言は narrow guard の外に置く**（`static readonly` フィールドと `[MonoPInvokeCallback]` 本体だけが guard 内）。既存 `MacShareManager.cs:57-58` / `IosClipboardManager.cs:193-205` と同じ配置。
...
- 永続 delegate は `static readonly` で 15 個 + 変更通知 1 個
...
| 3 | `MacClipboardJsonBuilder.cs` | 7 種の入力 JSON を組み立てる `public static` メソッド群 |
| 4 | `MacClipboardJsonParser.cs` | 12 種の出力（出力専用 9 + 共用 2 + イベント 1）を結果型へ変換する `internal static` メソッド群 |
| 5 | `MacClipboardOperationResult.cs` | 値を返さない操作（removePasteboard / startObserving / stopObserving） |
| 6 | `MacClipboardOwnershipResult.cs` | copy / append。`Operation` を持つ |
| 7 | `MacClipboardReadResult.cs` | `MacClipboardItem`（読み出し item）と結果型 |
| 8 | `MacClipboardReadDataResult.cs` | `byte[]? Data`（型不在は成功 + null） |
| 9 | `MacClipboardSnapshotResult.cs` | `MacClipboardSnapshot` と結果型 |
| 10 | `MacClipboardChangeCountResult.cs` | `clear` の結果 |
| 11 | `MacPasteboardScopeResult.cs` | `createPasteboard` の結果 |
| 12 | `MacClipboardDetectionResults.cs` | `MacClipboardDetectedLink` / `MacClipboardLabeledValue` / `MacClipboardPostalAddress` / `MacClipboardCalendarEvent` / `MacClipboardShipmentTracking` / `MacClipboardFlightNumber` / `MacClipboardMoneyAmount` / `MacClipboardDetectedValues` / **`MacClipboardDetectedMetadata`** と、patterns / values / metadata の 3 結果型 |
| 13 | `MacClipboardAccessBehaviorResult.cs` | `accessBehavior` の結果 |
| 14 | `MacClipboardForegroundChangeResult.cs` | `checkForegroundChange` の結果 |
| 15 | `MacClipboardChangeEvent.cs` | 監視イベント |
| 16 | `MacClipboardManager.cs` | Manager 本体 |
...
| ファイル | 内容 |
| --- | --- |
| `MacClipboardManagerIntegrationTests.cs` | ガードチェーンの 5 段、dispatch 順序、監視の世代管理、tombstone、完了シーム経由の結果配送 |
...
- ネイティブが検証する値（サイズ超過、不正 UTI、空 item、pasteboard 名の形式）は C# で再検証しない。C# が投げるのは**呼び出し側のバグ**（factory への空白名）だけ

#### 5.1.1 rawValue 対応表（`MacClipboardDetectionPatternExtensions`）

置き場所は `MacClipboardPayloads.cs` の `internal static class MacClipboardDetectionPatternExtensions`（`IosClipboardPayloads.cs:462-509` の `IosClipboardDetectionPatternExtensions` と同形）。`RawValues` 辞書 + `ToRawValue` / `TryParse` を持つ。
...
- base64 のデコードは `JsonValue.TryGetBase64Bytes(maxDecodedLength, out bytes)` を使い、`JsonBase64Status.TooLarge` は失敗（9006）にする。`maxDecodedLength` は 5.6.3 の `MaxResponseBytesPerRepresentation` を渡す
...
- `IsSuccess == false` ⇔ payload プロパティが `null`（`MacClipboardReadDataResult.Data` を除く）
- **コレクション型のプロパティは常に非 null。** 失敗時も空コレクションを返す。呼び出し側に null チェックを強いない
...
- `Operation` を持つのは**複数の操作が同じ結果型を共有する場合**（`MacClipboardOperationResult` / `MacClipboardOwnershipResult`）。1 型 1 操作の結果型は持たない
...
| `MacClipboardReadContents` | `int ChangeCount` / `IReadOnlyList<MacClipboardItem> Items` | Items は常に非 null |
...
- `public const double DefaultObservationInterval = 0.5;`（ネイティブの `defaultObservationInterval` と一致）
...
- **共通イベントの取得元は経路で異なる**（決定 2）
  - **拒否経路**: 公開 API はすべて instance メソッドなので `this.XxxCompleted` から取る。`_instance` に依存しないため、`OnDestroy` で `_instance = null` にした後の 9004 拒否でも確実に発火する
...
| Copy / Append | representations の合計バイト数が `MaxRequestBytes` を超える | **9007** | base64 化で 4/3 に膨らみ managed 側で OOM しうる。ネイティブの 1506 とは目的も閾値も異なる独立した制約 |
...
| `RemovePasteboard` の対象が標準 pasteboard | 1508（general だけでなく `font` / `ruler` / `find` / `drag` も網羅される） |
...
```csharp
public const long MaxRequestBytes = 32L * 1024 * 1024;                  // 送信側（暫定）
internal const long MaxResponseBytesPerRepresentation = 32L * 1024 * 1024;  // 受信側（暫定）
```

- **`MaxRequestBytes` が利用者から見える実効上限である。** ネイティブの 100 MiB / 200 MiB には通常到達せず、1506 はほぼ返らない。XML コメントとマニュアルにはネイティブ値ではなくこの値を書く
- 両方とも暫定値。実機で所要時間とピークメモリを測って確定する（8 章 V-3）

受信側の制約（明記する）:

- **`MaxResponseBytesPerRepresentation` はデコード時に効くのであって、マーシャリング時には効かない。** 他アプリが 100 MiB の representation をコピーした状態で `Read` を呼ぶと、ネイティブは約 133 MB の base64 を含む JSON を返し、**マーシャリングの時点で約 266 MB の UTF-16 string が確保される**。上限を掛けてもこのメモリは減らせない
...
| 8 | ネイティブコールバック到着 | ネイティブのコード | 通常経路 `FireXxxResult`（**`EndOperation` する**） |
...
- 段階 5 の例外は `try/catch` で捕捉し、`{operation} could not be started.` として 9002 を返す
...
| 制御呼び出しの完了（成否問わず） | 変更なし | `ReleaseChangeRegistrationIfOwned()` の判定に従う | `= 0` |
...
3. `managedCleanup` は `ClearAllPendingCallbacks()`（per-call スロット全消し・世代 3 変数を 0・`s_inFlight.Clear()`）を行い、`finally` で `_instance = null` にする
...
| シーム | 用途 |
| --- | --- |
| `ResetForTests()` | tombstone と全 static 状態のリセット |
| `BridgeAvailableOverrideForTests` | ガードチェーン段階 4 を通過させる |
| `CompleteOwnershipForTests(bool, string?, long, string?)` | copy / append の結果を注入 |
| `CompleteOperationForTests(string, bool, long, string?)` | 値なし操作の結果を注入 |
| `CompleteReadForTests` / `CompleteReadDataForTests` / `CompleteSnapshotForTests` / `CompleteClearForTests` / `CompleteCreatePasteboardForTests` / `CompleteDetectForTests` / `CompleteAccessBehaviorForTests` / `CompleteForegroundChangeForTests` | 各値返却操作の結果を注入 |
| `DeliverChangeEventForTests(string?)` | 変更イベントを注入 |
| `IsInFlightForTests(string)` / `InFlightCountForTests` | in-flight 集合の観測 |
| `HasChangeRegistrationForTests` / `PendingObservationGenerationForTests` | 世代管理の観測 |
| `HasAnyPendingCallbackForTests` | per-call スロットのリーク検出 |
...
- 逸脱の宣言方法は既存の使い分けに従う
  - Manager / Builder / Parser: **クラスの XML `<summary>` 内 `<para>`**（`IosClipboardManager.cs:44-48`、`IosClipboardJsonBuilder.cs:22-25`）
  - 結果型・payload 型: **`#if` 直後のファイル先頭 `//` コメント**（`IosClipboardErrorInfo.cs:3-7`）

#### 5.6.12 Awaitable 版について
...
2. `MacClipboardPayloads.cs` / `MacClipboardErrorInfo.cs`
3. 結果型 11 ファイル
...
| 9007 `RequestTooLarge` | 送信 payload が `MaxRequestBytes` 超過 | `Clipboard content exceeds the {MaxRequestBytes} byte request limit.` |

- 段階 1〜6 の失敗は拒否経路、段階 7 の失敗は通常経路（5.6.4）。同じ 9002 でも経路が異なるので dispatch 関数を取り違えないこと
...
- **エンコード失敗由来の 1599 は値を返す操作に限る。** `removePasteboard` / `startObserving` / `stopObserving` は encode を通らない void 経路なので、1599 は `ClipboardError` 以外を catch した場合に限られる
...
| `CheckForegroundChange` の初回呼び出し | 成功 + `Changed == true`。**ただし同一 scope で `StartObserving` していない場合に限る**（監視中はほぼ常に false） |
...
- 5.5.1 の不変条件すべて（コレクションが失敗時も非 null であることを含む）
...
- ガードチェーン 5 段の各拒否経路（9003 / 9004 / 9005 / 9007 / 9002 / 9001）を `BridgeAvailableOverrideForTests` と完了シームで駆動する
- 拒否経路が in-flight マーカーと per-call スロットに触れないこと（`IsInFlightForTests` / `HasAnyPendingCallbackForTests` で観測）
- dispatcher 経由で共通イベント → 個別 callback の順序が保たれること
- **`EndOperation` が dispatch より前に走ること**（callback の中から同じ操作を再実行しても 9001 にならない）
- 監視の世代管理: start → stop → start の順で登録が正しく入れ替わること、`PendingObservationGenerationForTests` の遷移
- observation 共有キーの Busy メッセージが専用文言であること
- Editor（非 macOS Player）での失敗経路
- 破棄後の呼び出しが 9004 で拒否され、**共通イベントが発火すること**（決定 2 の検証）
- 破棄後に届いた変更イベントが捨てられること
...
| 17 | `StartObserving(interval: 0)` / `(interval: 61)` | **1523**（C# は事前拒否しない） |
...
**実機・実測が必要なものだけを残した。** v1 で要検証としていた設計判断（`ClipboardJsonReader` の改名、`RunDestroyCleanup` のシグネチャ、Editor 実行、BOOL のマーシャリング）は本計画で確定させ、9 章へ移した。
...
| V-3 | サイズ上限 | `MaxRequestBytes` / `MaxResponseBytesPerRepresentation` の 32 MiB は暫定値。所要時間・ピークメモリを実測して確定する。受信側は上限を掛けてもマーシャル時のメモリを減らせない点も実測する |
...
| V-6 | `NSInteger` / `BOOL` のマーシャリング実挙動 | `long` + `[MarshalAs(UnmanagedType.I1)]` の宣言で arm64 / x86_64 の IL2CPP ビルドから正しい値が届くこと |
| V-7 | `localOnly` の効果 | ネイティブ側が「Universal Clipboard への効果は実機未確認」と明記している。C# 側も同じ注意を XML コメントに引き写す |
...
| D-4 | 拒否経路の共通イベント取得元 | **`this` から取る。** `_instance` に依存するのは static なネイティブコールバックだけ |
...
- [ ] `.meta` を新規作成していない。改名は `git mv` で `.cs` と `.cs.meta` を対で移動している
- [ ] 8 章の要検証事項のうち、その段で判明したものが実測値で更新されている

段 4 完了時の追加条件:

- [ ] 7.5 の手動確認 29 項目を macOS Standalone Player で実施し、結果を実装結果ファイルに記録している
- [ ] V-1 〜 V-11 のすべてに結論または継続課題としての記載がある
...
| **`_instance = null` をやめて破棄後も共通イベントを配送する** | `Instance` getter の Unity `==` が破棄済みオブジェクトを null と判定するため、結局新しい GameObject が生成される。決定 2（拒否経路は `this` を使う）の方が副作用が小さい |
...
| 2 | `MacClipboardPayloads` / `MacClipboardErrorInfo` / 結果型 11 / Builder / Parser + EditMode 3 テスト | ネイティブに一切依存しない。型仕様（5.5）をここで確定できる |
| 3 | Manager 骨格（Singleton / lifecycle / tombstone / dispatcher / ガードチェーン / dispatch / テストシーム）+ Copy / Append / Read / ReadData / Clear + Dispatch テスト + PlayMode テスト | 設計上のリスク（5.6.1 / 5.6.3 / 5.6.4 / 5.6.6 / 5.6.8）がすべてこの段に集中する |
...
- `MacShareManager.cs:58` の素の `bool` 宣言の修正 → 別課題
