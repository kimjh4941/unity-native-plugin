...
対象レビュー: `artifact/features/clipboard/reviews/2026-08-15-ios-clipboard-design-review-v4.md`（総合評価「要修正（高優先度 1 件）」）

| 指摘 | severity | 対応 |
|---|---|---|
| `OnDestroy` が native cleanup の**後**に tombstone を設定するため、`clipboardStopObserving` / `clipboardCancelLoads` の P/Invoke が例外を投げると tombstone・pending clear・`_instance = null` に到達せず、終了処理が部分適用になる。再生成拒否と後着 callback 破棄という中心的な保証が成立しない | high | **指摘を全面的に受け入れ**。5.6.1 の `OnDestroy` を書き換え、**`_instance == this` 確認の直後・native cleanup より前**に `s_isTerminated = true` を設定。Stop / Cancel を**独立した try/catch** にして一方の失敗が他方を飛ばさないようにし、managed cleanup と `_instance = null` を **`finally`** で保証。例外を Unity lifecycle callback の外へ出さない。7.2 に順序・例外注入のテストを追加 |
| tombstone が同一 Play Mode session 中に永続するため、破棄系テストを 1 件実行すると後続の integration test がすべて B-11 になる。「Play Mode 終了で static が初期化される」は **domain reload 無効設定では成立しない** | medium | **指摘は正しい**（`Enter Play Mode Options` で domain reload を無効化できる）。5.6.1 に **`[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` による Play session 開始時の static 初期化**と、**Editor 限定の `ResetForTests()` seam** を追加。実機 runtime では同一セッション中に reset しないことを明記（Editor では native を一切呼ばないため未処理の後着コールバックが存在せず、reset が安全であることを根拠として記載） |
| `IosClipboardManager.Instance.Read(...)` では **`Instance` getter が先に評価される**ため、未生成状態の off-thread 呼び出しは B-9 に到達する前に GameObject 生成（Unity API 違反）へ進む。「全 off-thread 呼び出しを B-9 で拒否」は断定できない | medium | **指摘は正しい。v4 の記述は不正確だったため訂正する**。5.8.1 に **`Instance` getter も main thread 限定**であることを明記し、**B-9 が守れる範囲を「既存インスタンス参照に対する instance メソッド呼び出し」に限定**した。3.3 / 3.5 の公開契約と 7.2 のテスト条件（キャッシュ済み参照を使う）も合わせて修正 |
| 「tombstone を無効化した比較実装」が production Manager に切替可能な escape hatch として入ると、禁止した lifecycle を再有効化できてしまう | low | 7.2 から当該記述を削除。誤配送の再現は**テスト内の純粋な状態モデル**で行い、**production code に tombstone 無効化フラグを追加しない**ことを 7.2 に明記した |

#### 訂正（v4 の記述が不正確だった点）

v4 の 5.8.1 は「全 public API で main thread を検証し、違反は B-9 で拒否する」と書いたが、**`Instance` getter を経由する呼び出しには適用できない**。getter は `GameObject` を生成しうる Unity API であり、instance メソッドの先頭に置いた検証より先に評価される。v5 で契約を「`Instance` getter は main thread 限定」「B-9 はキャッシュ済みインスタンスへの off-thread 呼び出しを拒否する」に分けて記述し直した。

### v3 からの変更点（v4 で反映済み。記録として保持）

対象レビュー: `artifact/features/clipboard/reviews/2026-08-15-ios-clipboard-design-review-v3.md`（総合評価「要修正（高優先度 1 件）」）
...
対象レビュー: `artifact/features/clipboard/reviews/2026-08-15-ios-clipboard-design-review-v2.md`（総合評価「要修正（高優先度 2 件）」）
...
対象レビュー: `artifact/features/clipboard/reviews/2026-08-15-ios-clipboard-design-review-v1.md`（総合評価「要修正（高優先度 2 件）」）
...
- **すべて Unity main thread からのみ呼べる。`Instance` getter も同様**（5.8.0 / 5.8.1）。instance メソッドはキャッシュ済み参照への off-thread 呼び出しを B-9 で拒否するが、`Instance` getter は検証できない
...
private void OnDestroy()
{
    ...
    // Set the tombstone FIRST, before any native call. A P/Invoke here can throw
    // (DllNotFoundException / EntryPointNotFoundException); if the tombstone were set
    // afterwards, a partially applied teardown would leave the Manager recreatable and late
    // callbacks undiscarded — exactly the guarantees this tombstone exists to provide.
    s_isTerminated = true;

    RunDestroyCleanup(
        stop:           StopObservingForTeardown,
        cancel:         CancelLoadsForTeardown,
        managedCleanup: () => { ClearAllPendingCallbacks(); _instance = null; });
    // s_dispatcher は null にしない。破棄後の拒否結果（B-11）を届けるために必要。
}

/// <summary>
/// Runs the three teardown steps with the exception boundaries the destroy contract requires:
/// stop and cancel are isolated from each other, and managedCleanup always runs.
/// Pure with respect to Manager state — it only invokes what it is given — so tests can pass
/// throwing actions directly instead of the Manager exposing a mutable, swappable hook.
/// </summary>
internal static void RunDestroyCleanup(Action stop, Action cancel, Action managedCleanup)
{
    ...
    {
        try { stop(); }
        catch (Exception ex) { Debug.LogError($"[{LogTag}][{nameof(RunDestroyCleanup)}] stop: {ex.Message}"); }

        try { cancel(); }
        catch (Exception ex) { Debug.LogError($"[{LogTag}][{nameof(RunDestroyCleanup)}] cancel: {ex.Message}"); }
    }
    finally
    {
        try { managedCleanup(); }
        catch (Exception ex) { Debug.LogError($"[{LogTag}][{nameof(RunDestroyCleanup)}] managed: {ex.Message}"); }
    }
}
...
private static void StopObservingForTeardown()
{
    ...
    if (Application.platform == RuntimePlatform.IPhonePlayer) clipboardStopObserving(null);
#endif
}
...
{
#if UNITY_IOS && !UNITY_EDITOR
    if (Application.platform == RuntimePlatform.IPhonePlayer) clipboardCancelLoads(null);
#endif
}
```

**`OnDestroy` の例外安全性契約（v4 レビュー高優先度の反映）:**

| 順序 | 処理 | 保証 |
|---|---|---|
| 1 | `_instance != this` なら即 return | 重複インスタンスの破棄で状態を壊さない |
| 2 | **`s_isTerminated = true`** | **native 呼び出しより前**。以降どこで例外が出ても tombstone は立っている |
| 3 | `stop()`（実機では `clipboardStopObserving(null)`） | 独立した try/catch。失敗しても 4 を実行する |
| 4 | `cancel()`（実機では `clipboardCancelLoads(null)`） | 独立した try/catch |
| 5 | `finally`: `managedCleanup()` = `ClearAllPendingCallbacks()` + `_instance = null` | 3 / 4 の結果によらず必ず実行される |

- **cleanup 例外を `OnDestroy` の外へ出さない。** Unity の lifecycle callback から例外を投げると以降の破棄処理が止まりうるため、すべて catch してログのみ残す
- `s_isTerminated` を先に立てても副作用はない。native cleanup は `s_isTerminated` を参照せず、B-11 の拒否は public API 側でのみ働く

**テスト seam は「引数で受け取る純粋 helper」にする（v5 レビュー中優先度の反映）:**

v5 までの記述にあった「`internal static Action?` の差し替え可能フック」は**採用しない**。

| 方式 | 採否 | 理由 |
|---|---|---|
| `internal static void RunDestroyCleanup(Action stop, Action cancel, Action managedCleanup)` に**引数で渡す** | **採用** | player build に mutable な状態を残さない。`OnDestroy` は実 native アクションを、テストは throwing アクションを直接渡すだけで、順序・例外分離・`finally` を完全に検証できる |
| `internal static Action?` の差し替え可能フック | 不採用 | player build に mutable static が残り、同一アセンブリ内のコードが cleanup を差し替え・null 化できる余地が生まれる。`ResetForTests` がフック自体の初期化まで負う必要も出る |

- `RunDestroyCleanup` は Manager の static state を読み書きしない。**渡されたアクションを呼ぶだけ**であり、EditMode テストから直接呼べる（Manager インスタンスを生成しない層 1 の制約を満たす）
- したがって `ResetForTests` / `ResetStaticState` が cleanup seam を初期化する必要はない（差し替え可能な状態が存在しないため）

**Play session 開始時の static 初期化（v4 レビュー中優先度の反映）:**

```csharp
// Static state survives an Editor Play session when "Reload Domain" is disabled, so a test
// that destroys the Manager would leave every later test rejected with B-11. Reset explicitly
// at the start of each Play session / app launch, where no native call can be outstanding yet.
[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
private static void ResetStaticState() => ResetCore();

// Single reset core shared by the Play-session hook and the Editor-only test seam, so the two
// can never drift apart. It resets every mutable static except _instance, whose lifetime is
// owned by Unity (Awake / OnDestroy).
...
**reset 対象の static state 一覧（v5 レビュー低優先度の反映）:**

`ResetStaticState`（Play session 開始時）と `ResetForTests`（Editor テスト用）は**同じ `ResetCore` を共有する**。対象は次のとおりで、`_instance` のみ除外する。

| static | 初期値 | `ResetCore` での扱い | 除外理由 |
|---|---|---|---|
| `s_isTerminated` | `false` | リセット | - |
| `s_dispatcher` | `null` | リセット（`Awake` で再キャプチャ） | - |
| `s_mainThreadId` | `0` | リセット（`Awake` で再設定） | - |
| per-call callback スロット 15 個（`s_onCopy` 〜 `s_onCheckForegroundChange`） | `null` | `ClearAllPendingCallbacks()` | - |
| `s_onChanged` | `null` | `ClearAllPendingCallbacks()` | - |
| `s_onChangedGeneration` / `s_observingGeneration` / `s_pendingObservationGeneration` | `0` | `ClearAllPendingCallbacks()` | - |
| `s_inFlight`（`HashSet<string>`） | 空 | `ClearAllPendingCallbacks()` | - |
| `s_*Delegate`（`static readonly` の関数ポインタ 16 個） | - | **対象外** | `readonly` で変化しない。GC 防止のため生存し続ける必要がある |
| `_instance` | `null` | **対象外** | ライフタイムは Unity（`Awake` / `OnDestroy`）が所有する。reset で null にすると生存中の GameObject と不整合になる |

- **cleanup seam は一覧に現れない。** `RunDestroyCleanup` は引数で受け取る純粋 helper であり、差し替え可能な static state を持たないため（上記「テスト seam」参照）
- reset 後の不変条件をテストで固定する: `IsTerminated == false`、`s_inFlight` が空、全 callback スロットが `null`、全世代が `0`

- `SubsystemRegistration` は Play session の開始時（Editor）およびアプリ起動時（Player）に 1 回だけ走る。**その時点で native 呼び出しは 1 件も存在しない**ため、reset は安全である
- **同一セッションの実行中には reset しない。** 実機では旧 lifetime のコールバックがまだ到着しうるため
...
| tombstone のリセット | **Play session / アプリ起動の開始時のみ**（`[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]`）。**同一セッションの実行中は production コードから解除できない** |
| 検出手段 | `public static bool IsTerminated` を公開する |

**domain reload 無効設定への対応（v4 レビュー中優先度の反映）:**

v4 は「static は Editor の domain reload / Play Mode 終了で初期化される」と書いたが、**`Enter Play Mode Options` で Reload Domain を無効化している場合は成立しない**。この設定では static が Play session をまたいで残り、破棄系テストを 1 件走らせると以降の integration test がすべて B-11 になる。

対策は 2 段構えとする。

| 手段 | 適用範囲 | 内容 |
|---|---|---|
| `[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` | Editor / Player 共通 | Play session 開始時・アプリ起動時に static state を初期化する。domain reload の設定に依存しない |
| `internal static void ResetForTests()` | **Editor のみ**（`#if UNITY_EDITOR`） | テスト間の隔離用。`InternalsVisibleTo` により テストアセンブリからのみ到達できる |
...
/// rejected with B-1 before reaching P/Invoke), so no callback can be outstanding when this
/// runs. It is compiled out of player builds precisely because that reasoning does not hold
/// on device, where a callback from the destroyed lifetime may still arrive.
/// </summary>
internal static void ResetForTests() => ResetCore();   // 同じ reset core を共有する
#endif
```

- **production コードに tombstone を解除する経路を作らない。** `ResetForTests` は `#if UNITY_EDITOR` で player build から消える
- 破棄系 PlayMode テストは `[TearDown]` で `ResetForTests()` を呼び、テスト順序への依存をなくす
- 実機（層 2b / 層 3）では reset seam を使わない。破棄系の確認が必要なら**独立した Player run に隔離する**
...
| 有効性判定 | dispatch 前に破棄済みでないことを確認する。main thread 上では Unity の `== null`（破棄済みオブジェクトを null と判定する）を使ってよい。**off-thread の B-9 経路では `(object)s_dispatcher == null` のプレーン参照判定のみを使う**（Unity のオーバーロードは native オブジェクトに触れるため。5.8.2） |
| 無効時の挙動 | dispatch せず `Debug.LogError` のみ（main thread 上でのみ到達しうる） |
| リセット | **`SubsystemRegistration` の `ResetStaticState` で `null` に戻し、`Awake` で再キャプチャする**（本節「Play session 開始時の static 初期化」）。domain reload の有効・無効に依存しない |

**要確認（Reload Domain と Scene Reload を両方無効化した構成）:** この構成では `ResetStaticState` で `s_dispatcher` / `s_mainThreadId` が `null` / `0` に戻る一方、**既存 GameObject が破棄されず `Awake` が再実行されない**ケースが起こりうる。その場合 dispatch 経路が無効化されたままになる。層 2a のテスト着手時に該当構成で実挙動を確認し、必要なら `Instance` getter 側でも再キャプチャする（9.9）。
...
|---|---|---|---|
| 1 | main thread 検証（5.8.2） | **off-thread rejected 経路** | 触れない |
...
- **段階 1 は各メソッドの最初の文にする。** 通常の入口ログ（`Debug.Log`）よりも前に置く（5.8.2）
...
#### 5.8.0 公開 API は Unity main thread 限定（v3 / v4 レビュー中優先度の反映）

**`IosClipboardManager` の public API はすべて Unity main thread からのみ呼べる。`Instance` getter を含む。** これを XML ドキュメントコメントに明記し、instance メソッドでは実装でも検証する。

#### 5.8.1 `Instance` getter は検証できない（v4 レビュー中優先度の訂正）

**v4 の「全 off-thread 呼び出しを B-9 で拒否する」という記述は不正確だったため訂正する。**

一般的な呼び方は `IosClipboardManager.Instance.Read(...)` であり、**`Instance` getter が instance メソッドより先に評価される**。getter は未生成時に `new GameObject(...)` / `AddComponent` を行う Unity API であり、background thread から呼ぶと B-9 の検証に到達する前に Unity のスレッド制約へ違反する。

| 呼び出し形 | off-thread 時の挙動 |
|---|---|
| `IosClipboardManager.Instance.Read(...)`（**未生成**） | **getter が先に走り、Unity API 違反となる。B-9 では守れない** |
| `IosClipboardManager.Instance.Read(...)`（生成済み） | getter はフィールドを返すだけだが、`_instance == null` の Unity 比較が native オブジェクトに触れる。**契約違反であり保証しない** |
| キャッシュ済み参照（`var m = IosClipboardManager.Instance;` を main thread で取得）に対する `m.Read(...)` | **B-9 で安全に拒否できる**（5.8.2） |

**したがって公開契約は次の 2 段で書く。**

1. **`Instance` getter は Unity main thread 限定。** off-thread から呼ばないこと（検証手段がないため、違反時の挙動は未定義）
2. **instance メソッドは、キャッシュ済み参照に対する off-thread 呼び出しを B-9 で拒否する。** これは「誤って別スレッドから呼んでしまった」ケースの防御であり、`Instance` getter の制約を肩代わりするものではない

XML ドキュメントコメントには、`Instance` プロパティと各 instance メソッドの両方にスレッド制約を書く。

#### 5.8.2 instance メソッドの main thread 検証
...
- `s_inFlight` / callback スロット / 世代番号は 5.8.2 の契約により**メインスレッドからのみ触られる**。したがってロックは設けない。この前提をコード上のコメントにも明記する
...
| B-9 | **Unity main thread 以外から呼ばれた**（5.8.2） | `CLIPBOARD_MAIN_THREAD_REQUIRED` | `{operation} must be called from the Unity main thread.` |
...
|---|---|---|---|
| B-9（main thread 違反） | 1 | **off-thread rejected**（`DispatchOffThreadRejection`。5.8.2） | 触れない |
...
| **main thread で取得してキャッシュした参照に対し background thread から呼ぶと `CLIPBOARD_MAIN_THREAD_REQUIRED` で拒否され、`s_inFlight` が汚染されないこと**（5.8.2）。**`Instance` getter を off-thread から呼ぶテストは書かない**（契約上未定義のため。5.8.1） |
| **B-9 の拒否結果とログが main thread 上で届くこと**（common event の snapshot が closure 内で行われ、off-thread では `_instance` に触れない。5.8.2） |
...
| **【複合・v3 レビュー高優先度の再現】「旧 A 開始 → `Destroy` → `Instance` 再取得 → 新 A 開始（B-11 で拒否される）→ 旧 A の native コールバック後着（破棄される）」の順で、いかなる callback にも旧結果が渡らないこと** |
| **`Destroy` 後に到着した変更イベントが `ClipboardChanged` を発火しないこと**（5.6.5） |
| **`Destroy` 後に pending スロットと `s_inFlight` がクリアされ、`_instance == null` になること**（5.6.1 の `finally` 契約。例外注入そのものは層 1 で検証する） |

**テスト隔離と production code の不変性（v4 レビューの反映）:**

- 破棄系テストは `[TearDown]` で `ResetForTests()`（`#if UNITY_EDITOR`）を呼び、同一 Play session の後続テストが B-11 にならないようにする（5.6.1）
- **production code に tombstone を無効化するフラグ・切替を追加しない。** v4 に記載した「tombstone を無効化した比較実装」は escape hatch になりうるため撤回した
- 誤配送が起こりうることの再現は、**テストアセンブリ内の純粋な状態モデル**（`TryBeginOperation` / スロット取得 / `DiscardIfTerminated` を模した test-only helper）で行う。Manager 本体には正の安全性テストのみを適用する
- native cleanup の例外注入は、**`RunDestroyCleanup(stop, cancel, managedCleanup)` に throwing なアクションを引数で渡して**行う（5.6.1）。production に差し替え可能な static フックを置かないため、この検証は Manager インスタンスを生成せずに**層 1（EditMode）で完結する**
- `ResetForTests()` / `ResetStaticState()` が同じ `ResetCore` を共有し、5.6.1 の一覧どおり `_instance` 以外の全 mutable static を初期値へ戻すことをテストで固定する
...
10. **`Instance` getter を含む全 public API が Unity main thread 限定として XML コメントに明記され、instance メソッドの違反が B-9 で拒否される**。B-9 経路が off-thread で `_instance` / `Debug.Log` に触れない（5.8.0〜5.8.2）
11. **Manager の明示破棄・再生成が非対応として XML コメントに明記され、`OnDestroy` 後の全操作が B-11 で拒否される**（5.6.1）
12. **`OnDestroy` が「tombstone → 独立 try/catch の native cleanup → `finally` で managed cleanup と `_instance = null`」の順序で実装され、例外を外へ出さない**（5.6.1）
13. **cleanup seam が `RunDestroyCleanup(Action, Action, Action)` の引数渡しで実装され、player build に差し替え可能な `internal static Action?` フックが存在しない**（5.6.1）
14. **`[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` の `ResetStaticState` と `#if UNITY_EDITOR` の `ResetForTests()` が同じ `ResetCore` を共有し、5.6.1 の一覧どおり `_instance` 以外の全 mutable static を初期化する。production code に tombstone 無効化フラグが存在しない**（5.6.1 / 7.2）
15. **全 native コールバックが `DiscardIfTerminated` から始まる**（5.6.5）
16. **`StartObserving` / `StopObserving` が single-flight キー `observation` を共有し、`s_pendingObservationGeneration` で登録を自己判定する**（5.6.4）
17. 3.5 の公開型仕様どおりに型が実装されている（nullability・collection の非 null 契約を含む）
18. 5.5.1 の E-1〜E-16、5.5.2 の変更イベント破棄規則が実装されている
19. 6.3 の C# Bridge 層エラー B-0〜B-11 がすべて実装されている
20. **base64 の上限確認が `TryGetDecodedLength`（padding 考慮）で行われ、`readData` で `Data.Length == byteCount` が検証されている**（5.5.1 / 5.9.2）
21. 5.9.2 の response 側メモリ削減策 1〜4 が実装されている
22. 層 1 / 層 2a のテストが追加され、Unity Test Runner で全 passed
23. **7.2 の複合テスト（旧 A 開始 → Destroy → 再取得 → 新 A 開始 → 旧 A callback 後着）が追加され passed**
24. クリップボード本文・base64・検出値・pasteboard 名がログに出力されない
25. `public` メンバに英語の XML ドキュメントコメントがある
26. 7.3 の手動確認項目 M-1〜M-25 が実機で確認済み（未実施項目は理由とともに記録）
...
### 9.9 Reload Domain / Scene Reload をともに無効化した構成

`ResetStaticState`（`SubsystemRegistration`）は `s_dispatcher` / `s_mainThreadId` を初期値へ戻すが、この構成では**既存 GameObject が破棄されず `Awake` が再実行されない**ケースが起こりうる。その場合、dispatch 経路が無効のまま Play session が始まる。

- 層 2a のテスト着手時に、当該構成（`Enter Play Mode Options` で Reload Domain / Scene Reload を両方無効）で実挙動を確認する
- 再キャプチャされないと判明した場合、`Instance` getter 側でも `s_dispatcher` / `s_mainThreadId` を補完する（main thread からしか呼べない契約のため安全。5.8.1）
- 本計画のサポート範囲に含めるかどうかは、実測結果を見て確定する
