# macOS Clipboard 実装計画書 (v8)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v7.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v7.md`（Codex）
...
v7 のレビュー（新規 A1 1 件 / B 4 件、A2・C は 0 件）を反映した。v7 で A1-2 / A1-3 / B-1 は反映済みと判定され、A1-1（active / pending 分離）は「反映されたが新たな問題あり」だった。

| 分類 | 内容 |
| --- | --- |
| teardown と進行中 Start の競合 | **ネイティブの `startObserving` / `stopObserving` はどちらも非構造化 `Task { @MainActor }` を投入して即座に戻る。同一アクター上の非構造化 Task に投入順の実行保証は無い。** そのため teardown の Stop が先に走り、その後に遅延した Start が監視を開始すると、破棄後もネイティブの poller が残り続ける。tombstone は managed 配送を捨てるだけでネイティブ状態には触れない。破棄後に届いた **Start 成功完了で `clipboardStopObserving` を再発行する**規則を追加（A1 / D-16） |
| teardown 競合のテスト | Start pending 中の `OnDestroy`、Stop / Start の逆順実行、Stop 失敗時の active 維持を PlayMode の必須項目に追加（B-1） |
| 表記の修正 | 2.3 に残っていた撤去済みシーム名（B-2）、5.5.1 の値型一覧が `long` を含んでいなかった点（B-3）、段 4 DoD の要検証範囲（B-4） |
...
- **teardown（5.6.8）は段 4 の監視機構に一部依存するため、段 3 に次の 3 点を先に入れる**（公開 API の `StartObserving` / `StopObserving` は段 4 のまま）
  ...
  2. 監視の static 状態 2 件（`s_onChanged` / `s_pendingOnChanged`）と `ClearAllPendingCallbacks` でのリセット
...
- **段 4 に入るシーム**: `CompleteOperationForTests`（removePasteboard）/ `CompleteObservationControlForTests` / `CompleteSnapshotForTests` / `CompleteCreatePasteboardForTests` / `CompleteDetectPatternsForTests` / `CompleteDetectValuesForTests` / `CompleteDetectMetadataForTests` / `CompleteAccessBehaviorForTests` / `CompleteForegroundChangeForTests` / `HasPendingChangeRegistrationForTests`
