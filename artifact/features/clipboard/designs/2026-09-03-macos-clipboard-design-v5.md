# macOS Clipboard 実装計画書 (v5)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v4.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v4.md`
...
v4 のレビュー（must fix 3 / should fix 5 / nit 9）をすべて反映した。指摘は 5.6.4（実行段階とヘルパ）に集中していた。

| 分類 | 内容 |
| --- | --- |
| 通常経路の欠落を補完 | `InvokeNative` に `onNativeFailureResult` を復活（M-1）。値を返す 12 操作で段階 7 の失敗が正しい結果型で返り、per-call スロットのリークが消える |
| 未定義シンボルの解消 | `ValidateRequestSize`（旧 `TooLargeOrNull`）と `Dispatch` / `FireXxxResult` のシグネチャ・命名規約を追加（M-2 / S4-3） |
| 検証順序の修正 | 合計バイト数の算出で要素の null を 0 バイト扱いにし、段階 3 の NRE を排除（M-3）。段階 5 の駆動手段が成立するようになった |
| 段の境界の修正 | 段 3 のイベントを 5 本（`ClipboardChanged` を追加）に修正し、S-2 と S-3 の食い違いを解消（S4-1）。シームの段割り当ての漏れも補完（N4-2） |
| observation の差分 | 骨格コードをそのまま写せない 4 点を明示し、`StartObserving` の差分骨格を追加（S4-2） |
| その他 | `BuildContentJson` のキー順（N4-3）、`Debug.Log` 配置の逸脱宣言（N4-4）、7.2 の段割り当て（N4-6）、`MaxRequestBytesOverrideForTests`（N4-7）、`#nullable enable`（N4-9）、参照・行番号の修正（S4-4 / S4-5 / N4-1 / N4-5 / N4-8） |
...
```csharp
/// 合計バイト数が上限を超えていれば (RequestTooLarge, message) を、超えていなければ null を返す。
private static (int Code, string Message)? ValidateRequestSize(MacClipboardContent content);
```

- **合計バイト数の算出は要素の null を 0 バイトとして扱う**（`item.Representations.Sum(r => r.Value?.Length ?? 0)`。`content.Items` の要素が null の場合も同様にスキップする）
- **要素の null 検査は C# でもネイティブでも行わない。** 段階 3 で NRE を投げると公開 API が例外を伝播させ、「拒否は結果で返す」という 5.6.2 の契約を破る。null 要素は段階 5 の `Convert.ToBase64String(null)` が投げる `ArgumentNullException` として捕捉し、9002 に落とす
- この設計により、7.2 は「値が null の `byte[]` を含む辞書」で**段階 5 の拒否経路を駆動できる**
...
**`FireXxxResult` の命名規約**（結果型ごとに 1 本ずつ、計 12 本）:
...
- 例: `FireOwnershipResult(MacClipboardOwnershipResult result, string inFlightKey)` / `FireReadResult(MacClipboardReadResult result)` / `FireOperationResult(MacClipboardOperationResult result, string inFlightKey)`
- `MacClipboardOperationResult` を返す 3 操作は `Operation` からスロットを引く `TakeOperationCallback(string operation)` を経由する
...
各項目の末尾の `(3)` / `(4)` は、12 章のどの段で追加するかを示す。段 3 の DoD は `(3)` の項目だけで判定する。
