# macOS Clipboard サンプルシーン実装計画書 (v3)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-05
- 前版: `2026-09-05-macos-clipboard-sample-scene-design-v2.md`
- レビュー:
  - 1 巡目: `artifact/features/clipboard/reviews/2026-09-05-macos-clipboard-sample-scene-design-review-v1.md`（Codex。v1 に対して**差し戻し**。A1 6 件 / A2 0 件 / B 1 件 / C 3 件）
  - 2 巡目: `artifact/features/clipboard/reviews/2026-09-05-macos-clipboard-sample-scene-design-review-v2.md`（Codex。v2 に対して実施したが**結論の書き出し直前でハングし打ち切り**。回収できた発見 3 件のみ。判定は得られていない）
- 前提とする実装結果: `artifact/features/clipboard/results/2026-09-03-macos-clipboard-implementation-feature-result-v3.md`（段 2 / 5 操作）、`-v4.md`（段 3 / 10 操作）
- 参照する実装計画: `artifact/features/clipboard/designs/2026-09-03-macos-clipboard-design-v12.md`（以降 `v12` と表記し、その節は `v12 §5.6.11` の形で参照する）
- 出力範囲: ExampleController・UXML/USS・ナビゲーション導線・EditMode の wiring / state テスト

---

## v2 からの主な変更（2 巡目レビューの部分成果の反映）

**Codex による v2 のレビューは、結論を書き出す直前でハングした。** 実作業 3 分ののち 45 分間ログへの書き込みが無く、打ち切った。ただし**打ち切り前に 3 件の発見が報告されており**、いずれも本体側で裏を取ったところ事実だった。本版はその 3 件の反映である。

| # | 発見 | 裏取り結果 | 反映 |
| --- | --- | --- | --- |
| R-1 | 公開 enum が「5 値固定」と一致しない | **6 値だった**（`Default` / `Ask` / `AlwaysAllow` / `AlwaysDeny` / `Unavailable` / **`Unknown`**） | `§6.6` と D-14 を訂正。`Unknown` の意味を追記 |
| R-2 | エラー追跡の母集団と正規化表が到達可能コードを落としている | **2 箇所とも事実。** `§3.4` の追跡集合は 8 件だが `v12 §7.5` が名指しするのは **10 件**（**1513 / 1514 が欠落**）。`§4.6` の正規化表は 14 件だが `MacClipboardErrorCodes` の定数は **29 件** | `§3.4` を 10 件に、`§4.6` を 29 件の網羅表に。**網羅を単体テストで固定する**（D-17） |
| R-3 | #4 / #25 の判定規則がまだ一意でない | 事実。`changeCount` による鮮度判定を欠いており、**自分の `Copy` と `Read` の間に他アプリがコピーすると誤判定する** | `§6.6` に `changeCount` ゲートを追加し、`readTypes` の定義を明確化 |

**R-2 が最も重い。** `§3.4` の欠落は `v12 §7.5` #12（1514）と #11 / #13（1513）の消化状況を画面で追えないことを意味し、A1-1 と同じ「確認できたつもりになる」種類の欠陥である。

**2 巡目の残りの成果（反映確認 10 件の判定・SV-5 への意見）は得られていない。** 再レビューの要否は `§10` に整理した。
...
`v12 §7.5` の 32 項目のうち **10 項目**が「特定のエラーコードが返ること」の確認である。実機で 1 項目ずつ潰すとき、どれが未消化かを Console から探すのは非効率になる。

| code | 対応する 7.5 の項目 | 到達条件 |
| --- | --- | --- |
| 1302 | #17c | 常に到達可能 |
| 1503 | #17b | 常に到達可能 |
| 1508 | #10 | 常に到達可能 |
| 1511 | #3 | 他アプリがコピーしたあとに Append |
| 1512 | #7 | 常に到達可能 |
| **1513** | #11 / #13 | **macOS 15.4 未満でのみ到達する** |
| **1514** | #12 | **利用者が許可ダイアログで拒否したときのみ到達する** |
| 1515 | #13 | plain text に対する DetectMetadata |
| 1523 | #17 | 常に到達可能 |
| 9007 | #23 | 常に到達可能 |

**v2 は 1513 と 1514 を落としていた**（R-2）。どちらも環境依存で到達するコードのため、集合から漏れると「押したのに増えない」のか「そもそも到達しない環境なのか」を区別できない。

ステータス行に `Codes: 1508,1512,9007 (3/10)` の形で、このセッションで到達したコードの集合を出す。記録するのは**コード番号のみ**。`ResetReachedCodesButton` で消去できる。

**分母は常に 10 とし、環境依存の 2 件も含める。** 8/10 で止まったときに「1513 は macOS 15.4 未満が要る」「1514 は拒否操作が要る」と気づけることが目的で、分母を環境ごとに変えるとその手掛かりが消える。この 2 件の到達条件は画面の注記に書く。
...
**`MacClipboardErrorCodes` の 29 定数すべてを網羅する**（R-2）。v2 は 14 件しか載せておらず、**7.5 が名指しする 1514 と、ほぼ全操作の受け皿である 1599 が抜けていた**。抜けたコードは `unmapped` に落ちるので機微情報は漏れないが、**画面から何が起きたのか分からなくなる**。

| code | token | code | token |
| --- | --- | --- | --- |
| 1301 | `parseFailed` | 1512 | `emptyTypeFilter` |
| 1302 | `contractViolation` | 1513 | `detectionUnavailable` |
| 1501 | `emptyContent` | 1514 | `detectionDenied` |
| 1502 | `emptyRepresentations` | 1515 | `detectionFailed` |
| 1503 | `emptyPatterns` | 1521 | `pasteLoadFailed` |
| 1504 | `invalidTypeIdentifier` | 1522 | `pasteLoadTimedOut` |
| 1505 | `invalidPasteboardName` | 1523 | `invalidConfiguration` |
| 1506 | `contentTooLarge` | 1524 | `cancelled` |
| 1507 | `pasteboardUnavailable` | 1599 | `unknown` |
| 1508 | `standardPasteboard` | 9001 | `busy` |
| 1509 | `writeRejected` | 9002 | `bridgeUnavailable` |
| 1510 | `appendRejected` | 9003 | `mainThreadRequired` |
| 1511 | `ownershipLost` | 9004 | `managerDestroyed` |
| — | — | 9005 | `invalidRequest` |
| — | — | 9006 | `responseParseFailed` |
| — | — | 9007 | `requestTooLarge` |

未知の code は `NG code=<n> reason=<unmapped>` とする。**`unmapped` が出たら「ネイティブが新しいコードを追加した」という意味になる。**

**網羅を単体テストで固定する**（D-17）。`MacClipboardErrorCodes` の `public const int` をリフレクションで列挙し、**すべてに token が定義されていること**を検証する。定数が増えたときにテストが落ちるので、表の更新漏れが起きない。**curation の問題を不変条件に変える。**
...
| `_lastWrittenChangeCount` | 直近の `Copy` 成功時の `Ownership.ChangeCount`。**`§6.6` の鮮度判定の基準** |
| `_lastWrittenTypeCount` | 直近の `Copy` で書いた representation 型の数（`§6.6` の #4） |
| `_lastWrittenType` | 直近の `Copy` で書いた representation の型名（`§6.6` の #25。**保持するだけで表示しない**） |
...
| 7.5 | 判定値 | 定義 | なぜ安全か |
| --- | --- | --- | --- |
| **#4** 派生型 | `writtenTypes` / `readTypes` / `derived` | 直近の `Copy` で書いた representation 型数と、`Read` が返した**先頭 item の** representation 型数。`derived = readTypes > writtenTypes` | **数のみ**。型名は出さない |
| **#14** accessBehavior | `behavior=<enum 名>` | `MacClipboardAccessBehavior` の **6 値**のいずれか | 閉じた C# enum で、利用者データを含みえない |
...
**`#4` と `#25` は `changeCount` で鮮度を判定する（R-3）。**

v2 は「直前の `Copy` が本アプリからでない場合は `n/a`」とだけ書いていたが、**それを知る手段を定義していなかった**。自分が `Copy` してから `Read` するまでの間に他アプリがコピーすると、`readTypes` も往復ハッシュも他アプリの内容に対する判定になり、**気づかないまま誤った結論を出す。**

判定手順を次に固定する。

1. `Copy` の成功時に、結果の `Ownership.ChangeCount` を `_lastWrittenChangeCount` へ、書いた型数を `_lastWrittenTypeCount` へ、書いたバイト列のハッシュと型名を `_lastWrittenPayloadHash` / `_lastWrittenType` へ記録する
2. `Read` の成功時に `contents.ChangeCount` と `_lastWrittenChangeCount` を比較する
   - **一致しない** → 自分の書き込み以降に pasteboard が変わっている。`derived=n/a roundTrip=n/a`
   - **一致する** → 判定に進む
3. `#4`: `readTypes` は**先頭 item の representation 数**とする。`Copy` は内容を置換するので、自分が書いた item が先頭になる。複数 item を書いた場合は判定対象外（`n/a`）
4. `#25`: `_lastWrittenType` と同じ型の representation を先頭 item から取り、そのバイト列のハッシュを `_lastWrittenPayloadHash` と比較する。**同型が存在しなければ `differ` ではなく `n/a`**（往復の失敗ではなく判定不能）

**`Copy` を経ていない状態（起動直後や他アプリのコピー後）では `_lastWrittenChangeCount` が未設定なので、常に `n/a` になる。**
...
- [ ] `MacClipboardSampleStateTests` が次を検証している
  ...
  - [ ] **`MacClipboardErrorCodes` の全定数に token が定義されている**（D-17。リフレクションで列挙して検証する）
  - [ ] **`changeCount` が一致しないとき `#4` / `#25` が `n/a` になる**（D-18）
...
| D-14 | `MacClipboardAccessBehavior` の enum 値を表示可とする | **閉じた C# enum（6 値）**で、利用者データを含みえない（`§6.6`）。v2 は「5 値固定」と書いていたが実際は `Unknown` を含む 6 値で、**`Unknown` は「ネイティブが未知の値を返した」ことを意味する**ため 7.5 #14 でも観測できる必要がある |
| D-15 | `elapsedMs` を結果行に出す | 7.5 #22 / #24 の所要時間記録に必要。数値のみ（`§7.4`） |
| D-16 | **到達コードの分母を 10 で固定し、環境依存の 2 件も含める** | 分母を環境ごとに変えると「1513 は macOS 15.4 未満が要る」という手掛かりが消える（`§3.4`） |
| D-17 | **エラー正規化表の網羅を単体テストで固定する** | `MacClipboardErrorCodes` の定数をリフレクションで列挙し、全件に token があることを検証する。**curation の問題を不変条件に変える。** v2 は 29 定数のうち 14 件しか載せておらず、1514 と 1599 を落としていた（`§4.6`） |
| D-18 | **`#4` / `#25` の判定を `changeCount` でゲートする** | v2 は「自分の Copy でなければ `n/a`」と書いたが、それを知る手段が無かった。`Copy` と `Read` の間に他アプリがコピーすると誤判定する（`§6.6`） |
...
**2 巡目のレビューは成果物を残さずハングしたため、次の 2 つが未確認のまま残っている。**

| 未確認のもの | 影響 |
| --- | --- |
| v1 指摘 10 件の反映判定（解決 / 不十分 / 未対応） | Codex は打ち切り前に「**v1 の重大点の多くは実質的に修正されています**」と述べているが、件別の判定は得られていない |
| SV-5（`v13` の要否とタイミング）への意見 | `§9.3` に未決のまま残る |

**一方、2 巡目が実際に見つけた 3 件（R-1 / R-2 / R-3）は本版で反映済みで、いずれも本体側で裏を取っている。**

判断材料:

- **収束はしている。** 1 巡目は A1 6 件、2 巡目は（打ち切りまでに）3 件で、うち 2 件は C クラス相当の記述誤り、1 件（R-3）が A1 相当だった
- **R-2 の再発防止は自動化した**（D-17）。同種の取りこぼしはテストが捕まえる
- **次の網がある。** `review-implementation-sample-scene` が実装後に控えている
- **ハングのコストは無視できない。** 2 巡目は 48 分拘束して成果物ゼロだった

**したがって、3 巡目は回さずに実装へ進むことを推奨する。** ただし SV-5 は実装着手前に決める必要がある（`§9.3`）。