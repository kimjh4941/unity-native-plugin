...
- 前版: `artifact/features/clipboard/designs/2026-08-16-ios-clipboard-sample-scene-design-v2.md`
- 対象レビュー: `artifact/features/clipboard/reviews/2026-08-16-ios-clipboard-sample-scene-design-review-v2.md`（総合評価「要修正」）
...
| 指摘 | severity | 対応 |
|---|---|---|
| Busy Start の 2 本目の拒否 callback が、進行中の 1 本目の observation state を解除できる | high | **全面的に受け入れ**。6.5 に **owner token** を導入。`BeginStart` / `BeginStop` が owner を発行し、`CompleteStart(owner, …)` / `CompleteStop(owner, …)` は **owner が一致する callback だけ**が状態を変更する。Busy デモの 2 本目は `NonOwningToken` で発行し、結果表示のみ行う。callback の順序も「必須 state / cleanup → active guard → UI」に固定（6.6） |
| Stop を挟まない 2 回の成功 Start（M-16 の generation gate）を実行できる導線がない | medium | `RestartObservingButton` を追加（観測中のみ有効）。S-25b を「Start #1 成功 → Stop を挟まず Restart → 外部変更 1 回 → Events が 1 だけ増える」に変更。**`onChanged` は常に `null`** を渡し、event count の発生源を共通 `ClipboardChanged` 1 本に固定（6.5.4） |
| `CopyCustomDataButton` の UTI が `public.data` と独自 UTI で矛盾 | medium | `CopyCustomDataButton` を **`public.data` に統一**。独自 UTI は `CopyMultiRepresentationButton` の representation として分離し、`LoadFileCustomButton` を追加して copy 側 UTI と load 側 request UTI の対応表を 4.5.1 に固定 |
| M-23 の parser ベンチでは P/Invoke の UTF-16 marshalling を測定できない | medium | **指摘は正しい**。7.4 を書き換え、parser ベンチを**補助測定**へ格下げ。M-23 本体は native → `DllImport` → parser → decoded `byte[]` を通す実機計測として、計測区間と引き継ぎ先を明記 |
| 画面破棄後 callback の、必須 cleanup と禁止する後続操作の順序が未確定 | medium | 6.6 に **callback lifetime 表**を新設（画面外でも必須 / 画面外では禁止 / UI 抑止）。`LoadItem(File)` は size 取得と directory 削除を分離し、全経路の表示契約（`fileSize=<n or -1>, cleanup=<ok\|failed>`）を確定 |
| S-15 の「14 文字」が `PlainTextBody` と一致しない | low | 長さ注記を削除。`items=2` を主眼に変更。あわせて `textLen` が **UTF-16 code unit 数**であることと `PlainTextBody` の実値（16）を 4.5 に明記 |
| `LargePng` のランダム生成条件が再現不能 | low | 固定 seed の**ローカル PRNG**（Unity の global random state に触れない）で生成し、PNG byte length を結果に表示。3〜5 MiB を外れたら計測を開始せず fixture failure を表示（4.5.2） |

ボタン数: 53 → **55**（`RestartObservingButton` / `LoadFileCustomButton` を追加）。
...
| O-5 | Restart が**失敗**した場合に native 側の旧 observation が生き残るかは Unity から判別できない | — | サンプルは保守的に **`IsObserving` を変更しない**（true のまま）。Stop を出せる状態を維持する方が、監視が残ったまま Stop 不能になるより安全（**要検証 V-7**） |
...
#### 4.5.1 copy 側 representation と load 側 request UTI の対応（v3 で確定）
...
**status の scope 表記**: 観測中は `Scope: <active> (observing <observed>)` とし、**観測開始時に capture した scope** を併記する。scope ボタンを無効化しているため通常は一致するが、`CreatePasteboard` の完了が観測開始後に届く可能性があるため、表示上区別できるようにする（V-5）。`Restart` は capture し直す。
...
| V-7 | 観測中の Restart が**失敗**した場合に native の旧 observation が継続するか（1.3 O-5）。継続しないと判明したら 6.5 の失敗時遷移を「`IsObserving = false`」へ変更する |
...
| Start / Restart 実行中 | 成功（owner 一致） | `IsObserving = true`, `ControlPending = false` | `StopRequestedAfterStart` なら即 Stop を発行 |
| Start / Restart 実行中 | 失敗（owner 一致） | `ControlPending = false`, **`IsObserving` は変更しない** | Restart 失敗時に旧 observation が残る可能性に備える（O-5 / V-7） |
...
| Stop 実行中 | 成功（owner 一致） | すべて false | - |
| Stop 実行中 | 失敗（owner 一致） | `ControlPending = false`, `IsObserving` は true のまま | 再度 Stop できる状態を残す |
...
private void OnBusyStartObservingTwiceClicked()
{
    ...
    var second = BeginResult("observe.busy#2");
    IosClipboardManager.Instance.StartObserving(_activeScope, onChanged: null,
        onStarted: r => CompleteObservationControl(second, IosClipboardSampleObservationState.NonOwningToken, r,
                                                  isStart: true));
}
```
...
2. `if (_observation.ShouldIssueStopNow())` → `BeginStop()` で owner を取り、`StopObserving(onResult: …)` を per-call callback つきで発行
3. **その後に**共通イベント 10 個を解除

per-call callback はイベントではないため、解除後も届く。Start pending 中の離脱では、Start の per-call callback が `ShouldIssueStopNow()` を再評価して Stop を発行する（6.6）。
...
|---|---|---|
| `StartObserving` / `Restart` | `CompleteStart(owner, …)`、`StopRequestedAfterStart` なら Stop 発行 | 結果行・status・enabled |
| `StopObserving` | `CompleteStop(owner, …)` | 結果行・status・enabled |
...
```csharp
private void CompleteObservationControl(
    IosClipboardSampleResultContext context, int owner, IosClipboardOperationResult result, bool isStart)
{
    Debug.Log($"[{LogTag}][{nameof(CompleteObservationControl)}] seq: {context.Sequence}, owner: {owner}, " +
              $"isStart: {isStart}, isSuccess: {result.IsSuccess}, errorCode: {result.Error?.Code}");
    ...
    bool owned = isStart
        ? _observation.CompleteStart(owner, result.IsSuccess)
        : _observation.CompleteStop(owner, result.IsSuccess);
    if (owned && _observation.StopRequestedAfterStart && _observation.ShouldIssueStopNow())
    {
        IssueStopObserving("observe.stop.deferred");
    }

    if (!IsScreenAlive()) return;
    if (owned && isStart && result.IsSuccess) { _observedScope = _activeScope; }
...
| Restart 失敗 | 失敗で `IsObserving` が **true のまま**であること（O-5 の保守的挙動） |
| Stop 失敗 | `IsObserving` が true のまま、`ControlPending` が false に戻ること |
...
| S-25a | `Busy Start Observing Twice` → 2 本目が `CLIPBOARD_BUSY`。その後 `Observing: on` になり **Stop が有効**であること | M-16 前半（+ owner 契約の実機確認） |
| S-25b | `Start Observing` 成功 → **Stop を挟まず** `Restart Observing (no stop)` → 他アプリで **1 回だけ**コピー → 復帰し `Events` が **1 だけ増える** | M-16 後半（世代ゲート、O-2） |
| S-25c | S-25a の直後に画面を離脱し、再入場して他アプリでコピー | 画面外に observation が残っていないこと（`Events` が増えない） |
...
- 変更対象は具体パスで示した（5 章）
- 不確実な事項は V-1〜V-7 および S-30 の要検証として明記した
