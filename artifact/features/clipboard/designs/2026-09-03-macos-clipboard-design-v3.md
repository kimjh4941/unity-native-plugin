# macOS Clipboard 実装計画書 (v3)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v2.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v2.md`
...
v2 のレビュー（must fix 5 / should fix 11 / nit 12）をすべて反映した。

| 分類 | 内容 |
| --- | --- |
| 自己矛盾の解消 | lazy data provider の発動条件（M1）、決定 2 の契約（M2）、ガードチェーンの段数表記（S4）、結果型の数え方（S11） |
| 実装可能性の修正 | 定数を `MacClipboardConstants.cs` へ分離し段 2 を単独コンパイル可能に（M3 / S3）、Editor 完了シームの再設計（M4）、`TryStartOperation` の分割方針を明記（M5） |
| ネイティブ仕様の追記 | 空白のみの pasteboard 名が素通りすること（S6）、変更イベントの encode 失敗が黙って捨てられること（S7）、1508 が unique にも適用されること（N2）、`CheckForegroundChange` の scope 単位（N3） |
| 型仕様の確定 | 結果型は `readonly struct` / 入れ子値型は `sealed class`（S1）、不変条件を 3 分岐に整理（S2）、Read の入れ子構成の理由（S10） |
| その他 | 6.3 の void 経路の 1599 を統一（S5）、段階 8 の 9006 帰属（S8）、4.1 #7 の型漏れ（S9）、要検証と決定事項の整理（N4 / N5）、手動確認の追加（N12） |
...
- **変更イベントの encode に失敗すると、ネイティブはそのイベントを黙って捨てる**（`UnityMacClipboardManager.swift:393` の `guard let json = ... else { return }`）。エラー通知は無い。C# からは「変更が起きたのに `ClipboardChanged` が来ない」としてしか観測できない
...
- **値を運ぶ入れ子型は `sealed class`**（`MacClipboardItem` / `MacClipboardReadContents` / `MacClipboardSnapshot` / `MacClipboardDetected*` / `MacClipboardChangeEvent`）
...
| コレクション | **常に非 null**（失敗時は空） | `Patterns` / `Items` / `ItemTypes` / `MatchingItemIndexes` / `MacClipboardDetectedValues` のコレクション 9 件 |
...
- **共通イベントの取得元は経路で異なる**（決定 2）
  - **拒否経路**: 公開 API はすべて instance メソッドなので **`this` の event から取る**。`private static TryStartOperation` へは iOS と同じ `Func<Action<TResult>?> commonSelector` 引数を使い、呼び出し側が `() => this.XxxCompleted` を渡す。`_instance` を経由してはならない
...
- **決定 2 が保証する範囲（重要）**
  ...
  - **保証しないのは、破棄後に `Instance` から取り直した場合。** その `this` は新しく生成された別インスタンスであり、購読者がいないため誰にも届かない（5.6.1 の「死んだ Manager」と同じ状態）。`Awake` の重複インスタンス破棄経路で掴んだ複製インスタンスも同様
...
```csharp
if (!TryPassGuards(...)) return;                                     // 段階 1〜4
string json;
try { json = MacClipboardJsonBuilder.BuildContentJson(content); }    // 段階 5
catch (Exception ex) { /* 拒否経路で 9002 */ return; }
if (!TryBeginOperation(s_inFlight, key)) { /* 拒否経路で 9001 */ return; }
s_onCopy = onResult;                                                 // 段階 7
InvokeNative(...);
```
...
| `CompleteObservationControlForTests` | `(string operation, bool isSuccess, long errorCode, string? errorMessage)` | **startObserving / stopObserving 専用。** この経路でのみ `ReleaseChangeRegistrationIfOwned()` と `s_pendingObservationGeneration = 0` が走る（5.6.5） |
| `CompleteOperationForTests` | `(bool isSuccess, long errorCode, string? errorMessage)` | **removePasteboard 専用**（値なし操作のうち observation 以外はこれだけ） |
...
| `HasAnyPendingCallbackForTests` | なし | per-call スロットのリーク検出 |
...
- **void 経路の 3 操作（`removePasteboard` / `startObserving` / `stopObserving`）に 1599 は挙げない。** encode を通らないため、1599 の発生源は `completeVoid` の総称 catch だけで実質非到達。3 操作とも同じ扱いに揃えている
...
- 5.5.1 の不変条件 3 分岐すべて（参照型 payload は失敗時 null / 値型 payload は失敗時に既定値 / コレクションは失敗時も非 null）
...
- `MacClipboardOperations` の 15 定数がすべて相異なり、`ObservationControlKey` が Start / Stop 以外のキーと衝突しないこと
...
- ガードチェーン 6 段の各拒否経路（段階 1: 9003 / 段階 2: 9004 / 段階 3: 9005・9007 / 段階 4: 9002 / 段階 5: 9002 / 段階 6: 9001）を `BridgeAvailableOverrideForTests` と完了シームで駆動する
...
- **破棄前に取得した参照を保持したまま**呼んだ場合、9004 で拒否され共通イベントが発火すること（決定 2 が保証する範囲）
- 破棄後に `Instance` から取り直して呼んだ場合、9004 で拒否されるが**旧購読者には届かない**こと（決定 2 が保証しない範囲。5.6.2）
...
- 段 3 の Manager は結果型 12 種のうち Copy / Append / Read / ReadData / Clear が使う 4 種（Ownership / Read / ReadData / ChangeCount）しか参照しない。残り 8 種は段 2 で作られたまま段 4 まで未使用でよい（コンパイルは通る）
