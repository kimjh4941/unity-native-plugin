# macOS Clipboard 実装計画書 (v9)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v8.md`
- レビュー: なし（レビュー指摘ではなく、命名規則の新設と段の切り直しによる改訂）
...
レビュー指摘による改訂ではない。命名規則が新設されたことと、段の切り方を見直したことによる。

| 分類 | 内容 |
| --- | --- |
| 段の切り直し | **監視に関わるものをすべて段 4 に寄せた。** v3 のレビュー S-3 で「teardown が段 4 の監視機構に依存するので段 3 に前倒しする」と決めたが、順序の付け方が逆だった。段 3 には公開 `StartObserving` が無く監視が動くことはありえないため、段 3 の teardown に監視の後始末は要らない。これで**段 1〜3 が、A1 が出続けている監視領域から完全に切り離される** |
| 改名根拠の訂正 | `ClipboardJsonReader` への改名根拠を、`agent-rules/coding-rules/common.md`「命名: OS 接頭辞」への準拠に差し替えた。v1〜v8 が根拠としていた「本パッケージは共有型を接頭辞なしで置く方針」は**存在しない規則の誤読**だった（2.4 / D-1） |
| 2.4 の訂正 | `ClipboardOperationResult` / `ClipboardReadResult` は「プラットフォームガードを持たない共有型」ではなく **Android 専用型**である。ガードの有無で共有かを判断してはならない |
...
| 型 | 現状 | 判断 |
| --- | --- | --- |
| `Clipboard/IosClipboardJsonReader.cs`（`JsonValue` / `JsonValueKind` / `JsonBase64Status` を含む） | `#if UNITY_IOS \|\| UNITY_EDITOR`。現在は iOS のみが利用 | **共有化する**（4.2）。macOS が 2 番目の利用者になるため、`common.md`「命名: OS 接頭辞」に従い `ClipboardJsonReader` へ改名しガードを広げる |
...
| `Clipboard/ClipboardJsonReader.cs`（改名後） | **`TryGetInt64(out long)` を追加する**（A1-2）。既存の `TryGetInt` は残す。iOS 側の利用箇所には影響しない |

- **改名はルール上の必須事項。** `agent-rules/coding-rules/common.md`「命名: OS 接頭辞」は、2 つ以上のプラットフォームが使う型に接頭辞を付けないと定めている。`ClipboardJsonReader` は iOS と macOS の 2 つが使うため、`Ios` 接頭辞のままにはできない
  - 同ルールは「単一プラットフォーム専用の型が 2 番目の利用者を得たら、接頭辞を外して共有型にする」ことも定めている。本件はまさにその適用例
  - **v1〜v8 が根拠としていた「本パッケージは共有型を接頭辞なしで置く方針（`ClipboardOperationResult` / `ClipboardReadResult`）」は誤りだった。** 挙げた 2 型は Android 専用であり、接頭辞が無いのは規則ではなく逸脱である（2.4 / `artifact/topics/os-prefix-violations/README.md`）
...
- **`changeCount` は `long` で読む。** 既存の `JsonValue.TryGetInt` は `int` 範囲外を**失敗**として返すため、そのまま使うと `changeCount > int.MaxValue` の正常な応答が 9006 に化ける。`ClipboardJsonReader` に `TryGetInt64(out long)` を追加し、`OwnershipJson` / `ReadResultJson` / `SnapshotJson` / `ChangeCountJson` / `ChangeEventJson` の `changeCount` はこれで読む（4.2 の共有化に含める）
...
| # | 事項 | 決定 |
| --- | --- | --- |
| D-1 | `ClipboardJsonReader` の共有化 | **改名する（ルール上必須）。** `common.md`「命名: OS 接頭辞」により、2 つ以上が使う型は接頭辞なし。`git mv` で `.cs` と `.cs.meta` を対で移動し、ガードを 3 プラットフォームに拡張、クラス doc を中立化する |
...
| 3 | Manager 骨格（Singleton / lifecycle / tombstone / dispatcher / ガードチェーン / dispatch / per-call スロット / テストシーム）+ Copy / Append / Read / ReadData / Clear + Dispatch テスト + PlayMode テスト。**監視には一切触れない** | 設計上のリスク（5.6.1 / 5.6.3 / 5.6.4 / 5.6.6 / 5.6.8）がすべてこの段に集中する |
| 4 | Snapshot / CreatePasteboard / RemovePasteboard / Detect×3 / AccessBehavior / CheckForegroundChange / Start・StopObserving + 監視の active / pending 遷移 | 3 で確立したパターンの反復。監視は新機構なので必要なら 4a（単発操作）/ 4b（監視）に分割してよい |
...
- **監視に関わるものはすべて段 4 に置く。** 段 3 には公開 `StartObserving` が存在せず、**段 3 の時点で監視が動いていることはありえない**。したがって段 3 の teardown に監視の後始末は要らない
  - v3 のレビュー S-3 は「teardown が段 4 の監視機構に依存するので段 3 に前倒しする」としたが、依存の向きの解決として逆だった。**前倒しではなく、teardown の監視部分ごと段 4 に送る**のが正しい
  - この切り方により、**段 1〜3 が A1 の出続けている監視領域から完全に切り離される**
- **段 3 が宣言する公開イベントは 4 本**（`OwnershipChanged` / `ReadCompleted` / `ReadDataCompleted` / `ClearCompleted`）。5.6.2 の 13 本のうち残り 9 本（`ClipboardChanged` を含む）は段 4 で追加する。段 2 で結果型 12 種はすべて揃っているので、どちらの順でもコンパイルは通る
- **段 3 の `OnDestroy` は監視に触れない。** `RunDestroyCleanup(stop:, managedCleanup:)` の `stop` には**空実装**を渡し、`managedCleanup` だけを実行する。段 4 で `StopObservingForTeardown` と D-16 の再発行規則を実装するときに `stop` を差し替える
  - 段 3 完了時点で teardown が監視を止めないコードになるが、**段 3 には監視を開始する手段が無いため実害は無い**
  - 段 4 の DoD に「5.6.8 の teardown を全体として実装し、`stop` の空実装を置き換えたこと」を含める
- **段 4 で追加する監視一式**: `clipboardStartObserving` / `clipboardStopObserving` の `extern` 宣言、`StopObservingForTeardown`、static 状態 2 件（`s_onChanged` / `s_pendingOnChanged`）と `ClearAllPendingCallbacks` でのリセット、`ClipboardChanged` イベント、`MacClipboardChangeEvent` の parse、`FireClipboardChanged`、変更コールバックへの `DiscardIfTerminated`、D-16 の再発行規則
- **段 3 に入るシーム**: `ResetForTests` / `BridgeAvailableOverrideForTests` / `MaxRequestBytesOverrideForTests` / `CompleteOwnershipForTests` / `CompleteReadForTests` / `CompleteReadDataForTests` / `CompleteClearForTests` / 観測系 3 本（`IsInFlightForTests` / `InFlightCountForTests` / `HasAnyPendingCallbackForTests`）
