# Windows Clipboard サンプルシーン 実装計画 v2

## 1. 基本情報

- 日付: 2026-09-08
- 機能名: clipboard / 対象プラットフォーム: Windows
- 実装結果ファイル: `artifact/features/clipboard/results/2026-09-08-windows-clipboard-implementation-feature-result-v6.md`
- 実装計画（機能側）: `artifact/features/clipboard/designs/2026-09-05-windows-clipboard-design-v8.md`
- 前版: `2026-09-08-windows-clipboard-sample-scene-design-v1.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-08-windows-clipboard-sample-scene-design-review-v1.md`（A1 13 / A2 2 / B 20 / C 8）
...
| M-6 | 3. Copy Image → 5. Paste Image（往復一致）→ ペイント | ペイント |
| M-5 | 3. Copy Files（13. でファイル作成後） | エクスプローラ |
| M-7 | 3. Copy Multiple Formats | 複数アプリで比較 |
| M-8 | 3. Copy Multiple Formats (CF_TEXT + 非 ASCII) | 貼り付け先で欠落を確認 |
...
| M | 担当 |
|---|---|
| M-13 | **11. Delayed History Call (5s)** を押し、5 秒以内に別ウィンドウを前面にする |
...
| M-18 | 7. Reserve → 他アプリに貼り付け | **判定は「貼り付け先に内容が届くか」。provider 呼び出し回数ではない**（8.3） |
| M-19 | 7. Reserve → **2. Quit** → 終了後に貼り付け | 履歴 ON だと D-8 で実体化されてしまい、M-19 が通ったことにならない |
| M-20 | **2. Shutdown While Disabled**（7.6） | `force` 分岐。`spent` 分岐は別途 |
...
| 候補 | 内容 | 採否 |
|---|---|---|
| **(a) 強制 1 回打ち切り** | `ShutdownWithDrain` は `isActiveAndEnabled == false` のとき `AdvanceDrain(force: true)` を 1 回だけ回し、完了しなければ `ShutdownTimeout` にする。`enabled` は公開プロパティであり試験用細工ではない | **採用。** 2. に `Shutdown While Disabled` ボタンを置く |
...
**M-20 は (a) で実施し、(b)（予算消尽）は未実施として記録する。**
...
| Multiple（M-8 用） | **`Text("CF_UNICODETEXT", 非 ASCII)` + `Text("CF_TEXT", 同一内容)`**。`CF_TEXT` は `CP_ACP` 変換されるため欠落し得る |
...
6 節のブロック A 〜 E に従う。**M-20 は 7.6 (a) で実施、(b) は未実施。D-9 は未実施。**
...
| # | 内容 |
|---|---|
| V-1 | `CF_TEXT` に非 ASCII を含めたときの欠落挙動（M-8）。設計 2.4 の記述と実挙動の一致 |
...
| V-4 | M-20 の (b)（予算消尽経路）を層 3 の道具立てとして用意するか |
...
| R-2 | M-20 の (b) 予算消尽経路 | 外部ツールが要る。(a) で `ShutdownTimeout` 自体は確認する |
