# macOS Clipboard 実装計画書 (v13)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03（v13 は 2026-09-05）
- 前版: `2026-09-03-macos-clipboard-design-v12.md`
...
| V-4 | コールバックのスレッド | macOS Standalone で AppKit のメインスレッド = Unity のメインスレッドであることの実測（7.5 #28）。**v13 で計測手段を用意した**（5.6.14）。それ以前は購読者側からもサンプルからも観測できず、実測する術が無かった |
...
| V-13 | 世代カウンタ撤去の妥当性 | D-13 で世代 3 変数を撤去した。single-flight により制御呼び出しが同時に 1 件しか存在しないこと（＝ pending スロットが所有者を一意に識別すること）が、実機の連続操作でも崩れないことを 7.2 と 7.5 #15〜#18 で確認する。iOS 版は世代を残しているため、両者の差は意図的な分岐である |
