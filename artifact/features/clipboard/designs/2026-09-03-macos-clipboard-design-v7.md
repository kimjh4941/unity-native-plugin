# macOS Clipboard 実装計画書 (v7)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v6.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v6.md`（**Codex による別モデルレビュー**）
...
v6 のレビュー（A1 3 件 / B 1 件、A2・C は 0 件）をすべて反映した。このラウンドは Claude 側 5 回とは別モデル（Codex）が担当しており、`review-document` の止める基準「レビュアーを替えて 1 回通していること」を満たす。3 件の A1 はいずれも Claude 側 5 回が検出できなかったものである。

| 分類 | 内容 |
| --- | --- |
| 監視登録の active / pending 分離 | **ネイティブは interval 検証と scope 解決を `stop()` より前に行うため、再 Start が失敗しても既存の監視は継続する**（IT-09 が保証）。v6 はネイティブ成功前に登録を上書きし失敗時に解放していたため、再 Start 失敗で旧 callback を失っていた。active と pending を分け、成功時のみ昇格する形に変更（A1-1） |
| `changeCount` の幅 | Swift `Int` は 64bit だが v6 は C# `int` に狭めていた。`TryGetInt` が `int` 範囲外を失敗させるため、正常な応答が 9006 になる。`long` に統一し `TryGetInt64` を追加（A1-2） |
| 結果型の `default` の位置づけ | `default(T)` は `IsSuccess=false` かつ `Error=null` かつ `Operation=null` になり、v6 が宣言した不変条件を破る。`default` を「未初期化値」と定義し、不変条件を factory 生成値に限定（A1-3） |
| 回帰テストの固定 | copy / append のスロット取り違えと、再 Start 失敗時の登録保持を PlayMode の必須項目に明記（B-1） |
| 世代カウンタの撤去 | active / pending の分離により、`ReleaseChangeRegistrationIfOwned` と世代 3 変数が不要になった（D-13） |
