# Windows Clipboard 実装計画書 v3

- 作成日: 2026-09-05
- 対象機能: clipboard
- 対象プラットフォーム: Windows（Windows 11 以降）
- 対象リポジトリ: unity-native-plugin（C# 層のみ）
- 前版: `2026-09-05-windows-clipboard-design-v1.md` → `-v2.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v1.md`（high 8 / medium 16 / low 10 / 不足 14 を v2 で反映）
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v2.md`（high 3 / medium 4 / low 1 / 不足 3 を本版で反映）
...
| 失敗地点 | 返却 | `renderers_` |
|---|---|---|
| 引数検証（owner null / 空 / fmt 0 / fn null） | `INVALID_PARAMETER`(1) | **旧世代のまま**（未変更） |
| `ClipboardScope` を開けない | `BUSY`(3) | **旧世代のまま**（未変更） |
| `EmptyClipboard()` 失敗 | `UNKNOWN`(19) | **旧世代のまま**（未変更） |
| 個別 `SetClipboardData` 失敗 → rollback 成功 | `UNKNOWN`(19) | `Clear()` 済み（空） |
| 個別 `SetClipboardData` 失敗 → rollback 失敗 | `PARTIAL_STATE`(13) | **新世代**（`partial_ = true`） |
| 全形式配置成功 | `NONE`(0) | 新世代 |
...
4. アパートメント確認（COM 所有権の記録を含む。M2 の反映）:
   - `CoGetApartmentType` が `S_OK` かつ `Sta` / `MainSta` なら**本層は何も初期化していない**（`s_comOwnership = None`）。そのまま次へ
...
| shutdown がタイムアウト / 失敗 | **しない**（保持したまま終了） | ネイティブがまだ COM オブジェクトと隠しウィンドウを保持している可能性があり、先に畳むと未定義動作になる。`Debug.LogWarning` に残す |

- 解放後は `s_comOwnership = None` に戻す（二重解放を防ぐ）
- `OnDestroy` の best-effort 経路（完了しない可能性がある）では解放しない
...
| 状態 | 新規操作の受付 | `_initialized` | ネイティブ資源 |
|---|---|---|---|
| `Uninitialized` | 拒否（`NotInitializedByHost`(1004)） | false | なし |
| `Running` | 受付 | true | 保持 |
| `Draining` | **拒否**（`ShuttingDown`(1011)）。**同期ドレインがユーザー callback を呼ぶ前に拒否へ切り替える**（callback 内からの新規呼び出しが受け付けられないようにする） | false へ落とす | 保持 |
| `ShutDown` | 拒否（`NotInitializedByHost`） | false | 解放済み |
| `Draining` から終端失敗 / タイムアウト | 拒否のまま | false | **保持したまま**（ネイティブが所有し続けるため C# 側の delegate・provider・キャッシュも解放しない） |

- `Initialize` は `ShutDown` からの再初期化を許可する（ネイティブは init 冪等 / 再 init 可能）。ただし tombstone が立っている場合は `ManagerDestroyed` が優先する
...
- teardown（`ShutdownWithDrain` の終了 / `OnDestroy` / `ResetForTests` / quit ドレインの打ち切り）では、
...
| ネイティブの結果 | `s_renderProviders` | `s_renderCache` |
|---|---|---|
| `NONE`(0) 成功 | 新世代へ**差し替える** | 全消去（新しい予約のため） |
| `INVALID_PARAMETER`(1) / `BUSY`(3) / `UNKNOWN`(19)（`EmptyClipboard` 失敗） | **旧世代を維持**（変更しない） | 変更しない |
| `UNKNOWN`(19)（配置失敗 → rollback 成功。native は `Clear()` 済み） | 旧世代・新世代のどちらが残るか C# から判別できないため、**両者を format 名でマージして保持**する（新世代のキーが優先） | 全消去 |
| `PARTIAL_STATE`(13) | native は新世代を保持しているため**新世代へ差し替える**。ただし旧世代のキーもマージして残す（安全側） | 全消去 |

- マージにより一時的に不要な provider を保持し得るが、次の予約成功・`RecoverDeferredState` 成功・shutdown 完了で整理される。
  「余分に保持する」ことは害が無く、「足りない」ことだけが形式の消失に直結するため、安全側に倒す
...
1. **純粋層**: `WindowsClipboardErrorCode` → 結果型 3 ファイル → `WindowsClipboardPayloads` → `WindowsClipboardJsonBuilder` / `WindowsClipboardJsonParser` →
...
3. **ライフサイクル**: アパートメント処理と COM 所有権 → `Initialize` → shutdown 状態機械 → `TryShutdown` / `ShutdownWithDrain` / `wantsToQuit` → `CanShutdownNow`
...
| `GetPreferredFormat` | 0 / 2 / 6 / 7 | 該当なしは空文字。`EMPTY` は返らない。`PickPreferredFormat` はクリップボードを開かない |
| `Clear` | 0 / 2 / 3 / 19 | |
...
| 予約世代マージ（H3） | 予約 A 成功 → 予約 B が `BUSY` で失敗 → A の provider が引き続き解決できること。`PARTIAL_STATE` / rollback 成功時のマージ規則 |
...
| `internal static void InjectReserveResultForTests(int pError)` | 予約の失敗地点（D-9）ごとの世代マージを検証する |
