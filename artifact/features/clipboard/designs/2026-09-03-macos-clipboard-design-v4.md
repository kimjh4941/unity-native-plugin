# macOS Clipboard 実装計画書 (v4)

- 対象機能: clipboard
- 対象プラットフォーム: macOS
- 対象パッケージ: `Packages/com.jonghyunkim.nativetoolkit`
- 作成日: 2026-09-03
- 前版: `2026-09-03-macos-clipboard-design-v3.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-03-macos-clipboard-design-review-v3.md`
...
v3 のレビュー（must fix 1 / should fix 5 / nit 9）をすべて反映した。v3 のレビューで A 領域（ネイティブ仕様との整合）は完成と判定されている。

| 分類 | 内容 |
| --- | --- |
| 実装可能性の修正 | 5.6.4 に拒否 dispatch の完全な骨格コードと 4 つのヘルパのシグネチャを追記（F-1）。これが v3 で唯一残っていた「書いてあるとおりに実装すると詰まる」箇所 |
| 型仕様の修正 | 5.5.1 の不変条件から入れ子型のコレクションを分離（S-1）。結果型が直接持つコレクションと、入れ子オブジェクト内のコレクションを書き分けた |
| 段の境界の修正 | 段 3 が宣言するイベントの範囲（S-2）、teardown が触る監視の static 状態が段 3 に必要であること（S-3）を明記 |
| テストの修正 | 段階 5（JSON 構築）の拒否経路を駆動する具体手段を追記（S-4） |
| 追加した設計 | per-call スロット表（5.6.13、N-5）、single-flight の粒度の要検証（V-12、N-7）、5.7 と 12 章の段の対応（N-6） |
| 表記の修正 | 5.2.1 / 5.2.2 の見出し階層（S-5）、引用行番号 3 件（N-1 / N-8）、1599 の発生源（N-2）、型の明示列挙（N-3）、個別 callback の挙動（N-4）、シームの補足（N-9） |
...
- Clipboard 機能の直近の前例である `IosClipboardManager.cs:194-201` は既にこの問題を認識し、`I1` で幅を固定したうえで理由コメントを付けている。macOS 版もこれに揃える
...
private static void InvokeNative(
    string operation, string inFlightKey, Action nativeCall, Action? onNativeFailure = null);
```
...
- メッセージ生成（`CouldNotStartMessage` / `BusyMessage` / `MainThreadMessage` / `DestroyedMessage` / `UnavailableMessage`）は 6.1 の文言を返す private static ヘルパ
...
- `MacClipboardOperationResult` を返す 3 操作は `Operation` からスロットを引く `TakeOperationCallback(string operation)` を用意する（`IosClipboardManager` と同形）
...
- **段 3 が宣言するのは、自分が実装する 5 操作の公開メソッドとイベント（`OwnershipChanged` / `ReadCompleted` / `ReadDataCompleted` / `ClearCompleted`）だけ。** 5.6.2 の公開イベント 13 本のうち残り 9 本は段 4 で追加する。段 2 で結果型 12 種はすべて揃っているので、どちらの順でもコンパイルは通る
- **teardown（5.6.8）は段 4 の監視機構に一部依存するため、段 3 に次の 3 点を先に入れる**（公開 API は段 4 のまま）
  ...
  3. 変更コールバックへの `DiscardIfTerminated` の適用
