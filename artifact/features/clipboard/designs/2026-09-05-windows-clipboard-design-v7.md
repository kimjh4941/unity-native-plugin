# Windows Clipboard 実装計画書 v7

- 作成日: 2026-09-05
- 対象機能: clipboard
- 対象プラットフォーム: Windows（Windows 11 以降）
- 対象リポジトリ: unity-native-plugin（C# 層のみ）
- 前版: `2026-09-05-windows-clipboard-design-v1.md` → `-v2.md` → `-v3.md` → `-v4.md` → `-v5.md` → `-v6.md`
...
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v3.md`（A1 3 領域 + 中 3 件 + C 8 件を v6 で反映）
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v4.md`（A1 1 種類 + B 2 件 + C 3 件を本版で反映。サーキットブレーカー適用）
...
**1 ファイル 1 型**とし、ファイル名とその主たる型名を一致させる（`common.md` の命名規約。macOS の `MacClipboard*Result.cs` と同じ粒度）。
`WindowsClipboardPayloads.cs` だけは既存の `IosClipboardPayloads` / `AndroidClipboardPayloads` に倣い、payload 群をまとめる。
...
| 観点 | 状態 | 根拠 |
|---|---|---|
| **P1** 新規ファイル名・型名の OS 接頭辞（テストを含む） | 適合 | Runtime 15 ファイル・テスト 7 ファイルすべて `Windows` 接頭辞。**1 ファイル 1 型でファイル名と主たる型名が一致**（6.1 / 6.2） |
...
| 6. 結果配送 | `origin` が `Drain` / `PublicApi` のときのみ、最終結果を共通イベントと per-call callback へ配送する（`TryShutdown` は 5.3 のとおり event 非発火） |
...
- **quit ドレインが終端失敗（`WrongThread` 注入）で終わっても quit が再開**し、`Application.Quit()` が多重起動しないこと
