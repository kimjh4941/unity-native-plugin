# Windows Clipboard 実装計画書 v2

- 作成日: 2026-09-05
- 対象機能: clipboard
- 対象プラットフォーム: Windows（Windows 11 以降）
- 対象リポジトリ: unity-native-plugin（C# 層のみ）
- 前版: `2026-09-05-windows-clipboard-design-v1.md`
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v1.md`（high 8 / medium 16 / low 10 / 不足 14 をすべて反映）
...
| 7 | 同期 API の結果配送を全経路 dispatcher 経由に統一（7.1 / 7.2 / 9.2 の三重矛盾を解消） | medium |
| 8 | `IsEmpty` 判定条件を API 別に修正、`HasFormat` の成功/存在を分離 | medium |
...
- **同梱 DLL は Clipboard 対応版へ差し替え済み**（2026-09-05 確認）。`Packages/.../Plugins/Windows/unity-windows-native-toolkit.dll` は
  `windows-native-toolkit-1.2.0.dll`（457,728 bytes）で、Clipboard の 27 export がすべて含まれている。残作業は `VERSION.txt` の `source:` 行のみ（2.7）
...
- **FALSE のときの `pError` は「エラー」ではなく「まだ終わっていない」**。`MONITOR_REGISTER_FAILED(12)` / `CANCELED(15)` / `BUSY(3)` などが入る。リトライ中は失敗として扱わない
...
| `Packages/.../Plugins/Windows/VERSION.txt` | バージョン行は `1.2.0` に更新済み。`source:` 行が `https://github.com/kimjh4941/native-toolkit/blob/main/dist/1.4.0/windows/windows-native-toolkit-1.1.0.dll` のまま古い | 同じフル URL 書式で `.../dist/1.11.0/windows/windows-native-toolkit-1.2.0.dll` へ更新する |
| `unity-windows-native-toolkit.dll.meta` | Win64 のみ enabled、Editor は disabled。ファイル名維持のため GUID も維持 | 変更不要 |
| `Microsoft.WindowsAppRuntime.Bootstrap.dll` | 同梱済み | 変更不要（Clipboard は WinAppSDK 非依存） |
| debug ビルド（`unity-windows-native-toolkit-debug.dll`） | **存在しない**（`native-toolkit/dist` 配下のどのバージョンにも無い） | 5.1 の `DLL_NAME` 方針を参照 |
...
- DLL 名定数を `DEVELOPMENT_BUILD` 時に `unity-windows-native-toolkit-debug` へ切り替えるが、**その DLL は配布物に存在しない**（2.7）。Clipboard ではこの分岐を持たない（5.1）
...
- `s_mainThreadId` / `s_dispatcher` を `Awake` でキャッシュする（`Instance` getter は GameObject を生成するためスレッドガードできない）
...
- `IosClipboardJsonReader` は `#if UNITY_IOS || UNITY_EDITOR` ガード内にあり `UNITY_STANDALONE_WIN` ではコンパイルされない。共有化のリファクタは iOS 実装とテストへ波及するため行わず、`JsonUtility` ベースの軽量パーサを新設する
...
- `private const string DLL_NAME = "unity-windows-native-toolkit";` の**単一定数**とする。`DEVELOPMENT_BUILD` 時の `-debug` 分岐は採らない（debug 版 DLL は配布物に存在せず、development build で全 API が `BridgeUnavailable` になるため）
...
| `uninitClipboardManager` | `WindowsClipboardResult TryShutdown(out bool completed)` | **同期**（ネイティブと同じ形） |
| （上記のフレーム跨ぎヘルパ） | `void ShutdownWithDrain(Action<WindowsClipboardResult>? onResult = null)` | Unity 側ヘルパ（7.4 に逸脱理由） |
| `canDestroyClipboardManager` | `WindowsClipboardFlagResult CanShutdownNow()` | 同期 |
...
| `cancelClipboardRequest` | `WindowsClipboardResult CancelRequest(uint requestId)` | 同期 |
...
| `WindowsClipboardReadResults.cs` | `WindowsClipboardTextResult` / `WindowsClipboardStringListResult` / `WindowsClipboardBytesResult` / `WindowsClipboardFormatPresenceResult` / `WindowsClipboardFlagResult` |
| `WindowsClipboardHistoryResults.cs` | `WindowsClipboardHistoryItem` / `WindowsClipboardHistoryResult` / `WindowsClipboardAvailabilityResult` |
...
| `Tests/PlayMode/WindowsClipboardManagerIntegrationTests.cs` | PlayMode（Editor 内） | 拒否経路、7 種の共通 event の発火、発火しない契約、`Awaitable` の完了保証、teardown ドレイン、`ResetForTests` による分離 |

### 6.3 既存変更

| ファイル | 変更内容 | 状態 |
|---|---|---|
| `Packages/.../Plugins/Windows/unity-windows-native-toolkit.dll` | Clipboard 対応版（1.2.0 相当）へ差し替え | **対応済み**（2026-09-05） |
| `Packages/.../Plugins/Windows/VERSION.txt` | `source:` 行を同じフル URL 書式で `https://github.com/kimjh4941/native-toolkit/blob/main/dist/1.11.0/windows/windows-native-toolkit-1.2.0.dll` へ更新 | 未対応 |
...
- **イベントと per-call callback は、同期・非同期・拒否のすべての経路で `UnityMainThreadDispatcher.Enqueue` を通し、次の `Update` で発火する**。
...
- XML コメントに「戻り値は即時、イベント / callback は次フレーム」と明記する
...
4. アパートメント確認:
   - `CoGetApartmentType` が `S_OK` かつ `Sta` / `MainSta` ならそのまま次へ
   - それ以外なら `CoInitializeEx(IntPtr.Zero, 0x2 /* COINIT_APARTMENTTHREADED */)` を 1 回だけ試行する
     - `S_OK` / `S_FALSE`（既に初期化済み）なら成功として次へ
...
// フレーム跨ぎヘルパ（内部で TryShutdown をコルーチンから繰り返す）
...
- **`completed == false` の間の `pError` は失敗として扱わない**（2.6。`MONITOR_REGISTER_FAILED` / `CANCELED` / `BUSY` は「まだ終わっていない」の意味）。ログには残すが結果型には反映しない
- リトライ中に `pError == PARTIAL_STATE` または `deferred` 由来の失敗が続く場合、諦める前に `recoverDeferredState` を 1 回試す
...
- `Initialize` 成功時に `Application.wantsToQuit += OnWantsToQuit;` を購読する。**`OnDestroy` と `ResetForTests` で必ず解除する**（static event のため多重購読が残る）
- `OnWantsToQuit`:
  ...
  4. ドレイン完了時（成功・`ShutdownTimeout` のいずれでも）に `s_quitDrainCompleted = true` を立てて `Application.Quit()` を再実行する
- **`ShutdownTimeout` でも終了は通す**（アプリが終了できなくなる事態を避ける。タイムアウトは `Debug.LogError` に残す）
- `OnDestroy` は最後の砦として `TryShutdown` を 1 回だけ呼ぶ（best-effort、戻り値をログに残す）。この経路ではフレームが進まないため完了しない可能性がある
...
- 読み出し系は **2 回呼び出しバッファ規約**:
  1. `buffer = IntPtr.Zero, bufferSize = 0` で呼び、必要サイズ（wchar_t 数 or バイト数）を得る
  2. **必要サイズが 0 なら `pError` の値に関わらず「空の成功」として扱う**（`pasteImage` / `pasteCustomFormat` はデータ長 0 のとき戻り値 0 + `BUFFER_TOO_SMALL` を返すため、`pError == NONE` を条件にすると取りこぼす。2.9）
...
| API | 空の判定 |
|---|---|
| `PastePlainText` / `PasteHtml` | `pError == EMPTY` または `FORMAT_UNAVAILABLE`、あるいは必要サイズ 0 |
| `PasteFiles` | 同上、またはパース後の要素数 0 |
| `PasteImage` / `PasteCustomFormat` | `pError == EMPTY` / `FORMAT_UNAVAILABLE`、または**必要サイズ 0**（`BUFFER_TOO_SMALL` を伴う） |
| `GetFormats` | **`EMPTY` は返らない。**`"[]"` が返るのでパース後の要素数 0 で判定する |
| `GetPreferredFormat` | **`EMPTY` は返らない。**取得文字列が空文字なら空（2.9） |
...
private static readonly WindowsClipboardRequestTable s_pending = new();                   // メインスレッドのみが触る
```

- 呼び出し手順: 事前チェック（7.5 と同じ）→ in-flight ガード（7.6.4）→ `getClipboardHistory(s_requestDelegate, out pError)` → `requestId != 0` なら表に登録して return
...
- `requestId == 0`（受付前失敗）: `pError` に基づく失敗結果を配送する（dispatcher 経由）。`pError == 0` は理論上到達しないが `RequestRejected`(1008) で防御する
- `OnRequestCompleted(requestId, error, json)`: 表から取り出し（未知 ID は警告ログのみ）、JSON をパースし、dispatcher で配送する。二重完了は表側で抑止する
...
**問題**: 通常経路の配送は dispatcher 経由（次の `Update`）だが、`OnDestroy` 後・Quit 確定後・PlayMode 終了時は `Update` が回らない。
pending の per-call callback と `AwaitableCompletionSource` が完了しないまま捨てられる。

**契約**:

- teardown（`TryShutdown` の完了 / `OnDestroy` / `ResetForTests` / quit ドレインの打ち切り）では、
  リクエスト表に残る全エントリを **dispatcher を経由せず同期的に**完了させる
- 完了時の結果は `Canceled`(15)（ネイティブのドレインで既に届いた場合はそれを優先）または `ManagerDestroyed`(1003)
- ドレインは**冪等**にする（ネイティブの `CANCELED` コールバックと二重に走っても、表側の二重完了抑止で 1 回に収束する）
- この同期発火は 4 章「dispatcher 経由」ルールの明示的な例外であり、理由（`Update` が回らない）をコードコメントに書く
- これにより「受付済みリクエストは必ず 1 回完了する」（8.4）が teardown を含めて成立し、`Awaitable` の永久未完了が発生しない
...
| `RestoreHistoryItem` | **不可** | クリップボード内容を置き換えるため、並行実行すると最後に完了したものが勝ち、呼び出し側の意図と食い違う | 操作単位の in-flight ガード |
...
- `Awaitable` 版: requestId を返せないため `CancellationToken` を最終引数で受け、登録済みトークンのキャンセル時に内部で `CancelRequest` を呼ぶ。
  MonoBehaviour からは `destroyCancellationToken` を渡す運用を XML コメントに例示する
- `cancelClipboardRequest` が `false` を返しても完了は届き得るため、表からは削除しない
...
private static Dictionary<string, Func<byte[]>>? s_renderProviders;                  // static 保持が必須
...
- キャッシュと `s_renderProviders` のクリアは、次回 `ReserveDeferredFormats` 呼び出し時、または `TryShutdown` が `completed == true` を返した後に行う（7.4）
...
1. **純粋層**: `WindowsClipboardErrorCode` → 結果型 3 ファイル → `WindowsClipboardPayloads` → `WindowsClipboardJsonBuilder` / `WindowsClipboardJsonParser` → `WindowsClipboardRequestTable`（EditMode テストを先に書く）
2. **Manager 骨格**: Singleton / `Awake`（`s_mainThreadId` / `s_dispatcher` 捕捉）/ `OnDestroy` / ログ / プラットフォームガード / `InvokeInOrder` / 7.10 の static リセット seam
3. **ライフサイクル**: アパートメント処理 → `Initialize` → `TryShutdown` / `ShutdownWithDrain` / `wantsToQuit` → `CanShutdownNow`
   - **この段階で V-1 / V-2 を実機検証する**（DLL は差し替え済みのため即座に実施できる）
4. **同期 API**: 書き込み 6 種 → 読み出し 5 種 → 内容確認 3 種 → `Clear`
5. **非同期 API**: リクエスト表結線 → 履歴 5 種 → in-flight ガード → teardown ドレイン → `CancelRequest` → `Awaitable` 版（ガードとドレインの完了後）
6. **イベント**: `ClipboardChanged` → `SetHistoryEventsEnabled` と履歴 3 イベント
7. **遅延レンダリング**: `ReserveDeferredFormats` / `RecoverDeferredState`
8. **仕上げ**: 実機確認（9.3）、`VERSION.txt` の `source:` 行更新（6.3）
...
- リセット対象: `s_pending` / `s_renderProviders` / `s_renderCache` / in-flight セット / `_initialized` / `s_isTerminated`（tombstone）/ `s_quitDrainStarted` / `s_quitDrainCompleted` / `s_mainThreadId` / `s_dispatcher` / `_instance`
...
- **Reload Domain / Scene Reload をともに無効化した構成**では `Awake` が再実行されないため、`s_mainThreadId` / `s_dispatcher` は
  `RuntimeInitializeOnLoadMethod` 側でも再捕捉できる形にする（捕捉済みなら何もしない）
- teardown 中の再生成を防ぐため、`s_isTerminated == true` の間は `Instance` getter から新規生成しない旨を XML コメントに明記する（getter 自体はガードできないため運用契約とする）
...
| `TryShutdown` | 0 / 3 / 12 / 13 / 14 / 15 | **`completed == false` のときの値は「未完了」の意味**（2.6） |
| `CanShutdownNow` | 0 / 2 | 状態問い合わせのみ |
| copy 系 6 種 | 0 / 1 / 2 / 3 / 6 / 8 / 13 / 19 | 13 は `CopyMultipleFormats` のみ。`RecoverDeferredState` では回復できない |
| paste 系 5 種 | 0 / 1 / 2 / 3 / 4 / 5 / 6 / 7 / 8 / 19 | 7 は 2 回呼び出しの 1 回目で必ず出る（正常系） |
| `HasFormat` | 0 / 1 / 2 | 失敗時も戻り値は `FALSE`（2.9） |
| `GetFormats` | 0 / 2 / 3 / 7 / 8 | 空でも `"[]"`。`EMPTY` は返らない |
| `GetPreferredFormat` | 0 / 2 / 3 / 7 | 該当なしは空文字。`EMPTY` は返らない |
| `Clear` | 0 / 2 / 3 / 19 | |
| `ReserveDeferredFormats` | 0 / 1 / 2 / 3 / 13 / 14 / 8 | 14 は所有 UI スレッド外。既存内容を破棄する（D-1） |
| `RecoverDeferredState` | 0 / 2 / 13 / 14 / 19 | 対象は deferred の partial 状態のみ |
| 履歴 5 種（受付前） | 1 / 2 / 8 / 19 | requestId 0 + `pError`。コールバックは呼ばれない |
| 履歴 5 種（受付後） | 0 / 9 / 10 / 11 / 15 / 17 / 19 | **`GetHistoryAvailability` も 17 を返す**（前面でないと設定を取得できない） |
| `CancelRequest` | 0 / 2 / 19 | `false` でも完了は届き得る |

**`Busy`(3) の扱い**: 他プロセスがクリップボードを排他保持している一時的な失敗であり、全同期 API で発生し得る。
...
- 受付済み（`requestId != 0`）の非同期リクエストは、成功・失敗・キャンセル・shutdown ドレイン・teardown ドレインのいずれかで**必ず 1 回**結果が配送される
- 拒否（受付前失敗）でも共通イベントと per-call callback は必ず発火する
- 同期 API は戻り値で即時に結果を返し、イベント / callback は次の `Update` で発火する（teardown ドレインのみ同期発火）
...
| `WindowsClipboardRequestTable` | 登録 / 取り出しで削除、未知 ID の完了が無害、二重完了の抑止、ドレインで全件が 1 回ずつ完了、ドレインの冪等性 |
| `InvokeInOrder` | 共通イベント → 個別 callback の順序、片方 null、購読者の例外が他方の発火を妨げないこと |
| バッファ helper | 必要サイズ 0 は `pError` に関わらず空成功、`BUFFER_TOO_SMALL` の再試行上限 |
...
- **7 種の共通 event がそれぞれ発火すること**（`ClipboardOperationCompleted` / `TextReadCompleted` / `StringListReadCompleted` / `BytesReadCompleted` / `FormatPresenceChecked` / `HistoryReadCompleted` / `HistoryAvailabilityChecked`）
- **発火しない契約**: `Initialize` の失敗時に読み出し系 event が発火しないこと
- dispatcher の `Update` flush を跨いだ順序保証（共通 → 個別）と、「戻り値は即時・イベントは次フレーム」の契約
- `GetHistoryAsync` 等が拒否時にも必ず完了し、ハングしないこと
- teardown ドレイン: pending がある状態で `OnDestroy` / `ResetForTests` を実行すると、per-call callback と `Awaitable` が同期的に完了すること
- `ShutdownWithDrain` がリトライ後に完了結果を返すこと（ネイティブ不在環境では拒否結果で完了）
- `ResetForTests` によりテスト間で tombstone / in-flight ガード / リクエスト表が分離されること（テスト実行順序に依存しないこと）
...
| M-10 | 他アプリでコピー | `ClipboardChanged` が発火。自プロセスの copy では発火しない |
...
| # | 事項 | 影響 | 対応 |
|---|---|---|---|
| V-1 | Unity Windows Player のメインスレッドが STA として初期化されているか | MTA の場合 `initClipboardManager` が `WrongApartment` で失敗し、`CoInitializeEx` も `RPC_E_CHANGED_MODE` で回復不能。その場合は「専用 STA スレッド + 自前メッセージポンプ」を C# 層に持つ代替設計（大幅な追加実装）が必要になる | 7.9 の 3 で実機検証する。MTA だった場合は実装を止めて設計を見直す |
...
| V-3 | `Application.wantsToQuit` 経由のドレインが Player で確実に動作するか（タイムアウト時の終了可否を含む） | 予約形式の消失、または終了できない不具合 | M-19 / M-20 で確認 |
...
| V-6 | ~~debug 版 DLL の有無~~ | — | **解決済み**（2026-09-05）。`dist` 配下に存在しないため単一 DLL 名を使う（5.1）。既存 `WindowsNotificationManager` が development build で DLL を解決できない点は別途の是正候補として記録 |
...
- [ ] 非同期 API は requestId を返し、受付済みリクエストが必ず 1 回だけ完了する（teardown を含む）
- [ ] `Awaitable` 版は in-flight ガード（7.6.4）と teardown ドレイン（7.6.3）の実装後にのみ公開されている
- [ ] `Initialize` / `TryShutdown` / `ShutdownWithDrain` / `wantsToQuit` が 7.3 / 7.4 の規則どおり動作する
- [ ] 遅延レンダリングが 2.8 の D-1 〜 D-8 をすべて満たす
...
- [ ] `#nullable enable` / コンパイルガード / `using` 配置が既存ファイルと同形
...
- [ ] 9.3 の手動確認 M-1 〜 M-22 を実機で確認し、結果を実装結果ファイルに記録した

### 構成

- [ ] `VERSION.txt` の `source:` 行が更新されている
- [ ] 6.4 の非変更ファイルに差分が無い
...
- 既存 `WindowsNotificationManager` の是正（`-debug` DLL 名、非 Windows での無言 return）
