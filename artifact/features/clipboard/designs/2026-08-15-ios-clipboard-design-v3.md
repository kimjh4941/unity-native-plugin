...
- **すべて Unity main thread からのみ呼べる**（5.8）
...
| ファイル | 役割 |
|---|---|
| `IosClipboardManager.cs` | Bridge + Manager 本体（Singleton、15 操作 callback 版、10 event、single-flight ガード、busy 専用 dispatch、main thread 検証） |
...
| E-14 | **base64 の文字数から算出した最大デコード長が上限（64MiB）を超える** | 失敗 `CLIPBOARD_CONTENT_TOO_LARGE` / `The clipboard content exceeds the configured size limit.`（native と同一コード・文言）。**バッファを確保する前に判定する** |
...
1. base64 トークンの**文字数**から最大デコード長を算出する（`(length / 4) * 3`、パディング分は上振れ側の見積り）
...
private void Awake()
{
    ...
    s_dispatcher   = UnityMainThreadDispatcher.Instance;   // メインスレッド上で先に起こす
}
...
**破棄時の pending callback 契約（レビュー高優先度 2 の反映）:**

`ClearAllPendingCallbacks()` は、破棄時点で native の応答を待っている per-call callback を**破棄する**。破棄後に native コールバックが到着した場合、per-call callback は呼ばれず、共通 event も `_instance == null` のため発火しない（`Debug.LogWarning` に事実のみ記録する）。
...
- `OnDestroy` は `s_inFlight` も空にする。破棄後に再生成された Manager が busy 状態を引き継がないようにするため
...
|---|---|---|---|
| 1 | main thread 検証（5.8） | **rejected 経路** | 触れない |
| 2 | 引数検証（null / 空 patterns） | **rejected 経路** | 触れない |
| 3 | プラットフォーム検証（非 iOS / Editor） | **rejected 経路** | 触れない |
| 4 | `TryBeginOperation` | **rejected 経路**（busy） | 触れない |
| 5 | per-call スロットへ `onResult` を格納 | - | ここで初めて所有する |
| 6 | native 呼び出し（`DllImport` 例外） | **通常経路** | 自分が所有しているので解放する |
| 7 | native コールバック到着 | **通常経路** | スナップショット → クリア → `EndOperation` |

**段階 1〜4 の失敗は、この呼び出しが pending / in-flight を一度も所有していない。** したがって通常の `FireXxxResult` を通してはならない。
...
段階 6（`DllImport` 例外）だけは自分が pending / in-flight を所有しているため、通常経路（`FireXxxResult`）で `TakeXxxCallback` + `EndOperation` を行う。
...
| タイミング | 処理 |
|---|---|
| `StartObserving` 入口（段階 5） | `var generation = ++s_observingGeneration;` `s_onChanged = onChanged;` `s_onChangedGeneration = generation;` |
| 開始失敗（Editor / 非 iOS / `DllImport` 例外 / native の `CLIPBOARD_UNAVAILABLE`） | `if (s_onChangedGeneration == generation) { s_onChanged = null; }` |
| `StopObserving` 成功 | `if (s_onChangedGeneration <= generationAtStopCall) { s_onChanged = null; }` |
...
- 段階 1〜4 で拒否された `StartObserving` は `s_onChanged` に触れない（5.6.3 の不変条件 1）。世代も進めない
- `StopObserving` は共有キーにより `StartObserving` と交差しないが、世代判定は**二重の防御**として残す（native 側の完了順に依存しない）
...
**通常経路（この呼び出しが pending / in-flight を所有している場合にのみ使う。5.6.3 段階 6〜7）:**
...
- 各 public メソッドの**最初の段階**（5.6.3 の段階 1）で検証する
- 違反時は **B-9（`CLIPBOARD_MAIN_THREAD_REQUIRED`）** で拒否し、rejected 経路（5.6.3）で結果を返す。`s_inFlight` にも pending スロットにも触れない
- 検証は**すべてのビルド構成で行う**（Editor / Development Build 限定にしない）。`HashSet` の破損は静かに進行し、後から原因を特定できないため。コストは `int` の比較 1 回
- dispatch には `Awake` でキャプチャした `s_dispatcher` を使う（`Enqueue` 自体は `lock` により thread-safe。5.6.1）
- `s_dispatcher == null` の場合は dispatch できないため、`Debug.LogError` のみ記録する
...
3. **確保前の上限確認は `byteCount` ではなく base64 の文字数から行う。** 宣言値を信用すると、過小な `byteCount` に大きな base64 を添えた応答を通してしまう（5.5.1 の `byteCount` 信頼境界）。`(base64 文字数 / 4) * 3` が 64MiB を超えるなら確保せず E-14 で失敗し、デコード後に `Data.Length == byteCount` を検証する（E-15）
...
| B-10 | base64 の最大デコード長が上限を超える（E-14） | `CLIPBOARD_CONTENT_TOO_LARGE` | `The clipboard content exceeds the configured size limit.`（native と同一文言） |

**dispatch 経路の対応（5.6.3）:**

| エラー | 経路 | pending / in-flight への影響 |
|---|---|---|
| B-9（main thread 違反） | **rejected** | 触れない |
| B-3 / B-4（引数不正） | **rejected** | 触れない |
| B-1（非 iOS） | **rejected** | 触れない |
| B-0（busy） | **rejected** | 触れない |
| B-2（`DllImport` 例外） | 通常 | 自分が所有しているので解放する |
| B-5 〜 B-8 / B-10（応答の解析） | 通常 | 同上（native コールバック到着後） |

- B-0 / B-1 / B-2 / B-9 の `CLIPBOARD_BUSY` / `CLIPBOARD_BRIDGE_UNAVAILABLE` / `CLIPBOARD_MAIN_THREAD_REQUIRED` は **C# 側でのみ使う新規コード**（native の `ClipboardError` には存在しない）。`CLIPBOARD_BRIDGE_UNAVAILABLE` は `AndroidClipboardManager` の同名定数と値を揃える
...
| **`Destroy` 後に native コールバック相当が到着しても例外にならず、`s_inFlight` を持ち越さないこと**（5.6.1 の破棄時契約） |
...
10. **全 public API が Unity main thread 限定として XML コメントに明記され、違反が B-9 で拒否される**（5.8.1）
11. **`StartObserving` / `StopObserving` が single-flight キー `observation` を共有し、observation generation で登録を自己判定する**（5.6.4）
12. 3.5 の公開型仕様どおりに型が実装されている（nullability・collection の非 null 契約を含む）
13. 5.5.1 の E-1〜E-15、5.5.2 の変更イベント破棄規則が実装されている
14. 6.3 の C# Bridge 層エラー B-0〜B-10 がすべて実装されている
15. **base64 の上限確認が `byteCount` ではなく base64 文字数から行われ、`readData` で `Data.Length == byteCount` が検証されている**（5.5.1 / 5.9.2）
16. 5.9.2 の response 側メモリ削減策 1〜4 が実装されている
17. 層 1 / 層 2a のテストが追加され、Unity Test Runner で全 passed
18. クリップボード本文・base64・検出値・pasteboard 名がログに出力されない
19. `public` メンバに英語の XML ドキュメントコメントがある
20. 7.3 の手動確認項目 M-1〜M-25 が実機で確認済み（未実施項目は理由とともに記録）
