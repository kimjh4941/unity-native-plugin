# Windows Clipboard 実装計画書 v6

- 作成日: 2026-09-05
- 対象機能: clipboard
- 対象プラットフォーム: Windows（Windows 11 以降）
- 対象リポジトリ: unity-native-plugin（C# 層のみ）
- 前版: `2026-09-05-windows-clipboard-design-v1.md` → `-v2.md` → `-v3.md` → `-v4.md` → `-v5.md`
...
- レビュー: `artifact/features/clipboard/reviews/2026-09-05-windows-clipboard-design-review-v3.md`（A1 3 領域 + 中 3 件 + C 8 件を本版で反映）
...
| public `TryShutdown` | メインスレッド判定 → `TryShutdownCore` | 迂回する（`Uninitialized` のときだけ `NotInitializedByHost` を返して何もしない） |
...
| 状態 | 新規操作（shutdown 以外）の受付 | shutdown 経路 | `_initialized` | ネイティブ資源 |
|---|---|---|---|---|
| `Uninitialized` | 拒否（`NotInitializedByHost`(1004)） | 何もせず成功扱い | false | なし |
| `Running` | 受付 | core を実行し `Draining` へ | true | 保持 |
| `Draining` | **拒否**（`ShuttingDown`(1011)）。**同期ドレインがユーザー callback を呼ぶ前に拒否へ切り替える** | core を再実行（リトライ） | false | 保持 |
| `ShutDown` | 拒否（`NotInitializedByHost`） | 何もせず成功扱い（冪等） | false | 解放済み |
| `ShutdownFailed`（終端失敗 / タイムアウト） | 拒否（`ShuttingDown`） | core を再実行してよい | false | **保持したまま**（ネイティブが所有し続けるため C# 側の delegate・provider・キャッシュも解放しない） |

**public `TryShutdown` を直接呼んだ場合の遷移**（A1 指摘の反映）

利用者が `ShutdownWithDrain` を介さず `TryShutdown` を呼ぶ経路も同じ状態機械に載せる。
ネイティブ `Uninit` は最初の呼び出しで lifecycle gate を閉じるため、C# 側を `Running` のまま放置すると以後の操作が静かに不整合になる。

| 呼び出し時の状態 | 結果 | 遷移後 |
|---|---|---|
| `Running` | `completed == true` | `ShutDown`（COM 解放、キャッシュ解放） |
| `Running` | `completed == false`（未完了） | **`Draining`**（gate は既に閉じているため新規操作を拒否する） |
| `Running` | 終端失敗（`WrongThread` 等） | `ShutdownFailed` |
| `Draining` | `completed == true` | `ShutDown` |
| `Draining` | `completed == false` | `Draining` のまま |
| `ShutDown` / `Uninitialized` | 何もせず `completed == true` | 変化なし |

- `Initialize` は `ShutDown` / `ShutdownFailed` からの再初期化を許可する（ネイティブは init 冪等 / 再 init 可能）。ただし tombstone が立っている場合は `ManagerDestroyed` が優先する
