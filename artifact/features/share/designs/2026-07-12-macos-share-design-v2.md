...
- 更新履歴: v2 = レビュー（`artifact/features/share/reviews/2026-07-12-macos-share-design-review-v1.md`）低優先度 2 件を反映（PlayMode テストの変更一覧追加 / `ServiceName` 表示名の明示 + 参考定数）
...
/// <see cref="MacShareResult.ServiceName"/>.
/// </summary>
public static class MacShareServiceNames
{
    public const string MailCompose = "com.apple.share.Mail.compose";
    public const string Message     = "com.apple.share.Message.window";
    // 必要に応じて既知値を追加（未提供のサービスは serviceUnavailable エラーになる）
}
```

- 網羅は目的とせず、代表値のみ。任意文字列渡しは引き続き可能。
...
- `MacShareServiceNames` の各 raw 識別子（`MailCompose` 等）が対象環境で有効に解決されること（無効時は `serviceUnavailable` エラー）。
