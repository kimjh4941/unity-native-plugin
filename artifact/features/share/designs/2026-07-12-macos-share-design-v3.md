...
- 更新履歴:
  - v2 = レビュー v1 低優先度 2 件を反映（PlayMode テストの変更一覧追加 / `ServiceName` 表示名の明示 + 参考定数）
  - v3 = レビュー（`artifact/features/share/reviews/2026-07-12-macos-share-design-review-v2.md`）中優先度 1 件を反映（`MacShareServiceNames` の raw 識別子誤り修正 + 各定数の実機確認を計画化）
...
- **注意**: `MacShareManager`（Swift）には `canPerform(content:serviceName:)` が存在するが、**C Bridge には公開されていない**。C# からサービス可否を事前照会する手段はなく、可否は `shareViaService` 実行時の `serviceUnavailable` エラーで判定する。
...
- `serviceName`: 選択されたサービスの**表示名**（`NSSharingService.title`、例: `"Mail"`）。キャンセル/不明/失敗時は NULL。
  - **注意**: これは表示名であり、`shareViaService` の入力に使う **raw `NSSharingService.Name` 識別子（例: `"com.apple.share.Mail.compose"`）とは別物**。callback の `serviceName` を次回 `shareViaService` の入力へそのまま再利用することはできない。
...
- `shareViaService` の入力 `serviceName` は raw な `NSSharingService.Name` 値（例: `"com.apple.share.Mail.compose"`）。ネイティブ側に固定の公開定数は無い。
- C# 側では任意文字列として受け渡すが、利用者の利便のため既知値を参考定数として `MacShareServiceNames` に提供する（§5.2 参照）。
- **表示名（callback の `serviceName`）と raw 識別子（`shareViaService` 入力）は別物**であり、混同しない命名にする。
- **参考定数の値は実装前に対象 SDK の `NSSharingService.Name.*.rawValue` で確認する**（§5.2 / §8）。
...
- **PlayMode テスト（`IosShareManagerIntegrationTests`）**: 既存の Share PlayMode テストが存在。Instance 生成・dispatcher 経由の callback flush 経路を確認するパターン。→ macOS 版もこれに倣う。
...
| パス | 内容 |
| --- | --- |
| `Runtime/Share/MacSharePayloads.cs` | `MacShareItem`（Text/Url/Image/File ファクトリ）+ `MacShareContentPayload`（items / recipients / subject / excludedServiceTitles）+ `MacShareServiceNames`（既知 raw `NSSharingService.Name` の参考定数）。ガードなし |
...
| `Tests/PlayMode/MacShareManagerIntegrationTests.cs` | `MacShareManager.Instance` 生成・イベント購読・`UnityMainThreadDispatcher` 経由の callback flush 経路・非 OSX プレイヤーでの即時 `Failure` 経路を検証。既存 `IosShareManagerIntegrationTests.cs` に準拠 |
...
### 5.2 `MacShareServiceNames`（参考定数）

- `shareViaService` の raw 識別子（`NSSharingService.Name`）の既知値を提供する参考定数クラス。callback の**表示名（`MacShareResult.ServiceName`）とは別**であることを命名と XML コメントで明示する。
- **各定数値は実装前に対象 SDK の `NSSharingService.Name.*.rawValue` で確認すること**（誤った値は `serviceUnavailable` になり利用者を誤誘導する）。
- **確認済み方針**: 実機検証済みの `MailCompose` を基本とする。`Message` はレビューで raw 値が誤り（`com.apple.share.Message.window` は無効）と判明したため、SDK 確認済みの `com.apple.messages.ShareExtension` に修正する。未検証の値は追加しない。

```csharp
/// <summary>
/// Well-known raw <c>NSSharingService.Name</c> identifiers for
/// <see cref="MacShareManager.ShareViaService"/>.
/// These are input identifiers, NOT the display names returned in
/// <see cref="MacShareResult.ServiceName"/>. Confirm each value against the
/// target SDK's <c>NSSharingService.Name.*.rawValue</c> before relying on it.
/// </summary>
public static class MacShareServiceNames
{
    /// <summary>Compose a new Mail message. (NSSharingService.Name.composeEmail)</summary>
    public const string MailCompose = "com.apple.share.Mail.compose";

    /// <summary>Compose a new Messages message. (NSSharingService.Name.composeMessage)</summary>
    public const string Message = "com.apple.messages.ShareExtension";
}
```

- 網羅は目的とせず、SDK 確認済みの代表値のみ。任意文字列渡しは引き続き可能。
...
public readonly struct MacShareResult
{
    ...
    public string? ServiceName { get; }    // 選択サービスの「表示名」。失敗・キャンセル時 null
...
- **`ServiceName` の XML コメントに明記する**: 「これは表示名（`NSSharingService.title`）であり、`ShareViaService` に渡す raw service identifier ではない。次回呼び出しの入力に再利用しないこと」。raw 識別子は `MacShareServiceNames`（§5.2）を使う。
...
2. `MacSharePayloads`（`MacShareServiceNames` を含む）
3. `MacShareJsonBuilder`
4. `MacShareManager`
5. テスト（Result → JsonBuilder → Dispatch(EditMode) → Integration(PlayMode)）
...
### 7.2 PlayMode（`Tests/PlayMode/MacShareManagerIntegrationTests.cs`）

- `MacShareManager.Instance` 生成とイベント購読が機能すること。
- `UnityMainThreadDispatcher` 経由で per-call callback / 共通イベントがメインスレッドで flush される経路（既存 `IosShareManagerIntegrationTests.cs` と同様に dispatcher を回して結果を受領）。
- 非 OSX プレイヤー / native 呼び出し不可経路での即時 `Failure`（`RuntimePlatform` 依存分岐）の確認。

### 7.3 手動確認（実機 macOS 15+ / ネイティブ Bridge 依存）

- `ShareViaService`（verified-safe）: Mail 等で共有完了 → `Completed=true` / `ServiceName`（表示名）取得。
- **`MacShareServiceNames` の各定数を `ShareViaService` で実行し、`serviceUnavailable` にならず正しく起動することを確認する**（`MailCompose` / `Message`）。C Bridge に `canPerform` が無いため、可否確認は `ShareViaService` 実行で行う。
...
- **`MacShareServiceNames` の各 raw 識別子（`MailCompose` = `com.apple.share.Mail.compose`, `Message` = `com.apple.messages.ShareExtension`）が対象 SDK の `NSSharingService.Name.*.rawValue` と一致すること**を実装前に確認する。未検証値は定数から除外する（誤値は `serviceUnavailable` を誘発）。
