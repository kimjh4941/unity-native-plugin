# iOS Share サンプルシーン 実装計画書 (v1)

- 対象機能: share（iOS システム共有シート）
- 対象プラットフォーム: iOS（最小 iOS 18）
- 作成日: 2026-07-05
- 参照実装結果: `artifact/features/share/results/2026-07-05-ios-share-implementation-feature-result-v3.md`
...
- `TopMenuExampleController.OnShareClicked`: 既存の Android 分岐に iOS を追加:
  ```csharp
  #if UNITY_EDITOR
      // Editor: iOS/Android どちらの Share もプレビュー可能だが、既存 Dialog/Notification の
      //         Editor 挙動（DisplayDialog 案内）と揃えるか、iOS Share を優先表示するか要確定。
  #elif UNITY_ANDROID
  ...
  #endif
  ```
  - **要確定事項**: Editor では現状 `ShowAndroidShare` に飛ぶ。iOS Share も Editor 参照可能なため、
    Editor で iOS Share を見せるか Android を見せるかは実装時に確定する（既存 Dialog/Notification は
    Editor で `EditorUtility.DisplayDialog` の案内表示。Share は Android のみ実画面表示していた）。
    暫定方針: Editor では従来どおり `ShowAndroidShare`（既存挙動を壊さない）とし、iOS 実機で
    `ShowIosShare` を表示する。実装時に見直す。
...
- ExcludingActivities: Copy / Facebook が共有シートに表示されない。
...
- Editor 実行: 各ボタンで `Error: iOS share is only available on an iOS device.` が表示され、
  導線・イベント購読・結果表示の疎通が確認できる。
...
- Editor での Share ボタン遷移先（Android or iOS）は実装時に確定する（5.6 要確定事項）。
- `excludedActivityTypes` / 画像・ファイル共有の実機挙動は要実機確認。
