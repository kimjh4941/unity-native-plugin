...
- v3 変更点: **platform guard 戦略を「クラスは `UNITY_IOS || UNITY_EDITOR` で常時存在させ、native 呼び出しだけ `UNITY_IOS && !UNITY_EDITOR` に閉じる」へ変更**（再レビュー高優先度 #2 の代替案を採用）。これにより Editor から `IosShareManager` を全ビルドターゲットで参照可能にし、Manager 本体（入力ガード・非 iOS Failure・dispatch 順序）を EditMode で直接検証できるようにした（第 9 章に反映内容を集約）。
...
- 公開メソッド `Share(IosShareContentPayload? payload, Action<IosShareResult>? onResult = null)`:
  ...
  4. **非 iOS / Editor ガード（即時 Failure に統一。v3 で実装形態を確定）**:
     - native 実呼び出しは `#if UNITY_IOS && !UNITY_EDITOR` に閉じるため、
       Editor / 非 iOS ビルドでは以下の `#else` 経路が採られる。加えてランタイムでも
       `Application.platform != RuntimePlatform.IPhonePlayer`（実機 iOS 以外の Play）をガードする:
       ```csharp
       #if UNITY_IOS && !UNITY_EDITOR
  ...
  5. json 生成は native 呼び出しの直前で行う（`string json = IosShareJsonBuilder.BuildShareContentJson(payload);`）。
     Editor 経路では json を生成せず即時 Failure でよい（生成しても害はない）。
  6. 上記コードブロックのとおり、native 呼び出しは `try/catch` で `Internal error: {ex.Message}` の Failure に変換する。
- （任意）薄い便宜ラッパ:
...
## 9. レビュー指摘の反映（v1 → v2 → v3）

対象レビュー: `artifact/features/share/reviews/2026-07-05-ios-share-design-review.md`
（当該レビューは v1 を対象に実行された「再レビュー」。以下は各指摘の最終確定内容。）
...
- v2 → v3 の主変更: #2 の platform guard 戦略を「クラス全体 `#if UNITY_IOS`」から
  ...
  これにより #5 の Manager 直接検証範囲も拡大した。
- 不足項目（レビュー「不足項目」節）はすべて確定済み。
