...
- v4 変更点: 再レビュー（`...-review-v2.md`）を反映。(1) `Share` 手順の `json` 宣言前参照バグを修正、(2) `InvokeInOrder` の internal 可視化に必要な `Runtime/AssemblyInfo.cs`（`InternalsVisibleTo`）を変更ファイルに追加し §2.2 を訂正、(3) null item は defensive-only と明記（型は `IosShareItem[]` のまま）、(4) 実装順序に v3 追加テストを反映、(5) nm 期待値から固定アドレスを除去。
- v5 変更点: 再レビュー（`...-review-v3.md`）を反映（ドキュメント表記整合）。(1) §3.2 の `Share` シグネチャを nullable payload に更新、(2) §5.4 dispatch 節の曖昧な `InternalsVisibleTo` 記述を `Runtime/AssemblyInfo.cs` 表現へ統一、(3) §6.3 の「v3」注釈を最新化（第 9 章に集約）。
...
- `void Share(IosShareContentPayload? payload, Action<IosShareResult>? onResult = null)`（payload は nullable。§5.4 参照）
...
- **テスト可能性（v2 確定・v3 で guard 整理）**: dispatch の中核ロジック（「共通 → 個別」の順序、
  ...
  - internal 可視化は、`Runtime/AssemblyInfo.cs` に
    `[assembly: InternalsVisibleTo("NativeToolkit.Runtime.Tests")]` を付与して行う（§2.2 / §4.1 と同一）。
...
| 非 iOS プラットフォーム / Editor | `IosShareResult.Failure("iOS share is only available on an iOS device.")`（native 呼び出しなし、即時 Failure）。early-return による無応答は行わない。`#else`（`!UNITY_IOS \|\| UNITY_EDITOR`）経路として実装し EditMode で検証可能 |
...
## 9. レビュー指摘の反映（v1 → v2 → v3 → v4 → v5）

対象レビュー:
- `artifact/features/share/reviews/2026-07-05-ios-share-design-review.md`（v1 対象）→ 下表 #1〜#7
- `artifact/features/share/reviews/2026-07-05-ios-share-design-review-v2.md`（v3 対象）→ 下表 #8〜#12
- `artifact/features/share/reviews/2026-07-05-ios-share-design-review-v3.md`（v4 対象）→ 下表 #13〜#15
...
| 13 | medium | §3.2 の `Share` 一覧が non-nullable のままで §5.4 と不整合 | **【v5】** §3.2 を `Share(IosShareContentPayload? payload, ...)` に更新 | 3.2 |
| 14 | medium | §5.4 dispatch 節の `InternalsVisibleTo` 記述が曖昧（「Runtime asmdef 側で設定」） | **【v5】** `Runtime/AssemblyInfo.cs` に付与する表現へ統一（§2.2 / §4.1 と一致） | 5.4 dispatch |
| 15 | low | §6.3 非 iOS 行の「v3」注釈が古い | **【v5】** バージョン注釈を除去し最新表記に | 6.3 |
...
- v4 → v5 の主変更: #13〜#15 のドキュメント表記整合（実装挙動の変更なし）。
- 対象レビュー: `...-review-v3.md`（v4 対象）→ #13〜#15。
- 不足項目（各レビューの「不足項目」節）はすべて確定済み。
