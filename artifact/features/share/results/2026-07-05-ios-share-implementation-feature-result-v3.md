...
> 痕跡として `manual/1.10.0/images/ios/share/` に、
> サンプル画面を実機で撮った `Example_*` のスクリーンショットが 10 枚ある。
...
参照計画書: `artifact/features/share/designs/2026-07-05-ios-share-design-v5.md`（v1〜v5 のレビュー反映を経て確定）
参照レビュー:
- `artifact/features/share/reviews/2026-07-05-ios-share-implementation-feature-review-v1.md`（v1対象）
- `artifact/features/share/reviews/2026-07-05-ios-share-implementation-feature-review-v2.md`（v2対象）
...
v2 からの変更点（v2レビュー low #1 反映）: `unity-native-plugin.slnx`（Unity自動生成のソリューションファイル）
に PlayMode テスト用 `.csproj` 参照行が CRLF で追加され `git diff --check` が trailing whitespace として
検出していたため、当該行の末尾 `\r` を除去した。**注記**: この `.slnx` は Unity Editor 起動のたびに
自動再生成されるため、Unity Editor を開くと再度 CRLF に戻る可能性がある（実際、本レポート作成中の
テスト再実行でも一度再発し、再修正して解消したことを確認済み）。恒久対応が必要な場合は
`.gitattributes` での改行コード制御等を別途検討する必要がある。
...
- 補足ログ:
  ...
  - **（v3・low #1 対応）** `git diff --check`（develop...HEAD 相当の作業ツリー差分）: 実行結果 SUCCESS
    （trailing whitespace 等の指摘なし）。`unity-native-plugin.slnx` の PlayMode プロジェクト参照行の
    末尾 `\r` を除去して解消したことを確認。
...
- ○ **（v3）** `git diff --check` 通過（`unity-native-plugin.slnx` の trailing whitespace 解消）
