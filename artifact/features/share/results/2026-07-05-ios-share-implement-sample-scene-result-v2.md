...
参照計画書: `artifact/features/share/designs/2026-07-05-ios-share-sample-scene-design-v2.md`
参照レビュー: `artifact/features/share/reviews/2026-07-05-ios-share-implement-sample-scene-review-v1.md`

v1 からの変更点: レビュー指摘（low 1件）を反映。`CreateSampleImage` / `CreateSampleTextFile` の
先頭に `Debug.Log`（引数含む）を追加し、`csharp.md` のログ規約と v1 の自己申告内容を一致させた。
...
- **（v2・レビュー反映）** `CreateSampleImage` / `CreateSampleTextFile` への `Debug.Log` 追加後、
  ユーザーが本プロジェクトで Unity Editor を起動中だったため、バッチモードでの再テスト実行は
  「別の Unity インスタンスが同一プロジェクトを開いている」エラーで実施できなかった。
  変更内容は `AndroidShareManagerExampleController` 等の既存 helper と同一のログ出力パターン
  （`Debug.Log($"[{LogTag}][{nameof(Method)}] param: {param}");` を メソッド先頭に追加するのみ）であり、
  構文・型の観点で問題がないことをコードレビューで確認した。次回バッチテスト実行が可能なタイミングで
  再確認することを推奨する（要検証）。
...
- ○ 変更ファイル一覧との一致
- ○ 共通実装パターンの維持・拡張方針の反映
- ○ コンパイルエラーなし（v1: EditMode/PlayMode テスト実行で確認。v2: バッチ再実行不可のためコードレビューで確認）
- ○ 既存テストの回帰なし（v1 時点: EditMode 110件、PlayMode 5件、全成功）
- ○ **（v2）** `csharp.md` ログ規約の自己申告と実装の一致（`CreateSampleImage`/`CreateSampleTextFile` に
  先頭 `Debug.Log` を追加）
