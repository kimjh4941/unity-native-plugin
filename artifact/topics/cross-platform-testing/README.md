...
その後、操作しなくても前面にならないことが分かった。原因と対応は「S-2（全ボタン）を移した」の「前面」の項にある
...
3. **全ボタンを押す**（S-2）。終わったら Player のログを `scripts/check_windows_clipboard_sample_log.py` に渡して S-2 / S-4 / S-8 を判定する。
   **一部完了**（2026-09-26）。ボタンを選り分けず、テストを丸ごと `Destructive` にした。次の節

S-3 と S-9 は、D と 9 章のテストで実質的に確かめている。

#### S-2（全ボタン）を移した（2026-09-26）

`WindowsClipboardSampleRunPlayerTests` の 2 本。サンプルの画面操作は `WindowsClipboardSampleScreenDriver` に集め、
S-1 / S-5 / S-6 のテストと共有する。

**扱い。** 2 本とも `Destructive` カテゴリで、`verify_unity_windows.sh --include-destructive` のときだけ走る。
...
一覧に書いたボタンを押していたら、一覧が古いとして失敗にする。

**1 回押すごとに待つもの。**

- サンプル自身のログの `#N [accept]` と `[done] ... call=#N` が揃う（受け付けた要求がすべて終わる）まで。上限 10 秒
- クリップボードか履歴を変えるボタン（名前が Copy / Restore / Clear で始まるもの、Paste Html (text only)、Paste Plain Text (after Clear)）の後は、さらに 3 秒。
  **Windows は、すぐ置き換えられた内容を履歴にほとんど残さない。** 間を空けずに押すと、ブロック A 最初の Get History の前の 10 回ほどのコピーで履歴は 0 件、
  手動（人が数秒おきに押す）では 14 件だった。0 件だと Restore Last / Delete Last に渡す ID がなく `InvalidArgument` になる。
  3 秒の後は 8〜9 件になり、結果が手動と揃った。3 秒は人の間合いの代わりで、件数で妥当性を見ている

**ログの受け渡し。** 最初はテスト用 Player の隣にファイルで書いたが、スクリプトからは常に見つからなかった。
テスト用 Player はプロジェクトの `Temp/` の下に作られ、**Editor は終了時に `Temp/` を消す**。
テストはログを自分の出力に `[SampleRun] begin <名前>` と `[SampleRun] end <名前>` で挟んで書き、それが結果の XML に入る。
チェッカーの `--test-results` が XML から切り出し、XML の隣に `windows-clipboard-sample-run-<名前>.log` として保存して判定する。
途中の押下で失敗しても、そこまでのログは出る。

**前面。** 破壊的な実行を始めてから、履歴のテストが毎回 `NotForeground` で失敗した。テスト開始時の `Application.isFocused` はずっと `True` だった。

- ネイティブ側の判定は `GetForegroundWindow()` のウィンドウを持つプロセスが自分かどうか（native-toolkit の `WindowsClipboardHistoryWinRt.cpp` の `IsSelfForeground`）
- 同じ問い方で調べると、**実行の間ずっと前面は VS Code だった**。`isFocused` は、ウィンドウが自分のスレッドの中で active なら `True` になる。前面かどうかの判定には使えない
- Windows は、誰も触っていないプロセスに前面を渡さない。テスト用 Player はスクリプトが裏で起動した Editor から起動されるので、開発者が使っていたウィンドウの後ろに出る。
  開発者から見えたのは「一瞬出て、消えた」
- 対応: 各テストの開始時に、Player が自分のウィンドウを前面に出す（前面のウィンドウの入力に一時的につないで `SetForegroundWindow` を呼び、すぐ外す）。
  人がウィンドウをクリックするのと同じ前提を作るだけで、テスト対象には手を入れない。開始時の状態は `[PlayerTests][focus]` として、前面を持つプロセス名と一緒に残す
- **ブロック A / D は、前面のない押下があれば失敗にする。** 前面のないまま押した回は、履歴のボタンがすべて `NotForeground`（ブロック B の結果）になったのに、
  S-2 は例外とエラー行しか見ないため合格していた
...
- ブロック D: 手動の最初の 7 件（ブロック B / C のボタン）を除けば、違うのは Force Initialize While Draining だけ。
  手動は Initialize がそのまま成功、自動は GetHistory の取り消しの後、2 フレーム目で `ShuttingDown` で断られた
- 2 か所とも、手動の記録（2026-09-09）の後に直したサンプルの動き（`6b161d5`）。前者は、別の形式のコピーが成功すると一時ファイルの一覧まで消していた不具合、
  後者は、シャットダウン中の拒否にそもそも届いていなかった問題。**自動の結果は直した後の正しい動きで、後退ではない**

**再現しなかった失敗（各 1 回）。** どちらも原因は分かっていない。

1. **`GetHistory` が返らなかった。** その回は、前面のないまま走った 2 回分のコピーが履歴に残っていた。返らない要求を抱えたまま次のテストがシャットダウンし、
   8 回試して打ち切り（`shutdown exceeded its budget`）、Manager は `ShuttingDown` のまま戻らず、以降のテストはすべて Initialize で断られた（20 本中 13 本失敗）。
   履歴を手で空にした後の 3 回は再現していない。PowerShell（5.1）から WinRT の履歴を読んで確かめようとしたが、一覧を正しく列挙できず判断材料にならなかった。
   **返らない要求が 1 つあると Manager が戻らなくなる**のが設計どおりかは確認していない
...
**ブロック B（M-13）を移した。** session1 の 56 回目、Delayed History Call（押して 5 秒後に GetHistory を出す）。
手動では 5 秒の間に別のウィンドウを前に出した。テストは押した直後に Player のウィンドウを最小化して前面を手放し、
要求が受け付けられて答えが出るのを待ってから、元に戻して前面を取り直す。他のアプリには触らない。
期待値は `getClipboardHistory NG NotForeground`。最小化しても前面が移らなければテストが失敗する。
押す回そのものは前面で行うので、「前面のない押下は失敗」の規則はそのまま使える。
結果: Player **21/21**、S-2 は PART（61 / 67）、`outcomes blockA` / `blockB` / `blockD` はすべて合格。

**ブロック C の前提を確かめた。** 手動では設定アプリで履歴を切った。レジストリ
`HKCU\Software\Microsoft\Clipboard\EnableClipboardHistory` を 0 にすると、ネイティブ層が見る
`Clipboard.IsHistoryEnabled()` は**その場で** `False` になり、1 に戻すとその場で `True` に戻った（PowerShell から確認、設定アプリは開いていない）。
ただし確かめたのはこの戻り値だけで、履歴サービスが遅延レンダリングの形式を実体化しなくなるか（D-8、M-17〜M-19 の前提）は、次のブロック C の結果で確かめた。

**ブロック C（M-12、M-17、M-18、M-20a）を移した。** 手動の session1 の 57〜64 回目と session2 の 1〜9 回目を、2 本のテストに分けた。

- 履歴は、手動と同じく Disable History Events の後で切る（先に購読を外さないと、停止に失敗して `MonitorRegisterFailed` が残り続ける。設計 2.9）。
  session2 側は最初から切る。テストがレジストリを書き、後片付けの最初に元の値に戻す
- **Player が途中で落ちても履歴が切れたままにならないよう、`verify_unity_windows.sh` も実行前の値を覚えておき、実行後に違っていれば戻して失敗にする**
- session2 は Quit の後の新しいプロセスだった。テストではシーンを読み直すだけで、Manager は同じもの（DontDestroyOnLoad）。結果は手動と同じだった
- **M-18（他のアプリへの貼り付け）は、別プロセスの `Get-Clipboard` をメモ帳の代わりにした。** 遅延レンダリングの形式は、読みに来たプロセスが送るメッセージの中で、
  Player のメインスレッドの provider が答える。そのため読み終わるまでメインスレッドを止めると互いに待つことになるので、フレームを回しながら待つ版を作った
- provider の呼び出し回数はログに出ず、画面の状態行（`Render: text=N image=N`）にだけ出る。手動で人が見ていたのもここ。テストが状態行を読み、
  長さだけを `[SampleRun] #0 [local] test.*` の形でログに書く（S-4 のため中身は書かない）。チェッカーはサンプル自身の行と同じように照合する
- 結果: 自分の貼り付けと別プロセスの貼り付けで長さが同じ、`Render: text=1 image=0`。**履歴が有効なら D-8 で予約直後に全形式が実体化し、image も 1 になるはず**なので、
  レジストリで切った状態は履歴サービスにも効いている
- M-20a は `uninitClipboardManager OK` まで。手動は「リトライ行がない」ことも見ていたが、それは Manager 自身のログ行で、チェッカーの照合の対象外
