...
ただし確かめたのはこの戻り値だけで、履歴サービスが遅延レンダリングの形式を実体化しなくなるか（D-8、M-17〜M-19 の前提）はまだ見ていない。
これはブロック C の結果の照合（M-18 の provider 呼び出し回数）で分かる。

**残り。** ブロック C の自動化（`NotYetAutomated` の 6 個）。
